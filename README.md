...
Imports directly from `data/terms/*.json` in the glossary repo — no copy, no package install required. Each `GlossaryTerm` becomes a graph node. The `related` array becomes edges. Node size scales with connection count.
