...
var core_protocol_default = [
  ...
  },
  {
    id: "tower-bft",
    term: "Tower BFT",
    definition: "Solana's custom BFT consensus algorithm built on top of Proof of History. Tower BFT uses PoH as a clock to reduce communication overhead in traditional PBFT from O(n\xB2) to O(n). Validators vote on forks with exponentially increasing lockout periods\u2014each consecutive vote doubles the lockout, making rollbacks progressively more expensive. A fork is finalized when it reaches supermajority (66.7%+ of stake).",
    category: "core-protocol",
    aliases: ["Tower"],
    related: ["proof-of-history", "vote-account", "fork"]
  },
  {
    id: "slot",
    term: "Slot",
    definition: "A time window during which a designated leader validator can produce a block. Each slot lasts approximately 400 milliseconds. Slots are numbered sequentially from genesis and grouped into epochs of 432,000 slots (~2-3 days). Not every slot produces a block\u2014a skipped slot means the leader was offline or too slow.",
    category: "core-protocol",
    related: ["block", "epoch", "leader-schedule"]
  },
  {
    id: "block",
    term: "Block",
    definition: "A set of entries produced by a leader during a single slot. A block contains transactions bundled into entries, each with a PoH hash proving ordering. Blocks are broken into shreds for network propagation via Turbine. Maximum block size is limited by compute units (48M CU cap per block) rather than byte size.",
    category: "core-protocol",
    related: ["slot", "shred", "entry"]
  },
  {
    id: "epoch",
    term: "Epoch",
  ...
  },
  {
    id: "stake",
    term: "Stake",
    definition: "SOL tokens that are delegated to a validator to increase its voting weight and earn staking rewards. Staking is non-custodial\u2014stakers retain ownership of their SOL. Stake activates/deactivates at epoch boundaries with a warmup/cooldown period. Validators with more stake are assigned more leader slots and have proportionally more influence in consensus.",
    category: "core-protocol",
    related: ["validator", "delegator", "stake-account"]
  },
  {
    id: "stake-account",
    term: "Stake Account",
    definition: "A dedicated on-chain account that holds delegated SOL for staking. Each stake account has a staker authority (can redelegate) and a withdrawer authority (can withdraw). A stake account can only delegate to one validator at a time, so users create multiple stake accounts to split across validators.",
    category: "core-protocol",
    related: ["stake", "delegator", "validator"]
  },
  {
    id: "delegator",
    term: "Delegator",
    definition: "A SOL holder who delegates tokens to a validator's stake without running a node themselves. Delegators earn a share of inflation rewards proportional to their stake, minus the validator's commission (typically 0-10%). Delegation is non-custodial\u2014the delegator retains the ability to redelegate or withdraw at any time.",
    category: "core-protocol",
    related: ["stake", "validator", "stake-account"]
  },
  {
    id: "fork",
    term: "Fork",
    definition: "A divergence in the blockchain where two or more blocks are produced for the same slot or different validators build on different parent blocks. Forks are common during normal operation and are resolved by Tower BFT's fork-choice rule. Only one fork ultimately becomes the canonical chain; transactions on abandoned forks are dropped.",
    category: "core-protocol",
    related: ["tower-bft", "slot", "block"]
  },
  {
    id: "fork-choice",
    term: "Fork Choice",
    definition: "The algorithm validators use to decide which fork to build upon when multiple competing chains exist. Solana combines Tower BFT voting weight (stake-weighted votes with lockout) and a GHOST-like rule to select the fork with the heaviest cumulative stake support.",
    category: "core-protocol",
    related: ["fork", "tower-bft", "ghost"]
  },
  {
    id: "ghost",
    term: "GHOST (Greedy Heaviest Observed SubTree)",
    definition: "Greedy Heaviest Observed SubTree\u2014a fork-choice rule where validators select the fork that has the most cumulative stake-weighted votes. Unlike longest-chain rules, GHOST considers all votes in the tree, not just the tip. Solana's implementation weighs votes by stake and factors in Tower BFT lockout periods.",
    category: "core-protocol",
    aliases: ["GHOST"],
    related: ["fork-choice", "tower-bft"]
  },
  {
    id: "entry",
    term: "Entry",
    definition: "An ordered unit within a block containing a mix of PoH ticks and transaction batches. Each entry includes a PoH hash, a count of hashes since the previous entry, and zero or more transactions. Entries without transactions are tick entries that advance the PoH clock; entries with transactions prove those transactions occurred between two points in time.",
    category: "core-protocol",
    related: ["block", "transaction", "proof-of-history"]
  },
  {
    id: "tick",
    term: "Tick",
    definition: "A PoH entry with zero transactions that advances the clock. There are 64 ticks per slot by default, each representing a fixed number of SHA-256 hash iterations. Ticks establish the passage of time even when no transactions are being processed, maintaining the PoH chain continuity.",
    category: "core-protocol",
    related: ["entry", "proof-of-history", "slot"]
  },
  {
    id: "shred",
    term: "Shred",
    definition: "The smallest unit of block data propagated through the network via Turbine. Blocks are split into shreds of up to 1,228 bytes each (fitting in a single UDP packet). Shreds are Reed-Solomon erasure coded\u2014typically 32 data shreds produce 32 recovery shreds\u2014so blocks can be reconstructed even if up to half the shreds are lost.",
    category: "core-protocol",
    related: ["block", "turbine", "erasure-coding"]
  },
  {
    id: "turbine",
    term: "Turbine",
    definition: "Solana's block propagation protocol inspired by BitTorrent. Instead of the leader sending a full block to every validator, Turbine organizes validators into a layered tree and propagates shreds through successive tiers. Each node forwards shreds to a small fanout set, reducing the leader's bandwidth requirement from O(n) to O(log n).",
    category: "core-protocol",
    related: ["shred", "block-propagation", "erasure-coding"]
  },
  {
    id: "erasure-coding",
    term: "Erasure Coding",
    definition: "A Reed-Solomon error-correction scheme applied to shreds during block propagation. The leader encodes each batch of 32 data shreds into 32 additional recovery (parity) shreds. Any 32 of the 64 total shreds are sufficient to reconstruct the original data, providing 50% loss tolerance during Turbine propagation.",
    category: "core-protocol",
    aliases: ["Reed-Solomon"],
    related: ["shred", "turbine"]
  },
  {
    id: "block-propagation",
    term: "Block Propagation",
    definition: "The process of distributing a newly produced block from the leader to all validators in the cluster. Solana uses Turbine for fast propagation\u2014the leader shreds the block, erasure-codes the shreds, and feeds them into the Turbine tree. Target propagation latency is under 200ms for the full cluster.",
    category: "core-protocol",
    related: ["turbine", "shred", "gossip"]
  },
  {
    id: "gossip",
    term: "Gossip Protocol",
    definition: "A protocol used by validators to exchange metadata and discover peers without relying on a central server. Gossip propagates contact info, vote status, shred versions, epoch slots, and other protocol messages via CRDS. It does not carry transaction or block data\u2014those use TPU and Turbine respectively.",
    category: "core-protocol",
    related: ["cluster", "validator", "crds"]
  },
  {
    id: "crds",
    term: "CRDS (Cluster Replicated Data Store)",
  ...
  },
  {
    ...
    category: "core-protocol",
    aliases: ["TPU"],
    related: ["tvu", "transaction", "leader-schedule"]
  },
  {
    id: "tvu",
    term: "TVU (Transaction Validation Unit)",
    definition: "Transaction Validation Unit\u2014the pipeline on non-leader validators that receives shreds, reconstructs blocks, and replays transactions. TVU stages include: shred-fetch, retransmit, replay (execute transactions and verify results), and vote (submit Tower BFT votes). TVU runs continuously on all validators.",
    category: "core-protocol",
    aliases: ["TVU"],
    related: ["tpu", "validator", "replay-stage"]
  },
  {
    id: "replay-stage",
    term: "Replay Stage",
    definition: "The TVU component that replays received blocks by re-executing all transactions against the bank state. Replay stage verifies that transaction results match the leader's claimed results, updates the bank, and generates Tower BFT votes for valid blocks. If a replayed block is invalid, the validator marks that fork as dead.",
    category: "core-protocol",
    related: ["tvu", "bank", "fork"]
  },
  {
    id: "bank",
    term: "Bank",
    definition: "An in-memory representation of the accounts state at a specific slot. Each slot gets its own Bank that inherits from its parent slot's Bank. The Bank processes transactions, updates account balances, and tracks execution results. Once a slot is complete, the Bank is frozen and can be snapshotted or used as a parent for the next Bank.",
    category: "core-protocol",
    related: ["slot", "replay-stage", "accounts-db"]
  },
  {
    id: "accounts-db",
    term: "AccountsDB",
    definition: "The persistent storage layer for all Solana accounts. AccountsDB stores account data in memory-mapped AppendVec files, indexed by pubkey and slot. It supports fast lookups, handles account deduplication across slots, and periodically cleans up dead accounts. AccountsDB is the largest consumer of validator disk space.",
    category: "core-protocol",
    aliases: ["AccountsDb"],
    related: ["bank", "snapshot", "append-vec"]
  },
  {
    id: "append-vec",
    term: "AppendVec",
    definition: "A memory-mapped, append-only file used by AccountsDB to store account data. Each AppendVec corresponds to a specific slot and contains serialized account entries (pubkey, lamports, data, owner, etc.). AppendVecs are immutable once the slot is rooted, enabling efficient snapshotting and cleanup.",
    category: "core-protocol",
    related: ["accounts-db"]
  },
  {
    ...
    category: "core-protocol",
    related: ["accounts-db", "bank", "validator"]
  },
  {
    id: "blockstore",
    term: "Blockstore",
    definition: "A RocksDB-based database that stores raw shreds and block metadata on the validator's disk. Blockstore indexes shreds by slot and shred index, enabling block reconstruction and historical queries. It is separate from AccountsDB\u2014Blockstore holds the ledger history while AccountsDB holds the current state.",
    category: "core-protocol",
    related: ["block", "shred", "ledger"]
  },
  {
    id: "ledger",
    term: "Ledger",
    definition: "The complete ordered history of all transactions processed by the Solana cluster. The ledger is composed of sequential blocks (one per slot), each containing entries of transactions ordered by PoH. Validators store the ledger locally in Blockstore, and warehouse nodes or Bigtable archives provide long-term historical access.",
    category: "core-protocol",
    related: ["blockstore", "block", "entry"]
  },
  {
    id: "genesis-block",
    term: "Genesis Block",
    definition: "The first block (slot 0) of a Solana cluster that defines initial configuration: the mint account, initial token supply, built-in program deployments, and cluster parameters. The genesis config is generated by the `solana-genesis` tool and shared among all validators to bootstrap a new cluster.",
    category: "core-protocol",
    related: ["ledger", "cluster"]
  },
  {
    id: "sealevel",
    term: "Sealevel",
    definition: "Solana's parallel transaction execution engine. Sealevel can process thousands of transactions simultaneously by analyzing each transaction's declared account inputs\u2014transactions that don't touch the same writable accounts run in parallel across available CPU cores. This account-level parallelism is what enables Solana's high throughput.",
    category: "core-protocol",
    aliases: ["SVM Runtime"],
    related: ["svm", "parallel-execution", "transaction"]
  },
  {
    id: "svm",
    term: "Solana Virtual Machine (SVM)",
    definition: "The Solana Virtual Machine\u2014the execution environment that runs on-chain programs. SVM loads SBF bytecode, provides syscalls for account access and cryptographic operations, enforces compute budgets, and manages memory. The SVM is being modularized (via the SVM API) to enable use in rollups and other environments outside the main Solana validator.",
    category: "core-protocol",
    aliases: ["SVM"],
    related: ["sealevel", "bpf", "sbf"]
  },
  {
    id: "bpf",
    term: "BPF (Berkeley Packet Filter)",
    definition: "Berkeley Packet Filter\u2014the original bytecode format used for Solana programs, inherited from Linux's eBPF. Programs written in Rust or C are compiled to BPF bytecode and executed in a sandboxed VM. BPF has been superseded by SBF on Solana but the tooling names (BPF Loader, cargo build-bpf) persist for historical reasons.",
    category: "core-protocol",
    aliases: ["BPF", "eBPF"],
    related: ["sbf", "svm"]
  },
  {
    id: "sbf",
    term: "SBF (Solana Bytecode Format)",
    definition: "Solana Bytecode Format\u2014Solana's customized evolution of BPF. SBF adds Solana-specific syscalls, modifies calling conventions, disables certain eBPF instructions, and adds features like position-independent code. Programs are compiled with `cargo build-sbf` and deployed as SBF ELF binaries. SBF replaced BPF as the canonical bytecode format.",
    category: "core-protocol",
    aliases: ["SBF"],
    related: ["bpf", "svm"]
  },
  {
    id: "vdf",
    term: "VDF (Verifiable Delay Function)",
    definition: "Verifiable Delay Function\u2014a function that takes a prescribed amount of sequential computation to evaluate but whose output can be quickly verified. Solana's Proof of History uses a SHA-256 hash chain as a practical VDF: computing the chain is inherently sequential, but verifying any segment can be parallelized across multiple cores.",
    category: "core-protocol",
    aliases: ["VDF"],
    related: ["proof-of-history", "sha256-hash-chain"]
  },
  {
    id: "sha256-hash-chain",
    term: "SHA-256 Hash Chain",
    definition: "The core mechanism of Proof of History. A SHA-256 hash is computed sequentially\u2014each hash takes the previous hash as input\u2014creating an ordered, unforgeable timestamp sequence. The PoH generator runs approximately 400,000 hashes per slot. Data (transactions) can be inserted into the chain by mixing their hash with the current state.",
    category: "core-protocol",
    related: ["proof-of-history", "vdf", "tick"]
  },
  {
    id: "finality",
    term: "Finality",
    definition: "The guarantee that a transaction cannot be reversed. Solana offers two levels: optimistic confirmation (~400ms, 66.7%+ stake voted) and full finality (~12-13 seconds, 31+ confirmations rooted). Once a slot is rooted, its transactions are irreversible under the assumption that no more than 1/3 of stake is malicious.",
    category: "core-protocol",
    related: ["confirmation", "optimistic-confirmation", "rooted"]
  },
  {
    id: "confirmation",
    term: "Confirmation",
    definition: "A vote by a validator on a block, weighted by the validator's stake. The more stake-weighted confirmations a slot receives, the higher the confidence it will be finalized. RPC methods accept commitment levels (processed, confirmed, finalized) to specify the required confirmation threshold.",
    category: "core-protocol",
    related: ["finality", "optimistic-confirmation"]
  },
  {
    id: "optimistic-confirmation",
    term: "Optimistic Confirmation",
    definition: "A transaction confirmation level achieved when 66.7%+ of total stake has voted on the slot, typically within ~400ms. Optimistic confirmation provides high probability of finality without waiting for full root depth. It corresponds to the 'confirmed' commitment level in RPC queries.",
    category: "core-protocol",
    related: ["confirmation", "finality"]
  },
  {
    id: "rooted",
    term: "Rooted",
    definition: "A slot that has been finalized by achieving maximum lockout in Tower BFT\u2014meaning 31+ consecutive confirmations have been built on top of it. Once rooted, a slot and its transactions are considered irreversible. Rooted corresponds to the 'finalized' commitment level and takes approximately 12-13 seconds.",
    category: "core-protocol",
    related: ["finality", "slot"]
  },
  {
    ...
    related: ["finality", "rpc"]
  },
  {
    id: "lockout",
    term: "Lockout",
    definition: "The period during which a Tower BFT vote prevents a validator from switching to a competing fork. Lockouts double with each consecutive confirmation: vote depth 1 = 2-slot lockout, depth 2 = 4 slots, etc. This exponential lockout makes it increasingly expensive for validators to change their vote, providing economic finality.",
    category: "core-protocol",
    related: ["tower-bft", "vote-account"]
  },
  {
    id: "supermajority",
    term: "Supermajority",
  ...
  },
  {
    id: "cluster",
    term: "Cluster",
    definition: "A set of validators working together to maintain a single Solana ledger. Solana operates three main clusters: mainnet-beta (production), devnet (development), and testnet (stress testing). Each cluster has its own genesis block, token supply, and independent validator set. Validators in a cluster discover each other via the gossip protocol.",
    category: "core-protocol",
    related: ["validator", "gossip", "genesis-block"]
  },
  {
    id: "runtime",
    term: "Runtime",
    definition: "The component of the SVM responsible for loading programs, enforcing account access rules (ownership, writability, signer checks), managing compute budgets, and executing transactions. The runtime ensures programs can only modify accounts they own, charges rent, and enforces the 10MB per-transaction account data limit.",
    category: "core-protocol",
    related: ["svm", "sealevel", "transaction"]
  },
  {
    id: "firedancer",
    term: "Firedancer",
    definition: "A from-scratch Solana validator client written in C by Jump Crypto. Firedancer aims for significant performance improvements through a tiled, zero-copy architecture and hardware-optimized networking. It provides client diversity\u2014critical for network resilience\u2014and targets 1M+ TPS. Frankendancer is the intermediate version running Firedancer's networking stack with the Agave execution engine.",
    category: "core-protocol",
    aliases: ["FD"],
    related: ["validator", "frankendancer"]
  },
  {
    id: "frankendancer",
    term: "Frankendancer",
    definition: "A hybrid Solana validator client that combines Firedancer's high-performance networking and block production components (written in C by Jump Crypto) with Agave's execution and replay engine. Frankendancer serves as a stepping stone toward full Firedancer, allowing validators to benefit from networking improvements while maintaining execution compatibility.",
    category: "core-protocol",
    related: ["firedancer", "validator"]
  },
  {
    id: "agave",
    term: "Agave",
    definition: "The Rust-based Solana validator client maintained by Anza (formerly Solana Labs). Agave is the original and most widely deployed validator implementation. It handles all aspects of validation: TPU, TVU, gossip, AccountsDB, and RPC. The name 'Agave' was adopted to distinguish it from the Firedancer client.",
    category: "core-protocol",
    aliases: ["Anza Agave"],
    related: ["validator", "firedancer"]
  },
  {
    id: "jito-validator",
    term: "Jito Validator Client",
    definition: "A modified fork of the Agave validator client maintained by Jito Labs that adds an out-of-protocol block space auction. Jito's modifications include a block engine that accepts transaction bundles with tips, enabling MEV extraction while returning value to stakers. The majority of mainnet-beta stake runs the Jito client.",
    category: "core-protocol",
    aliases: ["Jito-Solana"],
    related: ["agave", "validator", "mev"]
  },
  {
    id: "simd",
    term: "SIMD (Solana Improvement Document)",
    definition: "A formal specification document describing proposed and accepted changes to the Solana protocol, analogous to Ethereum's EIPs. SIMDs are categorized as Standard (Core, Networking, Interface) or Meta, and require supermajority validator stake-weighted voting (66.67%) to pass. Notable SIMDs include SIMD-33 (Timely Vote Credits), SIMD-123 (Block Revenue Sharing), SIMD-228 (Market-Based Emissions), and SIMD-326 (Alpenglow).",
    category: "core-protocol",
    aliases: ["Solana Improvement Document"],
    related: ["validator", "consensus", "epoch"]
  },
  {
    id: "alpenglow",
    term: "Alpenglow",
    definition: "A next-generation consensus protocol proposed in SIMD-0326 that aims to collapse Solana's legacy systems (Proof of History, Tower BFT, gossip-based vote propagation) into two streamlined mechanisms: Rotor for data propagation and Votor for off-chain voting. Alpenglow targets reducing transaction finality from ~12.8 seconds to under 150 milliseconds, representing the most significant consensus overhaul in Solana's history.",
    category: "core-protocol",
    aliases: ["SIMD-326"],
    related: ["consensus", "finality", "tower-bft", "proof-of-history"]
  },
  {
    id: "simd-123",
    term: "SIMD-123 (Block Revenue Sharing)",
    definition: "A protocol change enabling validators to automatically share block revenue (priority fees and MEV tips) with their delegators through an on-chain mechanism. Validators set a commission rate for block revenue, and the protocol distributes the remainder to delegators at epoch boundaries based on their stake proportion. Available in Agave 3.0+.",
    category: "core-protocol",
    aliases: ["Block Revenue Sharing"],
    related: ["validator", "delegator", "priority-fee", "mev"]
  },
  {
    id: "anza",
    term: "Anza",
    definition: "A development organization that spun out of Solana Labs in early 2024 to independently maintain and develop the Agave validator client and core Solana toolchain. Anza also maintains the @solana/kit SDK (formerly web3.js v2), the Anchor framework, and the SVM API. The spin-off was intended to decentralize Solana's core development and improve client diversity.",
    category: "core-protocol",
    aliases: ["Anza Technology"],
    related: ["agave", "firedancer", "solana-web3js-v2"]
  },
  {
    id: "eclipse",
    term: "Eclipse",
    definition: "A modular Layer-2 rollup launched on Ethereum mainnet in November 2024 that uses the Solana Virtual Machine (SVM) for execution, Celestia for data availability, and RISC Zero for zero-knowledge proofs. Eclipse brings SVM's parallel execution performance to Ethereum users with faster speeds and lower fees, hosting 60+ dApps across DeFi, gaming, and digital services.",
    category: "core-protocol",
    aliases: ["Eclipse L2"],
    related: ["svm", "rollup", "layer-2"]
  },
  {
    id: "sonic-svm",
    term: "Sonic SVM",
    definition: "The first chain extension (appchain) on Solana, purpose-built for gaming applications using the HyperGrid framework. Sonic enables sovereign game economies that process millions of requests per second while settling back to Solana L1, reducing transaction costs by up to 100x compared to regular Solana transactions while maintaining native composability.",
    category: "core-protocol",
    aliases: ["Sonic", "SONIC"],
    related: ["svm", "gaming", "network-extension"]
  },
  {
    id: "network-extension",
    term: "Network Extension",
    definition: "A Solana-native scaling paradigm where an appchain runs its own SVM execution environment but settles state back to Solana L1, maintaining composability with Solana's programs and liquidity. Unlike general-purpose rollups that settle to other chains, network extensions are designed to scale specific applications (gaming, DeFi) within the Solana ecosystem.",
    category: "core-protocol",
    aliases: ["Chain Extension"],
    related: ["sonic-svm", "svm", "rollup"]
  },
  {
    id: "block-height",
    term: "Block Height",
    definition: "The number of blocks produced in the ledger since the genesis block. Block height increments only for slots that successfully produce a block (skipped slots do not increment it), so block height is always less than or equal to the current slot number. Block height is accessible via the getBlockHeight RPC method and the Clock sysvar.",
    category: "core-protocol",
    related: ["block", "slot", "genesis-block"]
  },
  {
    id: "bootstrap-validator",
  ...
  {
    id: "fast-finalization",
    term: "Fast Finalization",
    definition: "The sub-second transaction finality goal of the Alpenglow consensus protocol, targeting approximately 150 milliseconds from block production to irreversible confirmation. Fast finalization is achieved by replacing Tower BFT's exponential lockout mechanism (which requires ~31 confirmations over ~12.8 seconds) with Alpenglow's certificate-based approach where a single round of supermajority voting produces an irrevocable finality proof.",
    category: "core-protocol",
    related: ["alpenglow", "alpenglow-certificate", "finality"]
  },
  {
    id: "core-bpf-programs",
    term: "Core BPF Programs",
  ...
  },
  {
    ...
    category: "core-protocol",
    related: ["ed25519", "secp256k1-precompile", "signature"]
  },
  {
    id: "program-cache",
    term: "Program Cache",
    definition: "A runtime cache that stores recently loaded and JIT-compiled program binaries in memory, avoiding the cost of re-loading and re-compiling SBF ELF binaries from AccountsDB on every invocation. The program cache significantly reduces execution latency for frequently called programs. Cache entries are keyed by program ID and slot, and entries are evicted based on usage patterns and slot advancement.",
    category: "core-protocol",
    related: ["runtime", "svm", "program"]
  },
  {
    id: "call-depth-limit",
    term: "Call Depth Limit",
    definition: "The maximum depth of 4 nested cross-program invocations (CPIs) allowed within a single Solana transaction. Program A can call Program B, which can call Program C, which can call Program D, but no further nesting is permitted. This limit prevents unbounded recursion, stack overflows, and excessive compute consumption. The runtime enforces this limit and returns an error if exceeded.",
    category: "core-protocol",
    related: ["cpi", "invoke", "runtime"]
  },
  {
    id: "heap-memory",
    term: "Heap Memory",
    definition: "The dynamic memory region available to SBF programs during execution, defaulting to 32KB. Programs use heap memory for dynamic allocations (Vec, String, Box). The heap size can be increased up to 256KB by requesting additional compute units via the Compute Budget Program's RequestHeapFrame instruction. Heap exhaustion causes the program to fail with an AccessViolation error. Efficient programs minimize heap usage through zero-copy patterns and stack-allocated buffers.",
    category: "core-protocol",
    related: ["compute-budget", "sbf", "runtime"]
  },
  {
    id: "nakamoto-consensus",
    term: "Nakamoto Consensus",
    definition: "Consensus family where the longest (or heaviest) valid chain is considered canonical, originally used in Bitcoin's Proof of Work. Participants probabilistically converge on a single chain without explicit voting rounds. Solana's Tower BFT is not Nakamoto consensus \u2014 it uses explicit BFT voting with lockout periods rather than chain-length selection, providing deterministic rather than probabilistic finality.",
    category: "core-protocol",
    related: ["consensus", "tower-bft", "finality", "fork-choice"]
  },
  {
    id: "flp-impossibility",
    term: "FLP Impossibility",
    definition: "Fischer-Lynch-Paterson impossibility result (1985): no deterministic consensus protocol can guarantee agreement in an asynchronous system with even one faulty process. All practical BFT protocols (including Tower BFT) circumvent FLP by introducing synchrony assumptions \u2014 Tower BFT uses Proof of History as a clock to break pure asynchrony.",
    category: "core-protocol",
    aliases: ["FLP Theorem"],
    related: [
      "consensus",
...
var programming_model_default = [
  {
    ...
    related: ["pubkey", "lamport", "rent"]
  },
  {
    id: "program",
    term: "Program",
    definition: "Executable code deployed on-chain, equivalent to a smart contract on other blockchains. Programs are stateless\u2014they store no data themselves but read/write data in separate accounts they own. Programs are compiled to SBF bytecode and loaded via the BPF Loader. Every program has a unique Program ID (its account's public key).",
    category: "programming-model",
    aliases: ["Smart Contract"],
    related: ["account", "instruction", "bpf"]
  },
  {
    id: "instruction",
    term: "Instruction",
    definition: "A single operation within a transaction that invokes a program. An instruction specifies: (1) the program ID to call, (2) an array of account metas (pubkey, is_signer, is_writable), and (3) an opaque data byte array. Programs decode the instruction data to determine which operation to perform.",
    category: "programming-model",
    aliases: ["IX"],
    related: ["transaction", "program", "account"]
  },
  {
    id: "transaction",
    term: "Transaction",
    definition: "An atomic unit of execution containing one or more instructions, a recent blockhash, and one or more signatures. All instructions in a transaction execute sequentially and atomically\u2014if any instruction fails, the entire transaction reverts. Transactions have a 1,232-byte size limit (matching IPv6 MTU) and a default 200,000 CU budget.",
    category: "programming-model",
    aliases: ["TX"],
    related: ["instruction", "signature", "message"]
  },
  {
    id: "versioned-transaction",
    term: "Versioned Transaction",
    definition: "A transaction format introduced with v0 that supports address lookup tables. Legacy transactions include all pubkeys inline (limited to ~35 accounts); v0 transactions reference ALTs to include up to 256 accounts while staying within the 1,232-byte limit. The version byte (0) precedes the message.",
    category: "programming-model",
    aliases: ["v0 Transaction"],
    related: ["transaction", "address-lookup-table"]
  },
  {
    id: "message",
    term: "Message",
    definition: "The unsigned payload of a transaction containing: a header (signer/read-only counts), an array of account public keys, a recent blockhash, and compiled instructions. Signers sign the serialized message bytes. In v0 messages, an additional section references address lookup tables.",
    category: "programming-model",
    related: ["transaction", "instruction"]
  },
  {
    id: "signature",
    term: "Signature",
    definition: "A 64-byte Ed25519 digital signature over a transaction's message. Each signer in a transaction produces one signature. The first signature (from the fee payer) also serves as the unique transaction ID. Signature verification is parallelized in the TPU's sigverify stage using GPU acceleration.",
    category: "programming-model",
    related: ["transaction", "keypair", "ed25519"]
  },
  {
    id: "ed25519",
    term: "Ed25519",
    definition: "The elliptic curve digital signature algorithm used by Solana for all transaction signing. Ed25519 uses Curve25519 and produces 64-byte signatures with 32-byte public keys. Solana also provides an Ed25519 precompile program for on-chain signature verification within program logic.",
    category: "programming-model",
    related: ["signature", "keypair"]
  },
  {
    id: "keypair",
    term: "Keypair",
    ...
    category: "programming-model",
    related: ["pubkey", "signature", "wallet-general"]
  },
  {
    id: "pubkey",
    term: "Public Key (Pubkey)",
  ...
  },
  {
    ...
    category: "programming-model",
    aliases: ["PDA"],
    related: ["seeds", "bump", "find-program-address"]
  },
  {
    id: "seeds",
    term: "Seeds",
    definition: "Byte arrays used as inputs to derive a Program Derived Address. Seeds can be any combination of static strings, user pubkeys, mint addresses, or other identifiers (each seed max 32 bytes, up to 16 seeds). For example, seeds=[b'vault', user.key()] derives a unique vault PDA for each user.",
    category: "programming-model",
    related: ["pda", "bump"]
  },
  {
  ...
  {
    id: "find-program-address",
    term: "find_program_address",
    definition: "A function that derives a PDA by iterating bump seeds from 255 down to 0 until finding a pubkey not on the Ed25519 curve. It returns (pubkey, bump). In Anchor, the #[account(seeds=[...], bump)] constraint calls this automatically. The corresponding create_program_address skips iteration and takes an explicit bump.",
    category: "programming-model",
    related: ["pda", "seeds", "bump"]
  },
  {
    id: "cpi",
    ...
    category: "programming-model",
    aliases: ["CPI"],
    related: ["program", "instruction", "invoke", "invoke-signed"]
  },
  {
    id: "invoke",
    term: "invoke()",
    definition: "The Solana SDK function used to execute a CPI without PDA signing: `invoke(instruction, account_infos)`. All required signers must have already signed the outer transaction. Use invoke when calling programs that only need signatures from user wallets, not from PDAs.",
    category: "programming-model",
    related: ["cpi", "invoke-signed"]
  },
  {
    id: "invoke-signed",
    term: "invoke_signed()",
    definition: "The Solana SDK function for CPIs where a PDA must sign: `invoke_signed(instruction, account_infos, signer_seeds)`. The runtime verifies that the seeds + bump produce the expected PDA pubkey and grants signer privileges for that account. This is how programs authorize actions on behalf of PDAs they own.",
    category: "programming-model",
    related: ["cpi", "invoke", "pda"]
  },
  {
    id: "system-program",
    term: "System Program",
    definition: "The built-in native program (address: 11111111111111111111111111111111) responsible for creating new accounts, transferring SOL between accounts, allocating account data space, and assigning account ownership to programs. It is the only program that can create new accounts and is invoked in nearly every transaction.",
    category: "programming-model",
    related: ["program", "create-account", "transaction"]
  },
  {
    id: "create-account",
    term: "Create Account",
    definition: "A System Program instruction that creates a new account with specified space and owner, funding it with enough lamports for rent exemption. Parameters: from (fee payer), to (new account keypair), lamports, space (bytes), and owner (program ID). The new account's data is zero-initialized.",
    category: "programming-model",
    related: ["system-program", "rent", "lamport"]
  },
  {
    id: "lamport",
    term: "Lamport",
    definition: "The smallest unit of SOL, named after Leslie Lamport. 1 SOL = 1,000,000,000 (10^9) lamports. All balances and transfers on Solana are denominated in lamports internally. The minimum balance for an account to be rent-exempt is calculated in lamports based on the account's data size.",
    category: "programming-model",
    related: ["sol", "rent"]
  },
  {
    id: "sol",
    term: "SOL",
    definition: "The native token of the Solana blockchain, used for paying transaction fees, staking, and rent. 1 SOL equals 1 billion lamports. SOL has an inflationary supply schedule starting at 8% annually, decreasing by 15% per year, with a long-term floor of 1.5%. Transaction base fees (5,000 lamports) are partially burned.",
    category: "programming-model",
    related: ["lamport", "native-token"]
  },
  {
    id: "rent",
    term: "Rent",
    definition: "A fee mechanism that charges accounts for storing data on-chain. Accounts must maintain a minimum lamport balance proportional to their data size (~6.96 SOL per MB) to be rent-exempt. Since 2022, all new accounts must be rent-exempt at creation. Accounts falling below the threshold are removed during rent collection.",
    category: "programming-model",
    related: ["rent-exemption", "account", "lamport"]
  },
  {
    id: "rent-exemption",
    term: "Rent Exemption",
    definition: "The state where an account holds enough lamports to cover two years of rent, making it effectively permanent. The minimum balance is approximately 0.00089088 SOL per byte of data. Since Solana requires all new accounts to be rent-exempt, this is the minimum funding required when creating accounts.",
    category: "programming-model",
    related: ["rent", "account"]
  },
  {
    id: "owner",
    term: "Owner (Account)",
    definition: "The program that has exclusive write access to an account's data and can debit its lamport balance. Every account has an owner field (a program pubkey). The System Program owns all new/wallet accounts. When a program creates a derived account, it typically assigns itself as owner to manage that account's state.",
    category: "programming-model",
    related: ["program", "account", "authority"]
  },
  {
    id: "authority",
    term: "Authority",
    definition: "A pubkey with administrative privileges over a resource\u2014such as the mint authority (can mint tokens), freeze authority (can freeze accounts), upgrade authority (can upgrade a program), or the signer authorized to perform operations on a PDA-controlled account. Authority is a convention, not a runtime concept.",
    category: "programming-model",
    related: ["owner", "signer", "multisig"]
  },
  {
    id: "signer",
    term: "Signer",
    definition: "An account that has provided a valid Ed25519 signature in the current transaction. The runtime enforces the is_signer flag on instruction account metas\u2014if an instruction declares an account as a signer, the transaction must include its signature. Missing signer checks are a common vulnerability in Solana programs.",
    category: "programming-model",
    related: ["authority", "keypair", "transaction"]
  },
  {
    id: "writable",
    term: "Writable Account",
    definition: "An account marked as writable in an instruction's account metas, allowing the program to modify its data or lamport balance. Non-writable accounts are read-only. The runtime enforces that programs can only modify writable accounts. Writable accounts also affect scheduling\u2014transactions sharing writable accounts cannot run in parallel.",
    category: "programming-model",
    related: ["account", "transaction"]
  },
  {
    id: "executable",
    term: "Executable Account",
    definition: "An account flag indicating the account contains deployed program bytecode. Once marked executable, an account cannot be modified except through the upgrade mechanism. The data field contains the SBF ELF binary (or for upgradeable programs, a pointer to the program data account).",
    category: "programming-model",
    related: ["program", "bpf-loader", "upgradeable-loader"]
  },
  {
    id: "bpf-loader",
    term: "BPF Loader",
    definition: "A built-in program that loads and executes SBF programs. Solana has multiple loader versions: the deprecated BPF Loader (immutable programs), BPF Loader 2 (deprecated), and the current Upgradeable BPF Loader (BPFLoaderUpgradeable) that supports program upgrades. The loader verifies bytecode, manages deployment, and sets the executable flag.",
    category: "programming-model",
    related: ["program", "executable", "upgradeable-loader"]
  },
  {
    id: "upgradeable-loader",
    term: "Upgradeable BPF Loader",
    definition: "The current BPF Loader (BPFLoaderUpgradeab1e111...) that supports deploying and upgrading programs. It creates three accounts: the program account (thin proxy with executable flag), the program data account (holds the actual ELF bytecode), and records the upgrade authority. Programs can be made immutable by setting the authority to None.",
    category: "programming-model",
    aliases: ["BPFLoaderUpgradeable"],
    related: ["bpf-loader", "program-data", "upgrade-authority"]
  },
  {
    id: "program-data",
    term: "Program Data Account",
    definition: "The account that stores the actual SBF ELF bytecode for an upgradeable program. It is separate from the program's main account (which just points to the program data). This separation allows the bytecode to be swapped during upgrades while the program ID stays the same. The program data account also stores the upgrade authority pubkey.",
    category: "programming-model",
    related: ["upgradeable-loader", "program"]
  },
  {
    id: "upgrade-authority",
    term: "Upgrade Authority",
    definition: "The pubkey authorized to upgrade a program's bytecode via the Upgradeable BPF Loader. Only the upgrade authority can deploy new bytecode to the program data account. Setting the upgrade authority to None makes the program immutable. Multisig (e.g., Squads) is commonly used to manage upgrade authority for production programs.",
    category: "programming-model",
    related: ["upgradeable-loader", "program"]
  },
  {
  ...
  },
  {
    ...
    category: "programming-model",
    related: ["clock", "rent-exemption", "epoch-schedule"]
  },
  {
    id: "clock",
    term: "Clock Sysvar",
  ...
  },
  {
    id: "account-data",
    term: "Account Data",
    definition: "The byte array stored in an account that holds program-specific state. Data is typically serialized using Borsh and must be explicitly allocated at account creation. The maximum data size is 10MB. Programs are responsible for defining and managing their own data layout, including discriminators for type identification.",
    category: "programming-model",
    related: ["account", "serialization", "borsh"]
  },
  {
    id: "borsh",
    term: "Borsh",
    definition: "Binary Object Representation Serializer for Hashing\u2014the standard serialization format for Solana program data. Borsh produces deterministic, compact binary encodings with a fixed schema. It's used by Anchor for all account and instruction data serialization/deserialization. Borsh supports structs, enums, vectors, and other Rust types.",
    category: "programming-model",
    aliases: ["Binary Object Representation Serializer for Hashing"],
    related: ["serialization", "account-data"]
  },
  {
    id: "serialization",
    term: "Serialization / Deserialization",
    definition: "The process of converting in-memory data structures to bytes (serialization) and back (deserialization) for on-chain storage. Solana programs primarily use Borsh, though some use bincode or custom formats. Anchor's #[account] macro auto-derives Borsh serialization. Incorrect deserialization (e.g., missing length checks) is a common vulnerability class.",
    category: "programming-model",
    aliases: ["Serde"],
    related: ["borsh", "account-data"]
  },
  {
    id: "address-lookup-table",
    term: "Address Lookup Table (ALT)",
    definition: "An on-chain account that stores up to 256 pubkeys, enabling v0 transactions to reference accounts by 1-byte index instead of full 32-byte pubkeys. ALTs dramatically reduce transaction size when interacting with many accounts (e.g., DEX routing). ALTs must be created and populated in advance; entries become active after a slot delay.",
    category: "programming-model",
    aliases: ["ALT", "LUT"],
    related: ["versioned-transaction", "transaction-size"]
  },
  {
    id: "transaction-size",
    term: "Transaction Size Limit",
    definition: "The maximum serialized size of a Solana transaction: 1,232 bytes, matching the IPv6 minimum MTU to ensure single-packet transmission. This limit constrains the number of accounts, instructions, and data per transaction. Address lookup tables (v0 transactions) help fit more accounts within this limit.",
    category: "programming-model",
    related: ["transaction", "address-lookup-table", "versioned-transaction"]
  },
  {
    id: "compute-budget",
    term: "Compute Budget",
    definition: "The maximum compute units a transaction can consume. Default is 200,000 CU per instruction (max 1.4M CU per transaction). Programs can request specific limits via the ComputeBudgetProgram's SetComputeUnitLimit instruction. Under-requesting CU risks transaction failure; over-requesting wastes priority fee budget.",
    category: "programming-model",
    aliases: ["CU Budget"],
    related: ["compute-units", "priority-fee"]
  },
  {
    id: "compute-units",
    term: "Compute Units (CU)",
    definition: "A measure of computational resources consumed by transaction execution, analogous to gas on Ethereum. Each BPF instruction costs CU; syscalls have predefined CU costs (e.g., SHA-256: 85 CU base + per-byte). Default per-instruction limit is 200,000 CU; max per-transaction is 1,400,000 CU. Priority fee cost = CU price \xD7 CU consumed.",
    category: "programming-model",
    aliases: ["CU"],
    related: ["compute-budget", "priority-fee"]
  },
  {
    id: "log",
    term: "Program Log (msg! / sol_log)",
    definition: "Diagnostic output from programs using `msg!()` (Anchor) or `sol_log()` (native). Logs appear in transaction metadata and explorer UIs. Each log call costs compute units (~100 CU base + data length). Logs are limited to 10KB per transaction. Anchor events also emit as base64-encoded log messages.",
    category: "programming-model",
    related: ["compute-units", "program"]
  },
  {
    id: "return-data",
    term: "Return Data (set_return_data)",
  ...
  },
  {
    ...
    category: "programming-model",
    related: ["anchor", "instruction", "account-data"]
  },
  {
    id: "idl",
    ...
    category: "programming-model",
    aliases: ["IDL"],
    related: ["anchor", "program"]
  },
  {
    id: "close-account",
    term: "Close Account",
    definition: "The process of reclaiming an account's lamports and marking it for deletion. To close an account: transfer all lamports to a recipient, zero out the data, and (in Anchor) set the discriminator to a closed sentinel. The runtime garbage-collects zero-lamport accounts. Failing to zero data enables revival attacks.",
    category: "programming-model",
    related: ["rent", "lamport", "account"]
  },
  {
    id: "fee-account",
    term: "Fee Account",
    definition: "The account designated to pay the transaction fee, always the first signer listed in the transaction's account keys array. The fee account must hold enough SOL to cover the base fee (5,000 lamports per signature) plus any priority fee. If the fee account's balance is insufficient, the transaction is rejected before execution.",
    category: "programming-model",
    related: ["transaction", "base-fee", "lamport"]
  },
  {
    id: "inner-instruction",
    ...
    related: ["cpi", "instruction", "invoke"]
  },
  {
    id: "instruction-handler",
    term: "Instruction Handler",
    definition: "The specific function within a program that processes a particular instruction variant. In native Solana programs, the entrypoint dispatches to different handler functions based on instruction data. In Anchor, each pub fn in the #[program] module is an instruction handler that is automatically routed to via the 8-byte discriminator prefix in the instruction data.",
    category: "programming-model",
    related: ["instruction", "entrypoint", "discriminator"]
  },
  {
    id: "loader",
    term: "Loader",
    definition: "A built-in program responsible for deploying, loading, and executing on-chain programs by interpreting their binary encoding. Solana has multiple loaders: the original BPF Loader (deprecated, immutable programs), BPF Loader 2 (deprecated), and the current Upgradeable BPF Loader that supports program upgrades and manages the program/program-data account split.",
    category: "programming-model",
    related: ["bpf-loader", "upgradeable-loader", "program"]
  },
  {
    id: "private-key",
    term: "Private Key",
    definition: "The secret 32-byte component of an Ed25519 keypair used to produce digital signatures. The private key must be kept confidential \u2014 anyone with access to it can sign transactions and transfer assets from the corresponding account. Private keys are typically stored in JSON files (as 64-byte arrays including the public key) by the Solana CLI or encrypted within wallet software.",
    category: "programming-model",
    related: ["keypair", "pubkey", "ed25519"]
  },
  {
    id: "transaction-id",
    term: "Transaction ID",
    definition: "The first signature in a transaction, which uniquely identifies it on the network. Since each transaction has a unique combination of signers and a recent blockhash, the first signer's Ed25519 signature over the transaction message serves as a globally unique identifier. Transaction IDs are displayed as Base58-encoded 64-byte strings and used for lookups via getTransaction and getSignatureStatuses.",
    category: "programming-model",
    aliases: ["TXID"],
  ...
  {
    id: "realloc",
    term: "Realloc (Account Resize)",
    definition: "The process of resizing an on-chain account's data allocation after creation. Realloc can increase or decrease the data field size, but each instruction can increase by at most 10,240 bytes (10KB). The account owner program must sign the realloc, and if the account grows, additional lamports must be provided to maintain rent exemption. In Anchor, the realloc constraint on #[account(mut, realloc = new_size, realloc::payer = payer, realloc::zero = false)] handles this automatically.",
    category: "programming-model",
    related: ["account", "rent-exemption", "account-data"]
  },
  {
    id: "micro-lamport",
    term: "Micro-Lamport",
    definition: "The unit used to express compute unit pricing for priority fees on Solana. One lamport equals 1,000,000 micro-lamports. When setting a compute unit price via the Compute Budget Program, the price is specified in micro-lamports per compute unit. The total priority fee is calculated as (compute unit price in micro-lamports * compute units consumed) / 1,000,000, rounded down to lamports.",
    category: "programming-model",
    related: ["lamport", "compute-unit-price", "priority-fee"]
  },
  {
    id: "compute-unit-price",
    term: "Compute Unit Price",
    definition: "The price per compute unit in micro-lamports that a transaction is willing to pay as a priority fee. Set via the Compute Budget Program's SetComputeUnitPrice instruction. Higher prices increase the likelihood of a transaction being included by the leader during congestion. The effective priority fee equals the compute unit price multiplied by the compute units consumed, converted from micro-lamports to lamports.",
    category: "programming-model",
    related: ["micro-lamport", "compute-units", "priority-fee"]
  },
  {
    id: "compute-budget-program",
    term: "Compute Budget Program",
    ...
    category: "programming-model",
    related: ["compute-budget", "compute-units", "priority-fee"]
  },
  {
    id: "max-cu-per-transaction",
    term: "Max Compute Units per Transaction",
    definition: "The hard upper limit of 1,400,000 compute units that a single Solana transaction can consume. This cap applies regardless of the SetComputeUnitLimit instruction value \u2014 no transaction can request more than 1.4M CU. The default per-instruction budget is 200,000 CU, but multi-instruction transactions share the overall 1.4M cap. Transactions exceeding their CU limit fail with a ComputeBudgetExceeded error.",
    category: "programming-model",
    related: ["compute-units", "compute-budget", "compute-budget-program"]
  },
  {
    id: "syscall",
    ...
    category: "programming-model",
    aliases: ["System Call"],
    related: ["svm", "sbf", "compute-units"]
  },
  {
    id: "zero-copy-deserialization",
    term: "Zero-Copy Deserialization",
    ...
    category: "programming-model",
    related: ["account-data", "borsh", "account-loader"]
  },
  {
    id: "account-loader",
    term: "AccountLoader (Anchor)",
    ...
    category: "programming-model",
    related: ["zero-copy-deserialization", "anchor", "account-data"]
  },
  {
    id: "buffer-account",
    term: "Buffer Account",
    ...
    category: "programming-model",
    related: ["upgradeable-loader", "program-data", "elf-binary"]
  },
  {
    id: "nonce-account",
    term: "Nonce Account",
  ...
  },
  {
    ...
    category: "programming-model",
    related: ["blockhash", "durable-nonce", "transaction"]
  }
];

// data/terms/token-ecosystem.json
var token_ecosystem_default = [
  {
    id: "spl-token",
    term: "SPL Token Program",
    definition: "The original Solana Program Library token program (TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA) that implements fungible and non-fungible token operations. It manages mints (token definitions) and token accounts (balances). Core instructions include InitializeMint, MintTo, Transfer, Burn, Approve (delegation), and Revoke. All SPL tokens on mainnet before Token-2022 use this program.",
    category: "token-ecosystem",
    aliases: ["SPL Token"],
    related: ["mint", "token-account", "spl"]
  },
  {
    id: "spl",
    term: "Solana Program Library (SPL)",
    definition: "Solana Program Library\u2014a collection of on-chain programs and client libraries maintained by Solana Labs. Key SPL programs include Token, Token-2022, Associated Token Account, Memo, Name Service, Stake Pool, and Account Compression. SPL programs are deployed to well-known addresses and serve as the standard building blocks for Solana applications.",
    category: "token-ecosystem",
    aliases: ["SPL"],
    related: ["spl-token", "associated-token-account"]
  },
  {
    id: "token-2022",
    term: "Token-2022 (Token Extensions)",
    definition: "The next-generation token program (TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb) that extends SPL Token with configurable extensions. Extensions include transfer fees, confidential transfers, transfer hooks, permanent delegate, non-transferable tokens, interest-bearing tokens, metadata, and more. Token-2022 is backwards-compatible with SPL Token for basic operations.",
    category: "token-ecosystem",
    aliases: ["Token Extensions", "Token22"],
    related: ["spl-token", "transfer-hook", "confidential-transfers"]
  },
  {
    id: "mint",
    term: "Mint",
    ...
    related: ["spl-token", "token-account", "decimals"]
  },
  {
    id: "token-account",
    term: "Token Account",
    definition: "An account that holds a balance of a specific token for a specific owner. Token accounts store: mint (which token), owner (who controls it), amount (balance), delegate (optional), and state (initialized/frozen). Each wallet needs a separate token account per token type. Token accounts require ~165 bytes and rent-exempt minimum of ~0.002 SOL.",
    category: "token-ecosystem",
    related: ["mint", "associated-token-account", "spl-token"]
  },
  {
    id: "associated-token-account",
    term: "Associated Token Account (ATA)",
    definition: "A deterministically derived token account for a given wallet and mint, using the PDA seeds [wallet, token_program, mint]. ATAs provide a canonical address so anyone can send tokens to a wallet without the recipient creating an account first. The ATA program creates the account on first use, funded by the sender.",
    category: "token-ecosystem",
    aliases: ["ATA"],
    related: ["token-account", "pda"]
  },
  {
    id: "mint-authority",
    term: "Mint Authority",
    definition: "The pubkey authorized to mint new tokens via the MintTo instruction. Set at mint creation and can be changed or revoked (set to None) by the current authority. Revoking mint authority makes the supply fixed\u2014critical for trust in fungible tokens. For NFTs, mint authority is typically revoked after minting the single token.",
    category: "token-ecosystem",
    related: ["mint", "authority"]
  },
  {
    id: "freeze-authority",
    term: "Freeze Authority",
    definition: "The pubkey authorized to freeze and thaw token accounts, preventing transfers. Set optionally at mint creation. When a token account is frozen, no transfers, burns, or closes can occur until thawed. Used for regulatory compliance (e.g., USDC) and security (freezing compromised accounts). Can be revoked by setting to None.",
    category: "token-ecosystem",
    related: ["mint", "authority", "token-2022"]
  },
  {
    id: "decimals",
    term: "Decimals",
  ...
  },
  {
    id: "transfer-hook",
    term: "Transfer Hook",
    definition: "A Token-2022 extension that invokes a custom program on every token transfer. The hook program receives the transfer details and can enforce additional logic (royalties, compliance checks, transfer restrictions). The hook program address is set on the mint. Hooks execute via CPI and must conform to the Transfer Hook Interface.",
    category: "token-ecosystem",
    related: ["token-2022", "cpi"]
  },
  {
    id: "confidential-transfers",
    term: "Confidential Transfers",
    definition: "A Token-2022 extension that uses zero-knowledge proofs (Twisted ElGamal encryption over Ristretto255) to hide transfer amounts while keeping the token mint and accounts public. Balances are stored in encrypted form with separate 'pending' and 'available' pools. The account owner can decrypt their balance but others cannot see amounts.",
    category: "token-ecosystem",
    aliases: ["CT"],
    related: ["token-2022", "zk-proofs"]
  },
  {
    id: "transfer-fee",
    term: "Transfer Fee Extension",
    definition: "A Token-2022 extension that automatically withholds a percentage of every transfer as a fee. The fee (basis points + max fee) is configured on the mint by the transfer fee authority. Withheld fees accumulate in recipient token accounts and can be harvested by the withdraw authority. Enables on-chain royalties for fungible tokens.",
    category: "token-ecosystem",
    related: ["token-2022", "mint"]
  },
  {
    id: "permanent-delegate",
    term: "Permanent Delegate",
    definition: "A Token-2022 extension that designates an authority who can transfer or burn tokens from any token account of that mint, regardless of the account owner. Use cases include regulated stablecoins (clawback), gaming items (reclaiming), and legal compliance. Set at mint initialization and cannot be changed.",
    category: "token-ecosystem",
    related: ["token-2022", "authority"]
  },
  {
    id: "non-transferable",
    term: "Non-Transferable Token",
    definition: "A Token-2022 extension that prevents tokens from being transferred between accounts, making them soulbound. Non-transferable tokens can only be minted to and burned from accounts. Use cases include achievement badges, identity credentials, governance participation tokens, and certifications.",
    category: "token-ecosystem",
    aliases: ["Soulbound"],
    related: ["token-2022"]
  },
  {
    ...
    definition: "A Token-2022 extension that applies a continuously compounding interest rate to the display amount of a token without changing the actual on-chain balance. The UI amount is calculated as: amount \xD7 e^(rate \xD7 elapsed_time). Useful for rebasing tokens and lending receipt tokens.",
    category: "token-ecosystem",
    related: ["token-2022"]
  },
  {
    id: "default-account-state",
    term: "Default Account State",
    definition: "A Token-2022 extension that sets the initial state of all new token accounts for a mint to either Initialized or Frozen. When set to Frozen, new token accounts are frozen by default and must be explicitly thawed by the freeze authority before they can receive or send tokens. Enables KYC/AML gating.",
    category: "token-ecosystem",
    related: ["token-2022", "freeze-authority"]
  },
  {
    id: "memo-required",
    term: "Required Memo on Transfer",
    definition: "A Token-2022 extension that requires all incoming transfers to include a memo instruction (via the Memo Program) in the same transaction. If no memo is present, the transfer fails. Used for compliance-driven tokens where transaction metadata must be recorded on-chain.",
    category: "token-ecosystem",
    related: ["token-2022", "memo-program"]
  },
  {
    id: "metadata-extension",
    term: "Token Metadata Extension",
    definition: "A Token-2022 extension that stores token metadata (name, symbol, URI, and custom key-value pairs) directly on the mint account instead of requiring a separate Metaplex metadata account. Reduces account creation costs and simplifies token setup. The update authority can modify metadata fields after creation.",
    category: "token-ecosystem",
    related: ["token-2022", "metaplex-metadata"]
  },
  {
    id: "group-pointer",
    term: "Group / Member Pointer",
    definition: "A Token-2022 extension that enables token grouping by pointing a mint to a group account or member account. The Group Pointer marks a mint as a group, and Member Pointer marks a mint as belonging to a group. Useful for NFT collections and hierarchical token relationships without requiring Metaplex.",
    category: "token-ecosystem",
    related: ["token-2022"]
  },
  {
    id: "cpi-guard",
    term: "CPI Guard",
    definition: "A Token-2022 extension that protects token accounts from certain actions during CPI. When enabled, it blocks approve, close, and transfer operations via CPI while still allowing them in top-level instructions. This prevents malicious programs from draining or closing token accounts during composable calls.",
    category: "token-ecosystem",
    related: ["token-2022", "cpi"]
  },
  {
    id: "metaplex-metadata",
    term: "Metaplex Token Metadata",
    definition: "The Metaplex Token Metadata program (metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s) that attaches rich metadata to any SPL token or NFT. It creates a PDA-derived metadata account storing: name, symbol, URI (pointing to off-chain JSON), creators array, seller fee basis points, and collection info. The de facto standard for NFT metadata on Solana.",
    category: "token-ecosystem",
    aliases: ["mpl-token-metadata"],
    related: ["nft", "mint"]
  },
  {
    id: "nft",
    term: "NFT (Non-Fungible Token)",
    definition: "A non-fungible token on Solana\u2014an SPL token with 0 decimals, supply of 1, and revoked mint authority. The NFT's metadata (image, attributes) is stored off-chain (Arweave/IPFS) linked via the Metaplex metadata account's URI field. NFTs can be standard, compressed (cNFT), programmable (pNFT), or Core assets, each with different trade-offs.",
    category: "token-ecosystem",
    aliases: ["NFT"],
    related: [
      "mint",
  ...
  },
  {
    ...
    category: "token-ecosystem",
    aliases: ["cNFT"],
    related: ["nft", "state-compression", "bubblegum"]
  },
  {
    id: "bubblegum",
    term: "Bubblegum",
    definition: "The Metaplex program for creating and managing compressed NFTs. Bubblegum wraps the SPL Account Compression program, providing NFT-specific instructions: mint_to_collection, transfer, burn, delegate, redeem, and decompress (convert to standard NFT). It stores creator arrays and collection info in the Merkle tree leaf schema.",
    category: "token-ecosystem",
    related: ["compressed-nft", "concurrent-merkle-tree"]
  },
  {
    id: "core-nft",
    term: "Metaplex Core NFT",
    definition: "Metaplex's latest NFT standard (mpl-core) that uses a single account per asset instead of multiple accounts (mint + metadata + master edition + token account). Core NFTs are cheaper to mint (~0.003 SOL), have built-in plugin support (royalties, freeze, attributes, app data), and native collection-level operations. Designed to replace the legacy Token Metadata standard.",
    category: "token-ecosystem",
    aliases: ["Core Asset", "mpl-core"],
    related: ["nft", "metaplex-metadata"]
  },
  {
    id: "token-metadata-standard",
    term: "Token Metadata Standard",
    definition: "The convention for structuring NFT metadata established by Metaplex. On-chain: name (32 chars), symbol (10 chars), URI, creators, seller_fee_basis_points, collection. Off-chain JSON (at the URI): name, description, image, animation_url, external_url, attributes array [{trait_type, value}], and properties (files, category).",
    category: "token-ecosystem",
    related: ["metaplex-metadata", "nft", "off-chain-metadata"]
  },
  {
    id: "off-chain-metadata",
    term: "Off-Chain Metadata (JSON)",
  ...
  {
    id: "wrapped-sol",
    term: "Wrapped SOL (wSOL)",
    definition: "SOL deposited into an SPL Token account to be used as an SPL token (mint: So11111111111111111111111111111111111111112). wSOL enables SOL to interact with programs that only accept SPL tokens (DEXs, lending protocols). Creating a wSOL account requires the SyncNative instruction to sync lamport balance to the token amount. Close the account to unwrap back to SOL.",
    category: "token-ecosystem",
    aliases: ["wSOL"],
    related: ["sol", "spl-token", "token-account"]
  },
  {
    id: "token-swap",
    term: "SPL Token Swap",
    definition: "An SPL program that implements a constant-product AMM (x*y=k) for swapping between two SPL tokens. It was one of the first DeFi primitives on Solana but has been largely superseded by more sophisticated DEXs like Raydium, Orca, and Meteora. The program manages liquidity pools and issues LP tokens to liquidity providers.",
    category: "token-ecosystem",
    related: ["amm", "spl"]
  },
  {
    ...
    category: "token-ecosystem",
    related: ["spl", "transaction"]
  },
  {
    id: "name-service",
    term: "Name Service (SNS)",
    definition: "The Solana Name Service (SNS) by Bonfida that maps human-readable .sol domain names to Solana addresses. Domains are NFTs that resolve to wallet addresses, enabling users to send tokens to 'alice.sol' instead of a pubkey. The registry supports subdomains, and domain records can store additional data (IPFS links, social handles).",
    category: "token-ecosystem",
    aliases: ["SNS", ".sol domains", "Bonfida Name Service"],
    related: ["spl"]
  },
  {
    id: "programmable-nft",
    term: "Programmable NFT (pNFT)",
    definition: "A Metaplex NFT type (pNFT) that enforces creator-defined rules on transfers using an on-chain authorization rules engine. pNFTs always require the token record (state PDA) and must go through the Metaplex program for transfers, enabling royalty enforcement. Transfer rules are defined in a RuleSet account that can allow/deny based on program IDs, collections, or custom logic.",
    category: "token-ecosystem",
    aliases: ["pNFT"],
    related: ["nft", "metaplex-metadata", "authorization-rules"]
  },
  {
    id: "authorization-rules",
    term: "Authorization Rules",
    definition: "A Metaplex program that defines and evaluates rule sets for programmable NFTs. Rules specify conditions for transfer, delegate, and other operations\u2014e.g., allow transfers only through specific marketplaces (by program ID), require payment of royalties, or restrict to certain collections. Rules are composable using AND/OR/NOT logic.",
    category: "token-ecosystem",
    related: ["programmable-nft", "metaplex-metadata"]
  },
  {
    id: "token-group",
    term: "Token Group Extension",
    definition: "A Token-2022 extension that natively represents collections of tokens without requiring Metaplex. A mint with the Token Group extension acts as a collection, storing max_size and current_size. Mints with Token Group Member point to their group mint. This provides lightweight, protocol-level NFT collections directly in the token program.",
    category: "token-ecosystem",
    related: ["token-2022", "group-pointer"]
  },
  {
    id: "candy-machine",
    term: "Candy Machine",
    definition: "Metaplex's on-chain program for creating and managing NFT minting launches on Solana. Candy Machine handles sequential or random minting from a pre-loaded collection, enforcing supply limits and mint pricing. It supports multiple payment tokens (SOL, SPL tokens) and integrates with Candy Guard for access control. Candy Machine v3 is the current version, widely used for PFP and generative art drops.",
    category: "token-ecosystem",
    aliases: ["CM", "Candy Machine v3"],
    related: ["candy-guard", "nft", "metaplex-metadata"]
  },
  {
    id: "candy-guard",
    term: "Candy Guard",
    definition: "A Metaplex program that wraps Candy Machine with configurable access control guards for NFT minting. Guards include allowlist (Merkle proof), mint limit (per wallet), start/end date, sol payment, token payment, NFT gate, freeze (lock minted NFTs), and more. Multiple guards can be combined and organized into groups, allowing different mint phases (e.g., allowlist phase, then public mint) within a single launch.",
    category: "token-ecosystem",
    related: ["candy-machine", "nft"]
  },
  {
    id: "umi-framework",
    term: "Umi",
    definition: "Metaplex's modular TypeScript framework for building Solana client-side applications. Umi provides a unified interface for signing, sending transactions, and interacting with Metaplex programs (Token Metadata, Bubblegum, Candy Machine, Core). It uses a plugin architecture for wallets, RPC providers, and serialization, making it more flexible than direct web3.js usage for NFT and token operations.",
    category: "token-ecosystem",
    aliases: ["Metaplex Umi"],
    related: ["metaplex-metadata", "candy-machine", "bubblegum"]
  },
  {
    id: "stake-pool-spl",
    term: "SPL Stake Pool",
    definition: "An SPL program that enables the creation of liquid staking pools on Solana. Operators create a pool, users deposit SOL and receive pool tokens (LSTs) representing their share. The program manages stake delegation across multiple validators, epoch-based reward distribution, and pool token pricing. Marinade, BlazeStake, and other liquid staking protocols are built on or inspired by the SPL Stake Pool program.",
    category: "token-ecosystem",
    aliases: ["Stake Pool Program"],
    related: ["spl", "liquid-staking", "stake-account"]
  },
  {
    id: "verified-creator",
    term: "Verified Creator",
    definition: "A flag in Metaplex NFT metadata indicating that a listed creator has cryptographically signed the metadata, confirming their association with the NFT. Each creator entry includes an address, share (royalty split percentage), and verified boolean. Unverified creators can be listed by anyone, but only the creator themselves can set verified to true via the Verify instruction. Marketplaces use verification to prevent impersonation.",
    category: "token-ecosystem",
    related: ["metaplex-metadata", "nft", "collection"]
  },
  {
    id: "seller-fee-basis-points",
    term: "Seller Fee Basis Points",
  ...
  },
  {
    id: "nft-staking",
    term: "NFT Staking",
    definition: "The process of locking NFTs in a program to earn rewards, typically in the form of a project's token. Staking programs verify NFT ownership and collection membership, track staking duration, and distribute rewards proportionally. On Solana, NFT staking is commonly implemented using Metaplex's freeze delegate or by transferring the NFT to a program-controlled escrow account. Rewards incentivize holding over flipping.",
    category: "token-ecosystem",
    related: ["nft", "collection"]
  },
  {
    id: "floor-price",
    term: "Floor Price",
    definition: "The lowest listed sale price for any NFT within a collection on marketplaces. Floor price serves as the baseline valuation for a collection and is the most commonly tracked metric for NFT market health. It fluctuates based on demand, new listings, and broader market conditions. Rarer traits within a collection typically trade at multiples above the floor. Tensor and Magic Eden display real-time floor prices for Solana collections.",
    category: "token-ecosystem",
    related: ["nft", "collection", "tensor"]
  },
  {
    id: "collection-bid",
    term: "Collection Bid",
    definition: "An open offer to purchase any NFT from a specific collection at a stated price, regardless of which specific token is sold. Collection bids provide instant liquidity to NFT holders who want to sell immediately without waiting for a buyer. Marketplaces like Tensor match collection bids with sellers. Bids can be funded with SOL or other tokens and are typically set at or near the floor price.",
    category: "token-ecosystem",
    related: ["nft", "collection", "floor-price", "tensor"]
  },
  {
    id: "dynamic-nft",
    term: "Dynamic NFT (dNFT)",
    definition: "An NFT whose metadata, image, or attributes can change based on on-chain events, oracle data, or program logic. Unlike static NFTs with fixed metadata, dynamic NFTs evolve over time\u2014a gaming character gaining experience, artwork changing with seasons, or stats updating from real-world data. On Solana, Metaplex Core's plugin system and Token-2022 metadata extension facilitate dynamic NFT implementations.",
    category: "token-ecosystem",
    aliases: ["dNFT"],
    related: ["nft", "core-nft", "metadata-extension"]
  },
  {
    id: "sft",
    term: "SFT (Semi-Fungible Token)",
    definition: "A token type that combines properties of fungible and non-fungible tokens\u2014multiple identical copies exist (like fungible tokens) but they represent a specific asset class (like NFTs). On Solana, SFTs are SPL tokens with 0 decimals but supply greater than 1, with Metaplex metadata attached. Use cases include event tickets, game items with limited editions, and redeemable vouchers.",
    category: "token-ecosystem",
    aliases: ["Semi-Fungible Token"],
    related: ["nft", "mint", "metaplex-metadata"]
  },
  {
    id: "minting",
    term: "Minting",
    definition: "The process of creating new tokens or NFTs on-chain by invoking the MintTo instruction on an SPL Token mint account. For fungible tokens, minting increases the supply by the specified amount. For NFTs, minting creates the single token and typically revokes mint authority afterward. Minting requires the mint authority's signature and a destination token account to receive the newly created tokens.",
    category: "token-ecosystem",
    related: ["mint", "mint-authority", "spl-token"]
  },
  {
    id: "inscription-solana",
    term: "Inscription (Solana)",
    definition: "Data inscribed directly into on-chain Solana account data, inspired by Bitcoin Ordinals. Solana inscriptions store images, text, or other content as raw bytes in account data rather than pointing to off-chain storage. While more expensive than off-chain metadata (requiring rent for storage), inscriptions are fully on-chain and immutable once the account's update authority is revoked. They emerged as an experimental standard in late 2023.",
    category: "token-ecosystem",
    related: ["nft", "account-data", "rent-exemption"]
  },
  {
    id: "token-gating",
    term: "Token Gating",
    definition: "Restricting access to content, features, communities, or experiences based on ownership of specific tokens or NFTs. A dApp or website verifies the user's wallet holds a qualifying token before granting access. On Solana, token gating is implemented by reading token accounts or checking NFT collection membership. Common uses include Discord server access, exclusive content, early feature access, and real-world event entry.",
    category: "token-ecosystem",
    related: ["nft", "collection", "token-account"]
  },
  {
    id: "securitize",
    term: "Securitize",
    definition: "SEC-registered transfer agent and broker-dealer providing infrastructure for digital securities: KYC/AML, transfer restriction enforcement, and secondary market compliance. Transfer agent for BlackRock's BUIDL fund. Enforces compliance through programmable transfer logic.",
    category: "token-ecosystem",
    related: [
      "transfer-agent",
      "security-token",
  ...
  },
  {
    id: "baxus",
    term: "BAXUS",
    definition: "Solana platform for tokenizing fine spirits (whisky, bourbon) and collectibles as digital assets with physical custody in regulated storage. Uses NFTs for individual bottles and fungible tokens for fractionalized casks. One of few Solana RWA protocols focused on alternative assets.",
    category: "token-ecosystem",
    related: [
      "rwa-tokenization",
      "asset-tokenization",
  ...
  },
  {
    ...
    category: "token-ecosystem",
    related: ["nft", "magic-eden", "floor-price"]
  },
  {
    id: "metadata-update-authority",
    term: "Metadata Update Authority",
    definition: "The account authorized to modify an NFT's or token's on-chain metadata, including name, symbol, URI, and attributes. In Metaplex's token metadata standard, this authority can update mutable metadata fields. Revoking or transferring update authority is a trust signal \u2014 immutable metadata means the creator cannot alter the NFT's properties post-mint.",
    category: "token-ecosystem",
    related: ["metaplex-metadata", "mint-authority", "nft"]
  },
  {
    id: "token-2022-extensions-list",
    term: "Token-2022 Extensions",
    definition: "The set of optional features available in Solana's Token-2022 program, each activated per-mint or per-account. Extensions include transfer fees, confidential transfers, transfer hooks, permanent delegate, non-transferable tokens, interest-bearing tokens, default account state, memo required, metadata pointer, group pointer, and CPI guard. Extensions compose \u2014 a single mint can enable multiple features simultaneously.",
    category: "token-ecosystem",
    aliases: ["Token Extensions List"],
    related: [
      "token-2022",
      "transfer-hook",
  ...
  },
  {
    ...
    definition: "A non-transferable token permanently bound to a specific wallet, representing identity, credentials, achievements, or reputation. On Solana, soulbound tokens can be primarily implemented using Token-2022's non-transferable extension or transfer hooks that reject transfers, which prevents any transfer after minting. Used for proof of attendance, skill certifications, and on-chain identity.",
    category: "token-ecosystem",
    aliases: ["SBT", "Soulbound NFT"],
    related: ["non-transferable", "token-2022", "nft"]
  }
];

// data/terms/defi.json
var defi_default = [
  {
    id: "amm",
    term: "AMM (Automated Market Maker)",
    definition: "A protocol that enables token swaps using algorithmic pricing against pooled liquidity instead of matching individual buyers and sellers. AMMs use mathematical formulas (typically constant product x*y=k) to determine prices based on the ratio of tokens in a liquidity pool. On Solana, major AMMs include Raydium, Orca, and Meteora.",
    category: "defi",
    aliases: ["AMM"],
    related: ["liquidity-pool", "swap", "constant-product"]
  },
  {
    id: "clmm",
    term: "CLMM (Concentrated Liquidity Market Maker)",
    ...
    related: ["amm", "tick", "liquidity-pool"]
  },
  {
    id: "liquidity-pool",
    term: "Liquidity Pool",
  ...
  },
  {
    ...
    category: "defi",
    aliases: ["DEX"],
    related: ["amm", "order-book", "swap"]
  },
  {
    id: "order-book",
    term: "Order Book (On-Chain)",
  ...
  },
  {
    ...
    category: "defi",
    aliases: ["Serum v4"],
    related: ["order-book", "dex"]
  },
  {
    id: "phoenix",
    term: "Phoenix",
    definition: "A high-performance on-chain order book DEX on Solana with a fully on-chain FIFO matching engine. Phoenix is designed for low-latency trading without cranks\u2014orders are matched atomically within the same transaction. It supports limit orders, market orders, and provides a maker rebate structure to incentivize liquidity.",
    category: "defi",
    related: ["order-book", "dex"]
  },
  {
    id: "raydium",
    term: "Raydium",
    definition: "One of the largest DEXs on Solana, offering both constant-product AMM pools and concentrated liquidity (CLMM). Raydium's standard AMM integrates with OpenBook order books for shared liquidity. It also provides AcceleRaytor (launchpad) and farming incentives. Key pools include SOL/USDC and major token pairs.",
    category: "defi",
    related: ["amm", "clmm", "dex"]
  },
  {
    id: "orca",
    term: "Orca",
    definition: "A leading Solana DEX focused on concentrated liquidity through its Whirlpool AMM. Orca emphasizes user experience and capital efficiency. Whirlpools allow LPs to set custom price ranges. Orca is a major liquidity source for Jupiter aggregation. It uses a tick-based CLMM similar to Uniswap v3 adapted for Solana's architecture.",
    category: "defi",
    related: ["clmm", "whirlpool", "dex"]
  },
  {
    id: "whirlpool",
    term: "Whirlpool",
    definition: "Orca's concentrated liquidity AMM program on Solana. Whirlpools divide the price space into discrete ticks (configurable spacing: 1, 8, 64, 128), and LPs provide liquidity within specific tick ranges. Active tick positions earn fees proportional to their share of in-range liquidity. The program is open-source and widely integrated by aggregators.",
    category: "defi",
    related: ["orca", "clmm"]
  },
  {
    ...
    category: "defi",
    aliases: ["Jup"],
    related: ["dex-aggregator", "swap", "limit-order"]
  },
  {
    id: "dex-aggregator",
    term: "DEX Aggregator",
    definition: "A protocol that splits and routes token swaps across multiple DEXs to find the best execution price. The aggregator's routing algorithm considers pool depths, fees, and multi-hop paths (e.g., SOL\u2192USDC\u2192TARGET). Jupiter is the primary aggregator on Solana; others include Prism and DFlow. Aggregators are critical for large trades to minimize price impact.",
    category: "defi",
    related: ["jupiter", "swap", "routing"]
  },
  {
    id: "routing",
    term: "Routing (Swap)",
    definition: "The process of finding the optimal path for a swap across multiple DEX pools and intermediate tokens. A router may split a single swap into multiple routes (e.g., 60% through Orca, 40% through Raydium) or use multi-hop paths (e.g., SOL\u2192USDC\u2192mSOL) to achieve better pricing. Jupiter's Metis algorithm computes routes in real-time.",
    category: "defi",
    related: ["dex-aggregator", "jupiter"]
  },
  {
    id: "slippage",
    term: "Slippage",
  ...
  },
  {
    ...
    category: "defi",
    related: ["oracle", "pyth", "switchboard"]
  },
  {
    id: "vrf",
  ...
  },
  {
    id: "borrowing",
    term: "Borrowing",
    definition: "The act of taking a loan against deposited collateral in a lending protocol. Borrowers pay a variable interest rate that increases with pool utilization. The borrowed amount must stay below the maximum loan-to-value (LTV) ratio or the position becomes eligible for liquidation. Flash loans allow uncollateralized borrowing within a single transaction.",
    category: "defi",
    related: ["lending", "collateral"]
  },
  {
    id: "collateral",
    term: "Collateral",
    definition: "Assets deposited into a lending protocol to secure a loan. Each asset has a collateral factor (e.g., SOL at 80% means $100 of SOL supports $80 in borrows). If the collateral value drops below the maintenance threshold, the position is liquidated. Volatile assets have lower collateral factors than stablecoins.",
    category: "defi",
    related: ["lending", "liquidation", "ltv"]
  },
  {
    id: "liquidation",
    term: "Liquidation",
    definition: "The process of repaying a borrower's debt by selling their collateral when their position's LTV exceeds the liquidation threshold. Liquidators (typically bots) repay a portion of the debt and receive the collateral at a discount (liquidation bonus, typically 5-10%). Liquidation keeps lending protocols solvent. On Solana, liquidation bots compete via Jito bundles.",
    category: "defi",
    related: ["lending", "collateral", "ltv"]
  },
  {
    id: "ltv",
    term: "LTV (Loan-to-Value)",
    definition: "Loan-to-Value ratio\u2014the percentage of collateral value that can be borrowed. Each lending protocol sets max LTV per asset (e.g., 75% for SOL means $100 SOL collateral = max $75 borrow). If debt/collateral exceeds the liquidation LTV (e.g., 85%), the position is liquidatable. Conservative LTV settings protect against rapid price drops.",
    category: "defi",
    aliases: ["LTV"],
  ...
  },
  {
    ...
    category: "defi",
    related: ["liquid-staking", "jito", "mev"]
  },
  {
    id: "limit-order",
    term: "Limit Order (On-Chain)",
    definition: "An on-chain order to buy or sell a token at a specific price or better, executed automatically when the market reaches the target price. Jupiter Limit Orders use a keeper network that monitors prices and executes orders. Unlike CEX limit orders, on-chain limits may have partial fills and rely on available DEX liquidity at the trigger price.",
    category: "defi",
    related: ["jupiter", "dex"]
  },
  {
    ...
    definition: "A strategy that automatically splits a large purchase into smaller periodic trades to reduce price impact and timing risk. Jupiter DCA allows users to set up recurring buys (e.g., buy $100 of SOL every day for 30 days). Orders execute via keeper bots. DCA reduces the risk of buying at a local peak.",
    category: "defi",
    aliases: ["DCA"],
    related: ["jupiter"]
  },
  ...
  },
  {
    ...
    category: "defi",
    related: ["perpetuals", "margin", "liquidation"]
  },
  {
    id: "margin",
    term: "Margin",
  ...
  },
  {
    ...
    category: "defi",
    aliases: ["Sanctum LST"],
    related: ["liquid-staking", "marinade", "jito-sol"]
  },
  {
    id: "tensor",
    term: "Tensor",
    definition: "A Solana NFT marketplace and trading protocol that launched the TNSR governance token in April 2024. Tensor provides advanced trading features including collection-level bidding, real-time price feeds, and lending against NFT collateral. It competes with Magic Eden as a primary NFT trading venue on Solana.",
    category: "defi",
    aliases: ["Tensor Trade", "TNSR"],
    related: ["nft", "compressed-nft"]
  },
  {
    id: "intents",
    term: "Intents",
    definition: "A transaction paradigm where users declare their desired outcome (e.g., 'swap 100 USDC for best SOL price') rather than specifying exact execution steps. Solvers compete to find the optimal execution path across protocols, chains, and liquidity sources. Intent-based architectures abstract away routing complexity, enable cross-chain execution, and shift MEV from adversarial extraction to competitive fulfillment.",
    category: "defi",
    aliases: ["Intent-Based Architecture"],
    related: ["chain-abstraction", "dex-aggregator"]
  },
  {
    ...
    category: "defi",
    aliases: ["Cross-Chain Abstraction"],
    related: ["intents", "interoperability", "bridge"]
  },
  {
    id: "eigenlayer",
    term: "EigenLayer",
  ...
  },
  {
    ...
    definition: "A tokenized receipt representing restaked assets in protocols like EigenLayer, allowing users to maintain DeFi liquidity while earning restaking yields. When users restake LSTs via protocols like EtherFi (eETH) or Renzo (ezETH), they receive LRTs that can be used as collateral, traded, or deployed in additional DeFi strategies. LRTs reached $14 billion combined market cap by 2025.",
    category: "defi",
    aliases: ["LRT", "Liquid Restaking"],
    related: ["restaking", "eigenlayer", "liquid-staking"]
  },
  {
    id: "rwa-tokenization",
    ...
    related: ["defi-general", "stablecoin"]
  },
  {
    id: "points-seasons",
    term: "Points / Seasons",
    definition: "A gamified incentive mechanism where protocols award off-chain loyalty points for specific on-chain actions across time-limited campaign periods called 'seasons.' Points are typically converted to token allocations at a later token generation event (TGE). The off-chain and opaque nature of points tracking has drawn criticism for lack of transparency and sybil vulnerability.",
    category: "defi",
    aliases: ["Points Program", "Points Farming"],
    related: ["airdrop-crypto", "sybil-resistance"]
  },
  {
    id: "mev-supply-chain",
  ...
  },
  {
    ...
    category: "defi",
    aliases: ["vAMM"],
    related: ["amm", "perpetuals", "drift"]
  },
  {
    id: "cpmm",
    term: "CPMM (Constant Product Market Maker)",
    definition: "The simplest and most widely used AMM model, based on the formula x * y = k where x and y are token reserves and k is an invariant. Every swap changes the ratio of reserves while keeping the product constant, producing a hyperbolic price curve. CPMMs provide liquidity across the full price range (0 to infinity) but are less capital-efficient than concentrated liquidity designs. Raydium's standard pools use this model.",
    category: "defi",
    aliases: ["CPMM"],
    related: ["constant-product", "amm", "liquidity-pool"]
  },
  {
    id: "utilization-rate",
    term: "Utilization Rate",
    definition: "The percentage of total deposited assets currently borrowed in a lending protocol, calculated as total borrows divided by total deposits. Utilization rate is the primary input to interest rate models: higher utilization drives up borrow rates to incentivize repayment and attract new deposits. Most Solana lending protocols (Kamino, MarginFi) target an optimal utilization of 70-90%, with rates spiking sharply above this threshold.",
    category: "defi",
    related: ["lending", "interest-rate-model", "borrow-apy"]
  },
  {
    id: "borrow-apy",
    term: "Borrow APY",
    definition: "The annualized interest rate paid by borrowers in a lending protocol, expressed as annual percentage yield with compounding. Borrow APY is determined algorithmically by the interest rate model based on the pool's utilization rate. On Solana lending protocols like Kamino and MarginFi, borrow APY typically ranges from 2-15% for major assets but can spike above 100% during high utilization periods.",
    category: "defi",
    aliases: ["Borrow Rate"],
    related: ["lending", "utilization-rate", "supply-apy"]
  },
  {
    id: "supply-apy",
    term: "Supply APY",
    ...
    related: ["lending", "utilization-rate", "borrow-apy"]
  },
  {
    id: "health-factor",
    term: "Health Factor",
    definition: "A ratio that measures the safety of a borrowing position in a lending protocol, calculated as the risk-adjusted collateral value divided by the total borrow value. A health factor above 1 means the position is safe; at or below 1, the position becomes eligible for liquidation. Users monitor health factor to avoid liquidation, and protocols display it prominently as the primary risk indicator.",
    category: "defi",
    related: ["lending", "liquidation", "collateral", "ltv"]
  },
  {
    id: "interest-rate-model",
    term: "Interest Rate Model",
    definition: "The algorithm a lending protocol uses to determine borrow and supply interest rates based on pool utilization. Most Solana lending protocols use a kinked (piecewise linear) model with a gentle slope below optimal utilization and a steep slope above it, creating urgency to repay when liquidity becomes scarce. Parameters include base rate, optimal utilization point, and slope values for each segment.",
    category: "defi",
    related: ["utilization-rate", "borrow-apy", "supply-apy", "lending"]
  },
  {
    id: "liquidation-bonus",
    term: "Liquidation Bonus",
    definition: "A discount on collateral that incentivizes liquidators to repay the debt of unhealthy positions in lending protocols. When a position's health factor drops below 1, liquidators can repay a portion of the debt and receive the equivalent collateral value plus a bonus (typically 5-15%). The bonus must be large enough to cover transaction costs and risk, but not so large that borrowers suffer excessive losses.",
    category: "defi",
    aliases: ["Liquidation Incentive", "Liquidation Penalty"],
    related: ["liquidation", "health-factor", "lending"]
  },
  {
    id: "bad-debt",
    term: "Bad Debt",
  ...
  },
  {
    ...
    category: "defi",
    related: ["oracle", "mark-price", "funding-rate", "pyth"]
  },
  {
    id: "fee-tier",
    term: "Fee Tier",
    definition: "A predefined fee level applied to a liquidity pool or trading pair, typically expressed in basis points. Different fee tiers exist for different asset volatility profiles: stable pairs (e.g., USDC/USDT) use low fees (1-5 bps), major pairs (SOL/USDC) use moderate fees (5-30 bps), and volatile pairs use higher fees (30-100 bps). LPs select fee tiers based on expected trading volume and impermanent loss risk.",
    category: "defi",
    related: ["liquidity-pool", "amm", "clmm"]
  },
  {
    id: "auto-compounding",
  ...
  },
  {
    ...
    category: "defi",
    related: ["cliff-vesting", "tge", "tokenomics"]
  },
  {
    id: "cliff-vesting",
    term: "Cliff Vesting",
    ...
    category: "defi",
    related: ["vesting", "tge", "tokenomics"]
  },
  {
    id: "tge",
    ...
    related: ["vesting", "airdrop-crypto", "launchpad"]
  },
  {
    id: "max-supply",
    term: "Max Supply",
    ...
    category: "defi",
    related: ["circulating-supply", "fdv", "tokenomics"]
  },
  {
    id: "circulating-supply",
    term: "Circulating Supply",
    definition: "The number of tokens currently available and tradeable in the open market, excluding locked, vesting, or otherwise restricted tokens. Circulating supply is used to calculate market capitalization (price times circulating supply). It increases as vesting schedules unlock tokens and decreases as tokens are burned. The gap between circulating and max supply indicates future dilution potential.",
    category: "defi",
  ...
  },
  {
    ...
    category: "defi",
    aliases: ["FDV"],
    related: ["market-cap", "max-supply", "circulating-supply"]
  },
  {
    id: "market-cap",
  ...
  },
  {
    id: "token-burn",
    term: "Token Burn",
    definition: "The permanent removal of tokens from circulation by sending them to a burn address or invoking the SPL Token Burn instruction, which reduces the mint's supply counter. Burns create deflationary pressure and are used for fee redistribution (burning a portion of transaction fees), buyback-and-burn mechanisms, and reducing supply post-TGE. On Solana, burned tokens are verifiable via the decreased supply on the mint account.",
    category: "defi",
    related: ["mint", "circulating-supply", "tokenomics"]
  },
  {
  ...
  },
  {
    ...
    category: "defi",
    related: ["launchpad", "bonding-curve", "tge"]
  },
  {
    id: "usdc-native",
    term: "USDC (Native Solana)",
    definition: "Circle-issued USDC minted directly on Solana through the SPL Token program, as opposed to bridged USDC from other chains. Native USDC (mint: EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v) is the most widely used stablecoin on Solana with deep DeFi integration. Circle maintains freeze authority over the mint for regulatory compliance and supports CCTP for native cross-chain transfers.",
    category: "defi",
    aliases: ["USDC"],
    related: ["stablecoin", "cctp", "spl-token"]
  },
  {
    id: "pyusd",
    term: "PYUSD (PayPal USD on Solana)",
    ...
    related: ["stablecoin", "spl-token"]
  },
  {
    id: "algorithmic-stablecoin",
    term: "Algorithmic Stablecoin",
    definition: "A stablecoin that maintains its peg through algorithmic mechanisms (mint/burn arbitrage, elastic supply) rather than being fully backed by reserves. Algorithmic stablecoins use smart contracts to expand or contract supply based on demand. The category carries significant risk after the UST/Luna collapse in 2022. On Solana, UXD Protocol attempted a delta-neutral algorithmic design.",
    category: "defi",
    related: ["stablecoin", "depeg"]
  },
  {
    id: "depeg",
    term: "Depeg",
    definition: "An event where a stablecoin's market price deviates significantly from its intended peg (typically $1.00). Depegs can result from loss of confidence in reserves, algorithmic failure, liquidity crises, or smart contract exploits. Even temporary depegs cause cascading liquidations across DeFi as collateral values drop. Notable depegs include UST (total collapse), USDC (briefly during SVB crisis), and various algorithmic stablecoins.",
    category: "defi",
    related: ["stablecoin", "algorithmic-stablecoin", "liquidation"]
  },
  {
    id: "cctp",
    term: "CCTP (Cross-Chain Transfer Protocol)",
    definition: "Circle's Cross-Chain Transfer Protocol that enables native USDC transfers between blockchains by burning USDC on the source chain and minting an equivalent amount on the destination chain. Unlike bridges that use wrapped tokens, CCTP transfers are native USDC on each chain, eliminating bridge risk and fragmented liquidity. CCTP supports Solana, Ethereum, and other major chains.",
    category: "defi",
    aliases: ["CCTP"],
    related: ["usdc-native", "bridge", "interoperability"]
  },
  {
    id: "pull-oracle-model",
    term: "Pull Oracle Model",
    definition: "An oracle design where price updates are fetched on-demand by the consuming protocol rather than pushed to on-chain accounts at fixed intervals. Pyth pioneered the pull model on Solana: prices are published to an off-chain service (Hermes) and only written on-chain when a transaction needs them. This reduces costs since unused price updates are never posted, but requires transactions to include price update instructions.",
    category: "defi",
    related: ["oracle", "pyth", "pyth-hermes"]
  },
  {
    id: "pyth-hermes",
    term: "Pyth Hermes",
    definition: "Pyth Network's off-chain price delivery web service that provides real-time price updates for the pull oracle model. Hermes aggregates publisher price submissions and serves them via REST and WebSocket APIs. When a Solana transaction needs a price, it fetches the latest update from Hermes and includes it as an instruction in the transaction, where the Pyth on-chain program verifies its authenticity.",
    category: "defi",
    related: ["pyth", "pull-oracle-model", "oracle"]
  },
  {
    id: "confidence-interval",
    term: "Confidence Interval (Oracle)",
    ...
    category: "defi",
    related: ["pyth", "price-feed", "oracle"]
  },
  {
    id: "ema-price",
    term: "EMA Price (Oracle)",
  ...
  },
  {
    ...
    definition: "The condition where an oracle price feed has not been updated within an acceptable time window, making the price data potentially unreliable. Stale prices can lead to incorrect liquidations, mispriced trades, or exploitable arbitrage. Solana DeFi protocols must check the timestamp of oracle updates and reject or pause operations when prices exceed a staleness threshold (e.g., 30 seconds for Pyth feeds).",
    category: "defi",
    related: ["oracle", "price-feed", "bad-debt"]
  },
  {
    id: "security-token",
    term: "Security Token",
  ...
  },
  {
    ...
    category: "defi",
    related: ["rwa-tokenization", "token-2022", "spv"]
  },
  {
    ...
    category: "defi",
    related: ["rwa-tokenization", "asset-tokenization", "spl-token"]
  },
  {
    ...
    category: "defi",
    related: ["stablecoin", "collateral", "rwa-tokenization", "usdc-native"]
  },
  {
    id: "tokenized-treasury",
    term: "Tokenized Treasury",
    definition: "Blockchain tokens representing ownership of US Treasury bills or money market funds invested in government debt. Fastest-growing RWA category ($5B+ by 2025). On Solana, Ondo Finance offers USDY backed by short-term US treasuries.",
    category: "defi",
    aliases: ["Tokenized T-Bills"],
    related: [
      "rwa-tokenization",
      "t-bill",
      "fixed-income",
  ...
  },
  {
    ...
    category: "defi",
    related: ["rwa-tokenization", "lending", "fixed-income", "credit-rating"]
  },
  {
    id: "nav",
    ...
    related: ["tokenized-treasury", "aum", "redemption-mechanism"]
  },
  {
    ...
    category: "defi",
    related: ["rwa-tokenization", "nav", "usdc-native", "asset-backed-token"]
  },
  {
    id: "spv",
    term: "SPV (Special Purpose Vehicle)",
    definition: "Legally separate entity (LLC/trust) created to hold assets, isolating risk from parent organization. In RWA tokenization, SPV holds the actual asset while investors hold tokens representing SPV shares. Provides bankruptcy remoteness.",
    category: "defi",
    aliases: ["Special Purpose Vehicle", "SPV"],
    related: ["rwa-tokenization", "asset-tokenization", "custodian"]
  },
  {
    id: "fixed-income",
    term: "Fixed Income",
    ...
    category: "defi",
    related: ["t-bill", "tokenized-treasury", "coupon-rate", "maturity-date"]
  },
  {
    id: "t-bill",
    term: "T-Bill (US Treasury Bill)",
    definition: "Short-term US government debt (4-52 week maturity) sold at discount, redeemed at par. Considered safest short-term investment. Most popular RWA category on-chain \u2014 Ondo's USDY and BlackRock's BUIDL hold T-bills.",
    category: "defi",
    aliases: ["Treasury Bill", "T-Bill"],
    related: ["tokenized-treasury", "fixed-income", "money-market-fund"]
  },
  {
    id: "money-market-fund",
    term: "Money Market Fund",
    definition: "Highly liquid fund holding short-term, high-quality debt (T-bills, commercial paper, repos). Targets stable $1/share NAV. BlackRock's BUIDL ($500M+ AUM) tokenizes money market fund shares as blockchain tokens.",
    category: "defi",
    aliases: ["MMF"],
    related: ["t-bill", "nav", "aum", "tokenized-treasury"]
  },
  {
    id: "basis-points",
    term: "Basis Points",
    definition: "Unit of measurement equal to 0.01% (one hundredth of a percentage point). 100 bps = 1%. Used across DeFi for swap fees, protocol fees, and yield spreads. Token-2022's transfer fee extension configures fees in basis points.",
    category: "defi",
    aliases: ["bps"],
    related: ["transfer-fee", "apy", "apr", "seller-fee-basis-points"]
  },
  {
    id: "aum",
    term: "AUM (Assets Under Management)",
    definition: "Total market value of assets managed by an institution or protocol. In DeFi, often synonymous with TVL though AUM more precisely refers to managed funds. BlackRock's BUIDL: $500M+ AUM; Ondo Finance: $600M+.",
    category: "defi",
    aliases: ["AUM", "Assets Under Management"],
    related: ["tvl", "nav", "money-market-fund"]
  },
  {
    id: "yield-curve",
    term: "Yield Curve",
    ...
    category: "defi",
    related: ["fixed-income", "t-bill", "apy"]
  },
  {
    id: "coupon-rate",
    term: "Coupon Rate",
    ...
    category: "defi",
    related: ["fixed-income", "t-bill", "basis-points"]
  },
  {
    id: "maturity-date",
    term: "Maturity Date",
    definition: "Date when a debt instrument expires and principal must be repaid. Ranges from overnight (repos) to 30 years (T-bonds). Short-maturity instruments dominate Solana RWA due to lower duration risk. On-chain protocols encode maturity in smart contract logic.",
    category: "defi",
    related: ["fixed-income", "bond-duration", "t-bill"]
  },
  {
    id: "bond-duration",
    term: "Bond Duration",
  ...
  },
  {
    id: "credit-rating",
    term: "Credit Rating",
    definition: "Assessment of creditworthiness by agencies (Moody's, S&P, Fitch). Ranges from AAA (near risk-free) to junk (BB+ and below). US Treasuries are AAA-rated. Private credit DeFi protocols must evaluate risk without formal ratings, a key risk factor.",
    category: "defi",
    related: ["fixed-income", "private-credit", "t-bill"]
  },
  {
    id: "accredited-investor",
    term: "Accredited Investor",
    definition: "SEC regulatory category: individuals with $1M+ net worth or $200K+ annual income permitted to participate in private securities offerings. Most tokenized security offerings on Solana restrict participation to accredited investors, requiring KYC and wallet whitelisting.",
    category: "defi",
    related: ["kyc", "security-token", "reg-d", "compliance-whitelist"]
  },
  {
    id: "reg-d",
    term: "Regulation D (Reg D)",
    definition: "SEC safe harbor exempting private securities offerings from full registration. Rule 506(c) allows unlimited accredited investors with verification. Commonly used for Solana security token issuances with on-chain transfer restrictions via Token-2022.",
    category: "defi",
    aliases: ["Regulation D"],
    related: [
      "security-token",
      "accredited-investor",
      "reg-s",
  ...
  {
    id: "reg-s",
    term: "Regulation S (Reg S)",
    definition: "SEC safe harbor exempting securities offered entirely outside the US to non-US persons. Ondo's USDY available globally under Reg S with on-chain transfer restrictions preventing US transfers. Combined with Reg D to cover domestic + international.",
    category: "defi",
    aliases: ["Regulation S"],
    related: ["security-token", "reg-d", "transfer-restrictions"]
  },
  {
  ...
  {
    id: "compliance-whitelist",
    term: "Compliance Whitelist",
  ...
  {
    id: "compliance-blacklist",
    term: "Compliance Blacklist",
    definition: "List of wallet addresses prohibited from holding or transacting a token due to OFAC sanctions, suspicious activity, or court orders. USDC (Circle) maintains a blacklist using SPL Token's freeze_authority. Token-2022's permanent delegate enables recovery from blacklisted accounts.",
    category: "defi",
    related: [
      "ofac-compliance",
      "freeze-authority",
  ...
  {
    id: "ofac-compliance",
    term: "OFAC Compliance",
    definition: "Compliance with US Treasury OFAC sanctions prohibiting transactions with designated individuals/entities. Circle blacklists sanctioned USDC addresses on-chain. Solana RWA protocols must screen wallets against the SDN list before whitelisting.",
    category: "defi",
    aliases: ["OFAC"],
    related: ["compliance-blacklist", "compliance-whitelist", "kyc"]
  },
  {
    id: "transfer-agent",
    term: "Transfer Agent",
    definition: "Firm maintaining official ownership ledger for securities, processing transfers and ensuring compliance. Securitize acts as transfer agent for BlackRock's BUIDL. Token-2022's transfer hook maps directly to programmable transfer agent logic on-chain.",
    category: "defi",
    related: [
      "security-token",
      "transfer-restrictions",
  ...
  },
  {
    id: "custodian",
    term: "Custodian",
    definition: "Institution holding and safeguarding assets on behalf of clients. In RWA tokenization, custodian holds off-chain assets (bonds, gold) while on-chain tokens represent claims. Qualified custodians include Anchorage Digital, BNY Mellon (for BUIDL).",
    category: "defi",
    aliases: ["Qualified Custodian"],
    related: ["rwa-tokenization", "spv", "asset-backed-token"]
  },
  {
    ...
    category: "defi",
    related: ["finality-general", "dvp", "clearing-settlement"]
  },
  {
    id: "t-plus-settlement",
    term: "T+Settlement",
    ...
    category: "defi",
    aliases: ["T+1", "T+2", "T+0"],
    related: ["atomic-settlement", "clearing-settlement", "finality-general"]
  }
];

// data/terms/zk-compression.json
var zk_compression_default = [
  {
  ...
  },
  {
    ...
    category: "zk-compression",
    related: ["pda", "compressed-account", "zk-compression"]
  },
  {
    id: "bls12-381-syscall",
    term: "BLS12-381 Syscall",
    ...
    category: "zk-compression",
    related: ["zkp-syscall", "zk-proofs", "syscall"]
  },
  {
    id: "alt-bn128-syscall",
    term: "alt_bn128 Syscall",
    definition: "A native Solana runtime syscall providing elliptic curve operations on the alt_bn128 (BN254) curve \u2014 specifically point addition, scalar multiplication, and pairing checks. These operations are essential for on-chain Groth16 zk-SNARK proof verification, which would otherwise exceed the 1.4M compute unit limit if implemented in pure BPF bytecode. Light Protocol's compressed account verification and Token-2022's confidential transfers both depend on the alt_bn128 syscall for ZK proof verification within practical compute budgets.",
    category: "zk-compression",
    aliases: ["BN254 Syscall"],
    related: ["zkp-syscall", "groth16", "zk-proofs"]
  },
  {
    id: "zk-token-proof-program",
    term: "ZK Token Proof Program",
    ...
    category: "zk-compression",
    related: ["elgamal-encryption", "zk-proofs", "token-2022"]
  },
  {
    id: "canopy-depth",
    term: "Canopy Depth",
...
var infrastructure_default = [
  ...
  },
  {
    ...
    category: "infrastructure",
    related: ["yellowstone", "geyser"]
  },
  {
    id: "das",
    term: "DAS (Digital Asset Standard) API",
    definition: "A unified RPC API specification, originated by Metaplex and adopted by major RPC providers, that provides indexed, queryable access to Solana digital assets including regular NFTs, compressed NFTs (cNFTs), and fungible tokens through methods like getAsset, getAssetsByOwner, getAssetsByGroup, and searchAssets. DAS abstracts away the complexity of reading raw Metaplex account data and reconstructing compressed NFT state from concurrent Merkle trees, making it the standard NFT data layer for wallets and marketplaces.",
    category: "infrastructure",
    aliases: ["DAS", "Digital Asset API"],
    related: ["compressed-nft", "indexer"]
  },
  {
    id: "indexer",
    term: "Indexer",
    definition: "A specialized off-chain service that continuously ingests on-chain data (typically via a Geyser plugin or RPC polling) and stores it in a queryable database optimized for complex lookups, aggregations, and historical queries that the RPC node's account-state store cannot efficiently serve. Indexers power DAS APIs, portfolio trackers, analytics dashboards, and DEX order-history features by transforming Solana's append-only ledger into relational or document-oriented schemas with fast secondary indexes.",
    category: "infrastructure",
    related: ["geyser", "das", "rpc"]
  },
  {
    id: "helius",
    term: "Helius",
    definition: "A Solana-focused infrastructure provider offering dedicated RPC nodes, DAS API access, Geyser-based webhooks, the LaserStream ultra-low-latency streaming product, and enhanced transaction parsing APIs that decode raw instruction data into human-readable event objects. Helius is widely used by developers for its generous free tier, Solana-specific tooling (NFT APIs, token metadata enrichment), and reliable globally distributed RPC endpoints.",
    category: "infrastructure",
    related: ["rpc-node", "das", "webhook"]
  },
  {
    id: "quicknode",
    term: "QuickNode",
    definition: "A multi-chain blockchain infrastructure provider that offers globally distributed Solana RPC and WebSocket endpoints, add-on marketplace integrations (including DAS, token APIs, and priority fee estimation), and performance-oriented features like endpoint analytics and dedicated node plans. QuickNode provides Solana developers with low-latency access to mainnet-beta, testnet, and devnet through a SLA-backed managed service model.",
    category: "infrastructure",
    related: ["rpc-node"]
  },
  {
  ...
  },
  {
    ...
    category: "infrastructure",
    related: ["program-automation", "cron"]
  },
  {
    id: "priority-fee-api",
    term: "Priority Fee API",
    ...
    related: ["priority-fee", "compute-units", "helius"]
  },
  {
    id: "program-automation",
    term: "Program Automation",
    definition: "The pattern of scheduling and triggering on-chain program instructions based on time intervals, cron expressions, or conditional state changes without manual intervention. On Solana, automation was pioneered by Clockwork (now sunset) using on-chain Thread accounts and has evolved toward off-chain keeper bot architectures where backend services monitor chain state and submit transactions when trigger conditions are met.",
    category: "infrastructure",
    related: ["clockwork", "cron"]
  },
  {
    id: "enhanced-transactions",
    term: "Enhanced Transactions (Helius)",
    definition: "A Helius-specific API that returns enriched, human-readable transaction data by parsing raw Solana transaction instructions into structured event objects with descriptive labels, token transfer summaries, and program interaction breakdowns. Instead of manually decoding instruction data and cross-referencing IDLs, developers get pre-parsed fields like transaction type, token amounts, source/destination wallets, and fee details. Available via the Helius parseTransaction and enhanced transaction history endpoints.",
    category: "infrastructure",
    related: ["helius", "get-transaction", "indexer"]
  },
  {
    id: "get-multiple-accounts",
    term: "getMultipleAccounts",
    definition: "A Solana RPC method that fetches the account data for up to 100 public keys in a single request, returning an array of AccountInfo objects in the same order as the input keys. This batched approach is significantly more efficient than calling getAccountInfo repeatedly, reducing round trips and RPC load. It supports the same encoding and commitment parameters as getAccountInfo. Accounts that do not exist return null in the response array.",
    category: "infrastructure",
    related: ["rpc-methods", "get-account-info", "account"]
  },
  {
    id: "get-latest-blockhash",
    term: "getLatestBlockhash",
    definition: "A Solana RPC method that returns the most recent blockhash along with the last valid block height for that blockhash, providing the two values needed to build and submit a transaction. Every Solana transaction must include a recent blockhash to prevent replay attacks; this method is typically called immediately before signing. The returned lastValidBlockHeight indicates when the blockhash expires, after which unsigned transactions using it will be rejected.",
    category: "infrastructure",
    related: ["rpc-methods", "blockhash", "transaction"]
  },
  {
    id: "send-transaction-rpc",
    term: "sendTransaction (RPC)",
    definition: "The Solana RPC method that submits a fully signed, serialized transaction to the cluster for processing. The transaction is passed as a base64-encoded string. By default, the RPC node performs preflight checks (simulation) before forwarding to the leader; this can be disabled with skipPreflight for lower latency at the risk of submitting invalid transactions. The method returns the transaction signature immediately without waiting for confirmation. Clients must separately poll or subscribe for confirmation status.",
    category: "infrastructure",
    related: ["rpc-methods", "transaction", "transaction-landing"]
  },
  {
    id: "signature-subscribe",
    term: "signatureSubscribe",
    ...
    category: "infrastructure",
    related: ["websocket", "signature", "commitment-levels"]
  },
  {
    id: "rpc-encoding",
    term: "RPC Encoding",
    ...
    category: "infrastructure",
    related: ["rpc-methods", "account-data", "serialization"]
  },
  {
    id: "staked-connection",
    term: "Staked Connection",
    ...
    category: "infrastructure",
    related: ["stake-weighted-qos", "rpc-node", "transaction-landing"]
  }
];

// data/terms/security.json
var security_default = [
  {
    id: "missing-signer-check",
    term: "Missing Signer Check",
  ...
  {
    id: "closing-accounts",
    term: "Closing Accounts Vulnerability",
  ...
  {
    id: "sysvar-spoofing",
    term: "Sysvar Spoofing",
    definition: "A vulnerability where a program retrieves a sysvar (such as Clock, Rent, or SlotHashes) by deserializing an account passed in the instruction's account list rather than using the runtime's native sysvar access API, allowing an attacker to substitute a fake account at the well-known sysvar address with crafted data \u2014 for example, a manipulated clock timestamp to bypass time locks. The safe pattern in modern Solana programs is to use Clock::get(), Rent::get(), and equivalent intrinsics that read from the runtime directly without trusting any account; Anchor's Sysvar<'info, Clock> account type validates the address but native programs should prefer the get() API.",
    category: "security",
    related: ["sysvar", "clock"]
  },
  {
    id: "remaining-accounts",
    term: "Remaining Accounts Misuse",
  ...
  },
  {
    ...
    category: "security",
    related: ["upgrade-authority", "upgradeable-loader", "verifiable-build"]
  },
  {
    id: "time-lock-security",
    term: "Time-Lock",
    ...
    category: "security",
    related: ["squads", "multisig", "upgrade-authority"]
  },
  {
    id: "squads-v4",
    term: "Squads v4",
    ...
    category: "security",
    related: ["squads", "multisig", "time-lock-security"]
  },
  {
    ...
    category: "security",
    related: ["invariant-testing", "fuzzing", "testing"]
  },
  {
    id: "stateful-fuzzing",
    term: "Stateful Fuzzing",
    ...
    category: "security",
    related: ["fuzzing", "trident", "invariant-testing"]
  },
  {
    id: "reentrancy-guard",
    term: "Reentrancy Guard",
    definition: "A protective pattern that prevents a program from being invoked recursively through cross-program invocations, mitigating reentrancy attacks. While the Solana runtime inherently prevents direct same-program reentrancy (a program cannot CPI into itself), indirect reentrancy through shared mutable accounts via third-party programs is still possible. Reentrancy guards typically use a flag in account state that is set at instruction entry and cleared at exit, causing reentrant calls to fail. Less critical on Solana than on EVM chains due to the runtime's built-in protection.",
    category: "security",
    related: ["reentrancy", "cpi", "account-reloading"]
  },
  {
    id: "audit-severity",
    term: "Audit Severity Levels",
  ...
  },
  {
    id: "ottersec",
    term: "OtterSec",
    definition: "Leading Solana security audit firm that has audited major protocols including Jupiter, Marinade, Jito, and the Solana runtime. Known for deep expertise in Rust/Solana program security and responsible vulnerability disclosure. One of the most trusted names in Solana security.",
    category: "security",
    related: ["audit", "fuzzing"]
  },
  {
    id: "neodyme",
    term: "Neodyme",
    definition: "Security research firm specializing in Solana program auditing and vulnerability research. Published influential work on common Solana vulnerabilities and maintains educational resources on program security patterns. Based in Germany.",
    category: "security",
    related: ["audit", "fuzzing"]
  },
  {
    id: "sec3",
    term: "Sec3 (formerly Soteria)",
    ...
    category: "security",
    aliases: ["Soteria"],
    related: ["audit", "fuzzing", "missing-signer-check"]
  },
  {
    id: "halborn",
    term: "Halborn",
    definition: "Blockchain security company providing smart contract auditing, penetration testing, and security advisory services across multiple chains including Solana. Known for auditing both DeFi protocols and infrastructure-level code.",
    category: "security",
    related: ["audit", "fuzzing"]
  },
  {
    id: "instruction-ordering-attack",
    term: "Instruction Ordering Attack",
  ...
  },
  {
    ...
    category: "security",
    related: ["account-reloading", "cpi", "integer-overflow"]
  },
  {
    id: "rent-evasion",
    term: "Rent Evasion",
  ...
  },
  {
    ...
    definition: "Process of responsibly reporting security flaws to affected parties before public disclosure. Solana has a bug bounty program through Immunefi, and most major protocols maintain responsible disclosure policies. Coordinated disclosure gives teams time to patch vulnerabilities before exploitation.",
    category: "security",
    related: ["audit", "bug-bounty"]
  },
  {
    id: "bug-bounty",
    term: "Bug Bounty",
    ...
    category: "security",
    related: ["audit", "vulnerability-disclosure"]
  }
];

// data/terms/dev-tools.json
var dev_tools_default = [
  {
    id: "anchor",
    ...
    category: "dev-tools",
    related: ["program", "idl", "discriminator"]
  },
  {
    id: "anchor-account-macro",
    term: "#[account] Macro (Anchor)",
    definition: "The Anchor macro applied to structs to define on-chain account data layouts. `#[account]` auto-derives Borsh serialization, adds an 8-byte discriminator prefix (SHA-256 of 'account:<Name>'), and implements space calculation. Optional attributes: `#[account(zero_copy)]` for zero-copy deserialization of large accounts.",
    category: "dev-tools",
    related: ["anchor", "discriminator", "borsh"]
  },
  {
    id: "anchor-derive-accounts",
    term: "#[derive(Accounts)] (Anchor)",
    definition: "The Anchor macro that defines the accounts struct for an instruction. Each field specifies an account with validation constraints. Account types include: `Account<'info, T>` (deserialized), `Signer<'info>` (must sign), `Program<'info, T>` (program reference), `SystemAccount<'info>`, and `UncheckedAccount<'info>` (no validation, use carefully).",
    category: "dev-tools",
    related: ["anchor", "anchor-constraints"]
  },
  {
    id: "anchor-constraints",
    term: "Anchor Constraints",
    definition: "Declarative validation rules on Anchor account fields. Key constraints: `#[account(mut)]` (writable), `#[account(init, payer=x, space=n)]` (create), `#[account(seeds=[...], bump)]` (PDA validation), `#[account(has_one=field)]` (field equality), `#[account(constraint = expr)]` (custom boolean), `#[account(close=target)]` (close and reclaim rent).",
    category: "dev-tools",
    aliases: ["has_one", "constraint", "seeds"],
    related: ["anchor", "anchor-derive-accounts"]
  },
  {
    id: "anchor-error",
    term: "Anchor Error Codes",
    definition: "Anchor's error handling system. `#[error_code]` defines an enum of custom errors (auto-numbered from 6000). `require!(condition, ErrorCode)` and `require_keys_eq!()` macros provide concise validation. Errors include the program name and error name in logs. Custom errors return in the transaction metadata and can be parsed by clients via the IDL.",
    category: "dev-tools",
    aliases: ["error_code", "require!"],
    related: ["anchor"]
  },
  {
    id: "anchor-context",
    term: "Context<T> (Anchor)",
    definition: "The `Context<T>` struct passed to every Anchor instruction handler, where T is the accounts struct. Fields: `ctx.accounts` (validated accounts), `ctx.program_id` (current program), `ctx.remaining_accounts` (extra accounts not in struct), and `ctx.bumps` (map of PDA bumps found during validation).",
    category: "dev-tools",
    related: ["anchor", "instruction"]
  },
  {
    id: "anchor-init",
    term: "anchor init / anchor build / anchor test",
    definition: "Anchor CLI commands for project lifecycle. `anchor init <name>` scaffolds a new project (program, tests, Anchor.toml). `anchor build` compiles to SBF and generates the IDL. `anchor test` builds, starts a local validator, deploys, and runs Mocha/Jest tests. `anchor deploy` deploys to the configured cluster. `anchor verify` checks deployed bytecode.",
    category: "dev-tools",
    related: ["anchor", "solana-cli"]
  },
  {
    id: "anchor-events",
    term: "Anchor Events (emit!)",
    definition: "Anchor's event system using the `emit!()` macro. Events are defined with `#[event]` on structs and emitted as base64-encoded data in program logs (prefixed with a discriminator). Clients subscribe via `program.addEventListener('EventName', callback)`. Events cost compute units for logging and are limited by the 10KB log size.",
    category: "dev-tools",
    related: ["anchor", "log"]
  },
  {
    id: "seahorse",
    term: "Seahorse (Python \u2192 Anchor)",
    definition: "A Python-to-Anchor transpiler that lets developers write Solana programs in Python syntax and compiles them to Rust/Anchor code. Seahorse supports a subset of Python with Solana-specific decorators. It lowers the barrier to entry for Python developers but may lag behind Anchor features. Suitable for learning and prototyping.",
    category: "dev-tools",
    related: ["anchor", "program"]
  },
  {
    id: "solana-cli",
    term: "Solana CLI",
    definition: "The official command-line tool for interacting with Solana clusters. Key commands: `solana balance`, `solana transfer`, `solana program deploy`, `solana config set --url`, `solana airdrop` (devnet). Configuration in `~/.config/solana/cli/config.yml` sets the default cluster URL and keypair path.",
    category: "dev-tools",
    aliases: ["solana-cli", "solana"],
    related: ["solana-keygen", "solana-test-validator"]
  },
  {
    id: "solana-keygen",
    term: "solana-keygen",
  ...
  {
    id: "solana-web3js-v2",
    term: "Web3.js v2 (@solana/kit)",
    definition: "The next-generation Solana TypeScript SDK (@solana/kit, formerly @solana/web3.js v2) featuring a modular, functional API. Unlike v1's monolithic classes, v2 uses composable functions and tree-shaking for smaller bundles. Key changes: pipe-based transaction building, codec-based serialization, and native support for versioned transactions. Significantly smaller bundle size than v1.",
    category: "dev-tools",
    aliases: ["@solana/kit", "Solana Kit"],
    related: ["solana-web3js"]
  },
  {
    id: "solana-rust-sdk",
    term: "Solana Rust SDK (solana-sdk)",
    definition: "The Rust crate (solana-sdk) providing types and utilities for building Solana clients and programs. For on-chain programs, use `solana-program` (no-std compatible subset). For off-chain clients, use `solana-sdk` (full features including keypair generation, transaction signing). The SDK includes types like Pubkey, Instruction, Transaction, and Account.",
    category: "dev-tools",
    aliases: ["solana-sdk", "solana-program"],
    related: ["program", "anchor"]
  },
  {
    id: "solana-program-crate",
    term: "solana-program Crate",
    definition: "The Rust crate (solana-program) providing the core types and syscalls for on-chain program development. It includes: entrypoint!, msg!, AccountInfo, Pubkey, ProgramResult, invoke/invoke_signed, and system instruction builders. This crate compiles to SBF and must be used instead of solana-sdk for on-chain code.",
    category: "dev-tools",
    related: ["solana-rust-sdk", "program"]
  },
  {
    id: "spl-crates",
    term: "SPL Crates (spl-token, spl-associated-token-account)",
  ...
  {
    id: "metaplex-sdks",
    term: "Metaplex SDKs (Umi, JS SDK)",
    definition: "Client libraries for interacting with Metaplex programs. Umi is the current framework-agnostic TypeScript SDK with plugins for each Metaplex program (Token Metadata, Bubblegum, Core). The older @metaplex-foundation/js SDK is deprecated in favor of Umi. Umi provides a unified interface for serialization, signing, and RPC across all Metaplex operations.",
    category: "dev-tools",
    aliases: ["Umi"],
    related: ["metaplex-metadata", "nft"]
  },
  {
    id: "solana-playground",
    term: "Solana Playground (SolPG)",
    definition: "A browser-based IDE (beta.solpg.io) for writing, building, testing, and deploying Solana programs without local toolchain setup. SolPG supports Anchor and native Rust programs, includes a built-in wallet, and connects to devnet/mainnet. Features include: code editor with syntax highlighting, one-click build/deploy, and a transaction explorer.",
    category: "dev-tools",
    aliases: ["SolPG"],
    related: ["anchor", "local-development"]
  },
  {
    id: "cargo-build-sbf",
    term: "cargo-build-sbf",
  ...
  {
    id: "solana-verify",
    term: "Solana Verify (solana-verifiable-build)",
    definition: "A tool and standard for verifying that deployed on-chain program bytecode matches public source code. `solana-verify build` produces a deterministic build using Docker, and `solana-verify verify-from-repo` checks the deployed binary against a Git repo. Verified programs are listed on the Solana Explorer and OtterSec's verify registry, increasing user trust.",
    category: "dev-tools",
    related: ["program", "audit"]
  },
  {
    id: "amman",
    term: "Amman",
    definition: "A testing toolkit by Metaplex that wraps solana-test-validator with additional features: account snapshots and restoration, labeled accounts for debugging, transaction history explorer, and a relay for live-reloading. Amman is particularly useful for testing Metaplex programs and complex multi-instruction workflows.",
    category: "dev-tools",
    related: ["testing", "local-development"]
  },
  {
    id: "trident",
    term: "Trident (Fuzzer)",
    ...
    category: "dev-tools",
    related: ["fuzzing", "anchor", "testing"]
  },
  {
    id: "solana-program-test",
    term: "solana-program-test",
    definition: "A Rust testing framework in the solana-sdk that provides a BanksClient for in-process program testing. Programs are loaded directly into a test Bank without a network. Supports adding accounts, advancing clock, and processing transactions. Slower than LiteSVM/Bankrun but provides the most accurate runtime behavior for Rust-only test suites.",
    category: "dev-tools",
    related: ["testing", "bankrun"]
  },
  {
    id: "squads",
    term: "Squads (Multisig)",
    definition: "A multisig protocol on Solana (Squads v3/v4) enabling M-of-N approval for transactions. Used to manage program upgrade authorities, treasury wallets, and DAO operations. Squads creates a multisig PDA that acts as the authority; members propose transactions that execute once the threshold is reached. Critical for production program security.",
    category: "dev-tools",
    related: ["multisig", "authority", "upgrade-authority"]
  },
  {
    id: "multisig",
    term: "Multisig",
    definition: "An account controlled by multiple parties requiring M-of-N signatures to authorize actions. On Solana, multisig is implemented by programs like Squads rather than at the protocol level. Use cases: program upgrade authority (require 3-of-5 team members), treasury management, and DAO governance execution.",
    category: "dev-tools",
    related: ["squads", "authority"]
  },
  {
    id: "magicblock",
    term: "MagicBlock (Ephemeral Rollups)",
    definition: "A framework for building on-chain games on Solana using Ephemeral Rollups. MagicBlock allows game state to temporarily move to a dedicated SVM instance for high-frequency updates (sub-second ticks), then settle back to Solana mainnet. This enables real-time multiplayer games while maintaining Solana's composability and security.",
    category: "dev-tools",
    related: ["svm", "gaming"]
  },
  {
    id: "bolt",
    term: "Bolt (ECS Framework)",
  ...
  },
  {
    ...
    category: "dev-tools",
    related: ["pinocchio", "anchor", "solana-program-crate"]
  },
  {
    id: "codama",
    term: "Codama",
    definition: "A code generation toolchain that takes a Solana program's IDL and generates fully-typed client SDKs in multiple languages. Codama processes IDLs produced by Shank or Anchor and emits TypeScript clients. It is the successor of the Kinobi project and is commonly used in the Pinocchio + Shank + Codama workflow for native Solana program development.",
    category: "dev-tools",
    aliases: ["Codama SDK Generator"],
    related: ["kinobi", "shank", "idl", "pinocchio"]
  },
  {
    id: "kinobi",
    term: "Kinobi",
  ...
  },
  {
    id: "shank",
    term: "Shank",
    definition: "A Rust macro-based tool for generating IDL files from native Solana programs that do not use Anchor. Shank provides attribute macros like #[derive(ShankAccount)] and #[derive(ShankInstruction)] that annotate program code to produce IDL output compatible with Codama/Kinobi for client generation. A key component of the Pinocchio + Shank + Codama workflow.",
    category: "dev-tools",
    related: ["codama", "pinocchio", "idl"]
  },
  {
    id: "anchor-v031",
    term: "Anchor v0.31",
    definition: "A major release (March 2025) of the Anchor framework introducing custom discriminators (overriding the default 8-byte discriminator to save transaction space), LazyAccount for on-demand deserialization to reduce compute costs, and stack memory optimizations for init constraints. Positioned as a major milestone on the roadmap toward v1.0.",
    category: "dev-tools",
    aliases: ["Anchor 0.31.0"],
    related: ["anchor", "discriminator"]
  },
  {
    id: "solana-mobile-stack",
    term: "Solana Mobile Stack (SMS)",
    definition: "A comprehensive mobile development toolkit for building Solana-native Android dApps, including an SDK for wallet integration, a Seed Vault for secure key management, and the Solana dApp Store for distribution outside traditional app stores. SMS enables mobile-first interaction with Solana programs, token management, and transaction signing from Android devices.",
    category: "dev-tools",
    aliases: ["SMS", "Solana Mobile"],
    related: ["seeker", "gaming", "wallet-adapter"]
  },
  {
    id: "seeker",
    term: "Seeker (Solana Phone)",
    definition: "The second-generation Solana mobile device with a native Seed Vault wallet, dApp Store 2.0, and a soulbound Seeker Genesis Token for exclusive ecosystem rewards. Priced at $450-$500, it surpassed 1 million pre-orders across 57 countries. Saga's BONK airdrops briefly pushed secondary market prices above $2,000, validating token-incentive distribution via hardware.",
    category: "dev-tools",
    aliases: ["Chapter 2", "Solana Seeker"],
    related: ["solana-mobile-stack", "wallet-general"]
  },
  {
    id: "formal-verification",
    term: "Formal Verification",
    definition: "The use of mathematical proofs to verify that a smart contract's behavior matches its specification for all possible inputs, providing stronger guarantees than testing alone. Techniques include model checking, deductive verification, SAT/SMT solving, and interactive theorem proving. Tools like Halmos (a16z), Kontrol, and Certora Prover enable proving properties like 'total supply never exceeds max.'",
    category: "dev-tools",
    aliases: ["FV"],
    related: ["invariant-testing", "symbolic-execution", "audit"]
  },
  {
    id: "invariant-testing",
    term: "Invariant Testing",
    definition: "A property-based testing approach where developers define invariants (properties that must always hold true) and a fuzzer generates random sequences of function calls attempting to violate them. Unlike unit tests that check specific scenarios, invariant tests explore the state space stochastically. Tools like Foundry invariant testing, Echidna, and Medusa support this approach.",
    category: "dev-tools",
    aliases: ["Property-Based Testing"],
    related: ["formal-verification", "symbolic-execution", "fuzzing"]
  },
  {
  ...
  {
    id: "init-if-needed",
    term: "init_if_needed (Anchor Constraint)",
    ...
    category: "dev-tools",
    related: ["anchor-constraints", "reinitialization", "idempotency"]
  },
  {
    id: "has-one-constraint",
    term: "has_one Constraint (Anchor)",
    definition: "An Anchor validation constraint that asserts a field stored in one account matches the pubkey of another account in the instruction's accounts struct. For example, #[account(has_one = authority)] on a vault account verifies that vault.authority equals the authority account's key. Failing the check returns an Anchor error. has_one prevents unauthorized callers from passing arbitrary accounts for privileged roles.",
    category: "dev-tools",
    related: ["anchor-constraints", "insufficient-constraints", "authority"]
  },
  {
    id: "unchecked-account",
    term: "UncheckedAccount (Anchor)",
    definition: "An Anchor account type (UncheckedAccount<'info>) that performs no automatic validation \u2014 no owner check, no discriminator check, no deserialization. It is an alias for AccountInfo and should only be used when the program manually validates the account or when the account belongs to an external program. Requires a /// CHECK: doc comment explaining why it is safe. Misuse is a common audit finding.",
    category: "dev-tools",
    aliases: ["AccountInfo wrapper"],
    related: ["anchor-derive-accounts", "missing-owner-check", "account-info"]
  },
  {
    id: "interface-account",
    term: "InterfaceAccount (Anchor)",
    definition: "An Anchor account type for CPI interactions with programs that implement a defined interface, allowing a single account type to work with multiple program implementations. For example, InterfaceAccount<'info, TokenAccount> accepts accounts from both the SPL Token Program and Token-2022, since both implement the token interface. This enables programs to be compatible with multiple token standards without separate code paths.",
    category: "dev-tools",
    related: ["anchor", "cpi", "spl-token", "token-2022"]
  },
  {
    id: "mollusk",
    term: "Mollusk",
  ...
  },
  {
    ...
    category: "dev-tools",
    related: ["anchor", "seahorse", "typescript"]
  },
  {
    id: "solana-program-deploy",
    term: "solana program deploy",
    definition: "The Solana CLI command for deploying programs to the network, replacing the older solana deploy command. Usage: solana program deploy target/deploy/program.so. The command creates a buffer account, uploads the ELF binary in chunks, and finalizes deployment via the Upgradeable BPF Loader. Supports --program-id to specify the program keypair and --upgrade-authority for setting the upgrade authority. Also used for program upgrades by deploying new bytecode to an existing program ID.",
    category: "dev-tools",
    related: ["solana-cli", "upgradeable-loader", "buffer-account"]
  },
  {
    id: "solana-explorer-tool",
    term: "Solana Explorer",
    definition: "The official Solana Foundation block explorer at explorer.solana.com providing transaction details, account inspection, program views, validator info, and network statistics. Supports mainnet, devnet, testnet, and custom RPC endpoints. The default tool for inspecting on-chain activity.",
    category: "dev-tools",
    related: ["explorer", "rpc", "solana-cli"]
  },
  {
    id: "anchor-workspace",
    term: "Anchor Workspace",
    definition: "The project structure and configuration defined by Anchor.toml that organizes Solana program development. Includes program source directories, test configurations, provider settings, and deployment targets. The workspace pattern enables multi-program monorepos with shared test suites and coordinated builds.",
    category: "dev-tools",
    related: ["anchor", "anchor-toml", "monorepo"]
  },
  {
    id: "program-test-framework",
    term: "Program Test Framework (solana-program-test)",
    definition: "Solana's built-in testing framework that spins up a lightweight BanksClient runtime to execute programs in an isolated environment without a full validator. Supports adding programs, setting account state, advancing slots/time, and processing transactions. The foundation for most Solana program integration tests.",
    category: "dev-tools",
    related: ["solana-program-test", "bankrun", "litesvm"]
  },
  {
    id: "idl-generation",
    term: "IDL Generation",
    definition: "Process of producing an Interface Definition Language file from a Solana program's source code. Anchor auto-generates IDLs from Rust program definitions. Shank and Codama provide IDL generation for non-Anchor programs. IDLs enable automatic TypeScript/Python client generation and human-readable instruction decoding.",
    category: "dev-tools",
    related: ["idl", "anchor", "codama", "shank"]
  },
  {
    id: "account-deserializer",
    term: "Account Deserializer",
    definition: "Tool or library that converts raw on-chain account bytes into structured data using program-specific schemas. Anchor provides automatic deserialization via its IDL. For non-Anchor programs, developers use Borsh, custom byte parsing, or tools like Codama-generated clients to interpret account data.",
    category: "dev-tools",
    related: ["borsh", "serialization", "idl", "account-data"]
  },
  {
    id: "quasar",
    term: "Quasar",
    definition: "A Rust framework for building Solana programs that combines Anchor-like developer ergonomics with lower-level performance control. Quasar provides procedural macros for account validation and instruction dispatching while generating smaller binaries and consuming fewer compute units than Anchor by avoiding some of its heavier abstractions. Positioned between Anchor's full-featured convenience and Pinocchio's bare-metal approach.",
    category: "dev-tools",
    related: ["anchor", "pinocchio", "steel"]
  },
  {
    id: "golana",
    term: "Golana",
...
var network_default = [
  ...
  },
  {
    ...
    category: "network",
    related: ["sealevel", "tps", "account-locking"]
  },
  {
    id: "account-locking",
    term: "Account Locking (Read/Write)",
    ...
    category: "network",
    related: ["parallel-execution", "writable"]
  },
  {
    id: "base-fee",
    term: "Base Fee",
    definition: "The fixed, non-negotiable minimum fee charged per transaction signature on Solana, currently set at 5,000 lamports (0.000005 SOL) per signature. A transaction with N signatures pays N \xD7 5,000 lamports as a base fee regardless of compute usage or network load. Fifty percent of the base fee is burned and fifty percent is paid to the current slot leader, making Solana mildly deflationary with usage.",
    category: "network",
    related: ["priority-fee", "transaction-fee", "compute-units"]
  },
  {
    id: "priority-fee",
    term: "Priority Fee",
    definition: "An optional additional fee paid on top of the base fee to increase the likelihood that a transaction is processed quickly by the current leader, expressed as a price in micro-lamports per compute unit (CU). The total priority fee equals (compute unit price \xD7 compute unit limit) / 1,000,000 lamports. Leaders sort transactions in their queue by fee-per-CU, so setting a competitive priority fee is the primary mechanism for ensuring reliable transaction landing during congestion.",
    category: "network",
    aliases: ["Compute Unit Price"],
    related: ["compute-units", "base-fee", "transaction-fee"]
  },
  {
    id: "transaction-fee",
    term: "Transaction Fee",
    definition: "The total SOL cost to submit a transaction on Solana, composed of the base fee (5,000 lamports per signature) plus any optional priority fee (compute unit price \xD7 compute units consumed). Unlike Ethereum, Solana fees are not purely dynamic gas auctions \u2014 the base fee is fixed and priority fees are additive. Fees are deducted from the fee-payer account before execution; if the account lacks sufficient SOL, the transaction fails.",
    category: "network",
    related: ["base-fee", "priority-fee", "lamport"]
  },
  {
    id: "fee-market",
    term: "Fee Market (Local/Global)",
    definition: "Solana operates a local fee market rather than a global one: priority fee competition is scoped to transactions that contend for the same writable accounts, so congestion on one hot account (e.g., a popular AMM pool) does not raise fees for unrelated transactions. This account-centric model means that a highly contested token mint or DEX pool can require very high priority fees to land while other transactions proceed cheaply in parallel. The local fee market is enforced by the scheduler's per-account fee-priority ordering.",
    category: "network",
    related: ["priority-fee", "account-locking"]
  },
  {
    id: "stake-weighted-qos",
    term: "Stake-Weighted QoS (SWQoS)",
  ...
  },
  {
    ...
    category: "network",
    related: ["blockhash", "transaction", "offline-signing"]
  },
  {
    id: "offline-signing",
  ...
  {
    id: "solana-runtime-v2",
    term: "Solana Runtime v2",
  ...
  {
    id: "token-extensions-network",
    term: "Token Extensions (Network Impact)",
    definition: "The aggregate effect on Solana network resources \u2014 primarily compute units and account space \u2014 introduced by Token Extensions (Token-2022), which adds on-chain extension data and additional CPI calls compared to the original SPL Token program. Tokens with multiple extensions (e.g., transfer fees + confidential transfers + interest-bearing) require significantly more compute budget per instruction, affecting transaction size, CPI depth, and priority fee calculations. Developers must account for the higher CU consumption of Token-2022 instructions when setting compute unit limits and priority fees to ensure reliable transaction landing.",
    category: "network",
    related: ["token-2022", "compute-units"]
  },
  {
    id: "session-keys",
    term: "Session Keys",
    definition: "Ephemeral keypairs with scoped permissions and time limits that allow users to delegate transaction signing to a temporary key, eliminating repeated wallet approval prompts during interactive sessions. On Solana, the session-keys crate provides Anchor macros for defining and validating session tokens. The Swig smart wallet SDK extends this with programmable wallets offering delegated permissions and gasless UX.",
    category: "network",
    aliases: ["Ephemeral Keys"],
    related: ["gaming", "wallet-general"]
  },
  {
    id: "confirmation-time",
    term: "Confirmation Time",
    definition: "The wallclock duration between a transaction being submitted and achieving a desired commitment level. On Solana, processed confirmation takes ~400ms (leader executes), confirmed (optimistic) takes ~400ms-1s (supermajority votes), and finalized (rooted) typically takes 26-32 slots (~10-13 seconds). Actual confirmation time varies with network congestion, leader distance, and transaction priority fee.",
    category: "network",
    related: ["commitment-levels", "finality", "transaction-landing"]
  },
  {
    id: "cooldown-period",
    term: "Cooldown Period",
    definition: "The number of epochs after stake deactivation during which the staked SOL remains locked and cannot be withdrawn. During cooldown, the stake progressively becomes available for withdrawal over one or more epoch boundaries. The cooldown mechanism prevents rapid unstaking that could destabilize consensus by suddenly reducing the validator's stake weight.",
    category: "network",
    related: ["stake", "epoch", "warmup-period"]
  },
  {
    id: "light-client",
    term: "Light Client",
  ...
  {
    id: "nakamoto-coefficient",
    term: "Nakamoto Coefficient",
    definition: "The minimum number of independent entities (validators, mining pools, or staking providers) that would need to collude to compromise or halt a blockchain network, typically by controlling 33%+ of stake or hash power. A higher Nakamoto coefficient indicates greater decentralization. Solana's Nakamoto coefficient fluctuates around 19-31 depending on stake distribution among validators.",
    category: "network",
    related: ["validator", "stake", "supermajority"]
  },
  {
    id: "node-count",
    term: "Node Count",
  ...
  },
  {
    id: "skipped-slot",
    term: "Skipped Slot",
    definition: "A slot in which the assigned leader validator fails to produce a block, resulting in no entries being added to the ledger for that slot. Slots are skipped when the leader is offline, too slow to produce a block within the time window, or unable to reach other validators. Skipped slots do not increment block height and contribute to a validator's skip rate metric.",
    category: "network",
    related: ["slot", "skip-rate", "leader-schedule"]
  },
  {
    id: "thin-client",
    term: "Thin Client",
    definition: "A blockchain client that trusts the cluster's communication without independently verifying transaction validity, relying on RPC responses from validators for all state queries. Thin clients are the simplest integration method \u2014 most dApp frontends and wallet interfaces operate as thin clients by trusting their configured RPC provider to return accurate data.",
    category: "network",
    related: ["light-client", "rpc", "rpc-node"]
  },
  {
    id: "vote-credit",
    term: "Vote Credit",
    definition: "A reward tally credited to a validator's vote account each time one of its votes reaches the root (becomes finalized). Vote credits accumulate throughout an epoch and are used at epoch boundaries to calculate staking rewards \u2014 validators with more credits earn proportionally more rewards for themselves and their delegators. Credits incentivize consistent, timely voting.",
    category: "network",
    related: ["vote-account", "staking-rewards", "root"]
  },
  {
    id: "warmup-period",
    term: "Warmup Period",
  ...
  },
  {
    ...
    category: "network",
    related: ["fee-market", "priority-fee", "account-locking"]
  },
  {
  ...
  },
  {
    ...
    category: "network",
    related: ["mev-protection", "tpu-client", "block-engine"]
  },
  {
    id: "bundle-simulation",
    term: "Bundle Simulation",
    ...
    category: "network",
    related: ["bundle", "block-engine", "jito"]
  }
];

// data/terms/blockchain-general.json
var blockchain_general_default = [
  {
    id: "blockchain",
    term: "Blockchain",
    definition: "A distributed, append-only ledger that records transactions in cryptographically linked blocks. Each block contains a hash of the previous block, forming an immutable chain. Nodes in the network maintain copies of the ledger and reach agreement through consensus mechanisms. Blockchains enable trustless, decentralized record-keeping without a central authority.",
    category: "blockchain-general",
    related: ["consensus", "distributed-ledger", "block"]
  },
  {
    id: "consensus",
    term: "Consensus Mechanism",
    definition: "The protocol by which nodes in a distributed network agree on the current state of the ledger. Common mechanisms include Proof of Work (Bitcoin), Proof of Stake (Ethereum, Solana), and BFT variants. Consensus ensures all honest nodes converge on the same transaction history despite potential network delays or malicious actors.",
    category: "blockchain-general",
    aliases: ["Consensus Algorithm"],
    related: ["proof-of-stake", "proof-of-work", "byzantine-fault-tolerance"]
  },
  {
    id: "proof-of-stake",
    ...
    category: "blockchain-general",
    aliases: ["PoS"],
    related: ["consensus", "staking-general", "slashing-general"]
  },
  {
    id: "proof-of-work",
    term: "Proof of Work (PoW)",
    definition: "A consensus mechanism where miners compete to solve computationally expensive puzzles to produce blocks and earn rewards. PoW provides strong security (51% attack resistance) but is energy-intensive. Bitcoin and pre-Merge Ethereum use PoW. The difficulty adjusts to maintain target block times regardless of total network hash power.",
    category: "blockchain-general",
    aliases: ["PoW", "Mining"],
    related: ["consensus", "hash-function"]
  },
  {
    id: "distributed-ledger",
    term: "Distributed Ledger",
    definition: "A database replicated and synchronized across multiple nodes in a network without central administration. All participants maintain identical copies of the ledger. Changes are propagated through consensus. Blockchains are the most common type of distributed ledger, but not all distributed ledgers use a chain-of-blocks structure.",
    category: "blockchain-general",
    aliases: ["DLT"],
  ...
  },
  {
    ...
    definition: "A one-way mathematical function that maps arbitrary-length input to a fixed-size output (digest). Properties: deterministic, fast to compute, infeasible to reverse, collision-resistant. SHA-256 (used in Bitcoin, Solana PoH) produces 256-bit hashes. Keccak-256 is used in Ethereum. Hashes are used for block linking, Merkle trees, and data integrity.",
    category: "blockchain-general",
    aliases: ["Hash", "SHA-256", "Keccak"],
    related: ["merkle-tree-general", "proof-of-work"]
  },
  {
    id: "merkle-tree-general",
    term: "Merkle Tree",
    definition: "A binary tree where each leaf node is a hash of data and each internal node is the hash of its two children. The root hash (Merkle root) uniquely represents all underlying data. Merkle trees enable efficient proof of inclusion\u2014proving a specific element exists requires only O(log n) hashes. Used in blockchains for transaction verification and state storage.",
    category: "blockchain-general",
    aliases: ["Hash Tree"],
    related: ["hash-function", "merkle-proof"]
  },
  {
    id: "smart-contract-general",
    term: "Smart Contract",
  ...
  },
  {
    ...
    category: "blockchain-general",
    related: ["compute-units", "transaction-fee-general"]
  },
  {
    id: "transaction-fee-general",
    term: "Transaction Fee",
  ...
  },
  {
    ...
    category: "blockchain-general",
    aliases: ["BFT"],
    related: ["consensus", "tower-bft"]
  },
  {
    id: "finality-general",
    term: "Finality (Blockchain)",
    ...
    category: "blockchain-general",
    related: ["finality", "consensus"]
  },
  {
    id: "staking-general",
    term: "Staking",
    definition: "The process of locking cryptocurrency as collateral to participate in network consensus (validation) or to earn rewards. Stakers either run validators directly or delegate to existing validators. Staking provides economic security\u2014validators risk losing staked tokens (slashing) for misbehavior. Annual staking yields typically range from 3-15% depending on the network.",
    category: "blockchain-general",
    related: ["proof-of-stake", "delegator", "slashing-general"]
  },
  {
    id: "slashing-general",
    term: "Slashing",
    ...
    category: "blockchain-general",
    related: ["staking-general", "proof-of-stake"]
  },
  {
    id: "bridge",
    term: "Bridge (Cross-Chain)",
    definition: "A protocol that enables transferring assets or data between different blockchains. Bridges use mechanisms like lock-and-mint (lock on source, mint wrapped on destination), liquidity pools, or relay networks. Bridges are high-value attack targets\u2014Wormhole ($320M, 2022), Ronin ($625M, 2022). Wormhole is Solana's primary bridge to Ethereum and other chains.",
    category: "blockchain-general",
    aliases: ["Cross-Chain Bridge"],
    related: ["wormhole"]
  },
  {
    id: "evm",
    term: "EVM (Ethereum Virtual Machine)",
    definition: "The stack-based virtual machine that executes smart contracts on Ethereum and EVM-compatible chains (Polygon, Avalanche, BSC, Arbitrum). Programs are compiled to EVM bytecode from Solidity or Vyper. The EVM is single-threaded\u2014all transactions execute sequentially. In contrast, Solana's SVM supports parallel execution and uses a register-based design.",
    category: "blockchain-general",
    aliases: ["EVM"],
    related: ["svm", "solidity"]
  },
  {
    id: "solidity",
    term: "Solidity",
  ...
  },
  {
    ...
    category: "blockchain-general",
    related: ["cluster", "local-development", "faucet"]
  },
  {
    id: "mainnet",
    term: "Mainnet",
  ...
  },
  {
    ...
    definition: "The ability of different blockchains to communicate and share data or assets. Achieved through bridges (Wormhole), messaging protocols (LayerZero, Axelar), and shared standards. Cross-chain interoperability enables multi-chain DeFi, unified liquidity, and chain-agnostic applications. Solana connects to other chains primarily through Wormhole and Circle's CCTP.",
    category: "blockchain-general",
    related: ["bridge"]
  },
  {
  ...
  },
  {
    ...
    definition: "A credible, collateral-backed promise from a block proposer or sequencer that a transaction will be included in a future block, providing near-instant soft confirmation before on-chain finality. Preconfirmations address latency in based rollups where Ethereum's 12-second block time is too slow for UX. Implementations achieve sub-10ms guarantees, secured by restaked ETH with slashing for broken commitments.",
    category: "blockchain-general",
    aliases: ["Preconf"],
    related: ["based-rollup", "sequencer", "finality-general"]
  },
  {
    id: "passkey-wallet",
    term: "Passkey / WebAuthn Wallet",
    ...
    related: ["account-abstraction", "wallet-general"]
  },
  {
    id: "erc-6551",
  ...
  },
  {
    ...
    category: "blockchain-general",
    aliases: ["Script"],
    related: ["bitcoin", "utxo-model", "op-cat", "taproot"]
  },
  {
    id: "segwit",
    term: "SegWit (Segregated Witness)",
    definition: "A Bitcoin soft fork activated in August 2017 (BIP 141) that separates transaction signature data (witness) from the transaction body, fixing transaction malleability and increasing effective block capacity from 1 MB to approximately 4 MB via a weight unit system. SegWit introduced a versioned witness program structure that enabled future upgrades like Taproot without additional soft forks. It was a prerequisite for the Lightning Network's secure operation.",
    category: "blockchain-general",
    aliases: ["Segregated Witness", "BIP 141"],
    related: ["bitcoin", "taproot", "lightning-network"]
  },
  {
    id: "taproot",
    term: "Taproot",
    definition: "A Bitcoin soft fork activated in November 2021 (BIPs 340-342) introducing Schnorr signatures for efficient key/signature aggregation, MAST (Merkelized Abstract Syntax Trees) for committing complex spending conditions where only the exercised branch is revealed on-chain, and Tapscript extending Bitcoin Script with new opcodes. Taproot makes complex transactions (multisig, timelocks, conditional spending) indistinguishable from simple single-key transactions, improving privacy and efficiency.",
    category: "blockchain-general",
    aliases: ["BIP 340", "Schnorr", "MAST", "Tapscript"],
    related: ["bitcoin", "segwit", "bitcoin-script", "bitcoin-ordinals"]
  },
  {
    id: "lightning-network",
    term: "Lightning Network",
    definition: "A Layer 2 payment channel network built on Bitcoin that enables near-instant, low-cost transactions by conducting most activity off-chain. Two parties open a channel by locking BTC in a multisig transaction, then exchange signed commitment transactions off-chain; only the opening and closing transactions are broadcast on-chain. Payments can be routed across multiple channels using Hash Time-Locked Contracts (HTLCs), enabling trustless multi-hop transfers.",
    category: "blockchain-general",
    aliases: ["Lightning", "LN"],
    related: ["bitcoin", "bitcoin-layer-2", "segwit", "layer-2"]
  },
  {
    id: "bitcoin-ordinals",
    term: "Bitcoin Ordinals",
    definition: "A protocol created by Casey Rodarmor in January 2023 that assigns a unique serial number (ordinal) to each individual satoshi based on mining order, enabling satoshis to carry arbitrary data (inscriptions) stored in Taproot witness data. Inscriptions can contain images, text, HTML, or other media up to the ~4 MB block weight limit, creating non-fungible digital artifacts natively on Bitcoin. Over 63 million inscriptions were created by early 2024.",
    category: "blockchain-general",
    aliases: ["Ordinals", "Inscriptions"],
    related: [
      "bitcoin",
      "taproot",
      "satoshi-unit",
  ...
  },
  {
    ...
    category: "blockchain-general",
    aliases: ["BRC20"],
    related: ["bitcoin-ordinals", "runes-protocol", "utxo-model"]
  },
  {
    id: "runes-protocol",
    term: "Runes",
    definition: "A Bitcoin fungible token protocol created by Casey Rodarmor that launched at block 840,000 on April 20, 2024, coinciding with the fourth Bitcoin halving. Unlike BRC-20, Runes is UTXO-native, using OP_RETURN to store token data directly in transaction outputs without creating junk UTXOs. The protocol allows multiple token transfers in a single transaction and briefly accounted for over 80% of Bitcoin transactions in its first days.",
    category: "blockchain-general",
  ...
  },
  {
    ...
    related: ["bitcoin", "proof-of-work-mining", "transaction-fee-general"]
  },
  {
    id: "satoshi-unit",
    term: "Satoshi",
  ...
  },
  {
    ...
    category: "blockchain-general",
    aliases: ["Bitcoin L2"],
    related: ["bitcoin", "lightning-network", "bitvm", "layer-2"]
  },
  {
    id: "bitvm",
    term: "BitVM",
  ...
  },
  {
    ...
    category: "blockchain-general",
    related: ["sol", "gas", "transaction-fee-general"]
  },
  {
    id: "point",
    term: "Point (Validator Rewards)",
  ...
  },
  {
    ...
    category: "blockchain-general",
    related: ["finality-general", "proof-of-stake", "slashing-general"]
  },
  {
    id: "bls-signature",
    term: "BLS Signature",
    ...
    related: ["consensus", "validator", "simd"]
  },
  {
    id: "on-curve-off-curve",
    term: "On-Curve / Off-Curve",
    definition: "In elliptic curve cryptography, a point is 'on-curve' if it satisfies the curve equation (e.g., Ed25519) and can serve as a valid public key with a corresponding private key. An 'off-curve' point does not lie on the curve and has no associated private key. Solana Program Derived Addresses (PDAs) are deliberately generated as off-curve points by appending a bump seed, guaranteeing that no private key exists and only the owning program can sign for the address.",
    category: "blockchain-general",
    related: ["hash-function", "signer"]
  },
  {
    id: "clearing-settlement",
    term: "Clearing and Settlement",
    definition: "Two phases of completing a financial transaction. Clearing: reconciling orders and calculating net obligations. Settlement: actual exchange of securities and cash. Traditional markets: T+1/T+2. Blockchain enables atomic settlement \u2014 simultaneous instant exchange eliminating counterparty risk.",
    category: "blockchain-general",
    aliases: ["Clearing and Settlement"],
    related: ["atomic-settlement", "finality-general", "dvp"]
  },
  {
    id: "dvp",
    term: "DvP (Delivery vs Payment)",
    definition: "Settlement mechanism ensuring security delivery occurs simultaneously and conditionally with payment. Blockchain's atomic transactions naturally implement DvP \u2014 a single transaction transfers tokenized security and USDC atomically.",
    category: "blockchain-general",
    aliases: ["Delivery vs Payment", "DvP"],
    related: ["clearing-settlement", "atomic-settlement", "finality-general"]
  },
  {
    id: "csd",
    term: "CSD (Central Securities Depository)",
    definition: "Institution holding securities in electronic form as central ownership registry. Examples: DTCC (US), Euroclear (Europe). On blockchain, the token program serves as a permissionless alternative \u2014 the ledger itself maintains the ownership registry.",
    category: "blockchain-general",
    aliases: ["Central Securities Depository", "CSD"],
    related: ["clearing-settlement", "blockchain", "spl-token"]
  },
  {
    ...
    category: "blockchain-general",
    related: ["clearing-settlement", "smart-contract-general", "dvp"]
  },
  {
    id: "omnibus-account",
  ...
  },
  {
    ...
    category: "blockchain-general",
    related: ["scalability", "consensus", "layer-1"]
  },
  {
    id: "liveness-safety",
    term: "Liveness and Safety",
    definition: "Two fundamental guarantees in distributed systems. Safety means the system never produces incorrect results (no double-spends, no conflicting blocks). Liveness means the system continues making progress (transactions eventually confirm). BFT systems like Solana's Tower BFT prioritize safety \u2014 the network halts rather than producing conflicting state.",
    category: "blockchain-general",
    aliases: ["Liveness", "Safety"],
    related: ["byzantine-fault-tolerance", "consensus", "finality-general"]
  },
  {
    id: "sharding",
    term: "Sharding",
    definition: "Scaling technique that partitions blockchain state and processing across multiple parallel chains (shards), each handling a subset of transactions. Used by Ethereum's roadmap (danksharding) and Near Protocol. Solana takes a different approach, using parallel execution on a single shard (via Sealevel) rather than splitting into multiple chains.",
    category: "blockchain-general",
    related: ["scalability", "parallel-execution", "layer-1"]
  },
  {
    id: "state-channel",
    term: "State Channel",
    ...
    category: "blockchain-general",
    related: ["layer-2", "lightning-network", "scalability"]
  },
  {
    ...
    category: "blockchain-general",
    related: ["validator", "staking-rewards", "inflation"]
  }
];

// data/terms/web3.json
var web3_default = [
  {
    id: "web3",
    term: "Web3",
  ...
  },
  {
    ...
    related: ["keypair", "wallet-general"]
  },
  {
    id: "defi-general",
  ...
  },
  {
    ...
    category: "web3",
    related: ["wallet-general", "seed-phrase"]
  },
  {
    id: "dex-general",
    term: "DEX (General Concept)",
    definition: "A decentralized exchange allowing peer-to-peer token trading without a centralized order book or custodian. Users trade directly from their wallets. DEXs use automated market makers (AMMs) or on-chain order books. Advantages: no KYC, self-custody, censorship-resistant. Disadvantages: potential for higher slippage, smart contract risk, and MEV extraction.",
    category: "web3",
    related: ["dex", "amm", "cex"]
  },
  {
  ...
  },
  {
    ...
    category: "web3",
    aliases: ["MEV"],
    related: ["mev", "sandwich-attack", "arbitrage"]
  },
  {
    id: "token-standard",
    term: "Token Standard",
    definition: "A specification defining how tokens are created and managed on a blockchain. Ethereum: ERC-20 (fungible), ERC-721 (NFT), ERC-1155 (multi-token). Solana: SPL Token (fungible/NFT), Token-2022 (extended features), Metaplex standards (NFT metadata). Standards ensure wallet/marketplace interoperability and composability across the ecosystem.",
    category: "web3",
    related: ["spl-token", "token-2022", "nft"]
  },
  {
    id: "composability",
    term: "Composability",
    definition: "The ability of blockchain protocols to interact with and build upon each other like building blocks ('money legos'). A lending protocol can accept LP tokens as collateral, which were minted by a DEX, which uses an oracle for pricing\u2014all in a single transaction. Solana's CPI mechanism and shared account model make composability a core design principle.",
    category: "web3",
    related: ["cpi", "defi-general"]
  },
  {
    id: "permissionless",
    term: "Permissionless",
  ...
  },
  {
    ...
    category: "web3",
    related: ["smart-contract-general", "consensus"]
  },
  {
    id: "rpc-general",
    term: "RPC (Remote Procedure Call)",
  ...
  },
  {
    ...
    category: "web3",
    related: ["testnet-devnet", "local-development"]
  },
  {
    id: "wormhole",
    term: "Wormhole",
    definition: "The primary cross-chain messaging and bridge protocol for Solana, enabling asset transfers between Solana, Ethereum, and 20+ other chains. Wormhole uses a network of 19 Guardian nodes that observe and attest to cross-chain messages. The protocol was exploited for $320M in Feb 2022 (patched). It supports token transfers, NFT bridging, and arbitrary message passing.",
    category: "web3",
    related: ["bridge", "interoperability"]
  },
  {
  ...
  },
  {
    ...
    category: "web3",
    related: ["socialfi", "decentralized-identity"]
  },
  {
    id: "apy",
  ...
  },
  {
    ...
    related: ["cex", "on-ramp-off-ramp", "permissionless"]
  },
  {
  ...
  },
  {
    id: "token-approval",
    term: "Token Approval",
    definition: "The permission granted to a program or delegate address to spend a specified amount of tokens from your account on your behalf. Token approvals are necessary for DeFi interactions like swapping or depositing into liquidity pools. On Solana, the SPL Token program's approve instruction sets a delegate and approved amount on a token account. Approvals should be set to the minimum needed and revoked when no longer required to limit exposure to potential exploits.",
    category: "web3",
    related: ["revoke-approval", "spl-token", "authority"]
  },
  {
    id: "cold-storage",
    term: "Cold Storage",
    ...
    category: "web3",
    aliases: ["Cold Wallet"],
    related: ["self-custody", "hot-wallet", "wallet-general"]
  },
  {
    id: "hot-wallet",
    term: "Hot Wallet",
    ...
    category: "web3",
    related: ["cold-storage", "wallet-general", "self-custody"]
  },
  {
    id: "hodl",
    term: "HODL",
  ...
  },
  {
    id: "rekt",
    term: "Rekt",
    definition: "Derived from 'wrecked.' Describes suffering a large financial loss from a liquidated leveraged trade, rug pull, exploit, or holding through a severe price decline. Used both seriously to describe genuine devastating losses and humorously as self-deprecating commentary. 'Getting rekt' is considered an almost inevitable rite of passage in crypto trading.",
    category: "web3",
    related: ["rug-pull", "degen"]
  },
  {
    id: "ngmi",
    term: "NGMI",
    definition: "Acronym for 'Not Gonna Make It.' Used to mock someone for a bad decision, weak conviction, or selling at the wrong time. Carries social pressure to hold through volatility and stay committed to the crypto space. Can be directed at others or used self-deprecatingly. The counterpart to WAGMI, NGMI implies the target has disqualified themselves from future gains.",
    category: "web3",
    aliases: ["NGMI"],
    related: ["wagmi", "fud", "paper-hands"]
  },
  {
    id: "wagmi",
    term: "WAGMI",
    definition: "Acronym for 'We're All Gonna Make It.' An expression of collective optimism and community solidarity used during bull markets, protocol milestones, and positive announcements. WAGMI reinforces group cohesion and shared belief in long-term success. The counterpart to NGMI, it originated from bodybuilding culture and was adopted by crypto communities during the 2021 bull cycle.",
    category: "web3",
    aliases: ["WAGMI"],
    related: ["ngmi", "fomo", "hodl"]
  },
  {
    id: "probably-nothing",
    term: "Probably Nothing",
    definition: "A heavily ironic phrase meaning the exact opposite \u2014 that something is very significant. Used when sharing a bullish signal, major partnership announcement, or notable on-chain activity that the speaker believes is underappreciated by the broader market. The sarcastic framing invites engagement and implies the audience should pay close attention.",
    category: "web3",
    related: ["alpha-info", "ct"]
  },
  {
    id: "aping-in",
    term: "Aping In",
    ...
    category: "web3",
    aliases: ["Ape In"],
    related: ["fomo", "degen", "dyor"]
  },
  {
    id: "bagholder",
    term: "Bagholder",
    definition: "Someone stuck holding a depreciating asset, either refusing to sell at a loss or having missed their exit opportunity. A 'bag' is a position in a specific token, especially one that has declined significantly from the purchase price. Bagholders often rationalize holding by citing long-term potential, but the term is generally pejorative and implies the position is unlikely to recover.",
    category: "web3",
    related: ["rekt", "diamond-hands", "exit-liquidity"]
  },
  {
    id: "diamond-hands",
    term: "Diamond Hands",
    definition: "Holding a position through extreme price volatility without selling, implying conviction and psychological resilience. The diamond emoji became shorthand during the 2021 bull cycle across crypto and meme stock communities. Diamond hands is celebrated when the asset eventually recovers or appreciates, but can also lead to significant losses if conviction is misplaced. Counterpart to paper hands.",
    category: "web3",
    related: ["paper-hands", "hodl", "wagmi"]
  },
  {
    id: "paper-hands",
    term: "Paper Hands",
    definition: "Selling a position too early, especially during a temporary price dip, out of fear or impatience. A pejorative counterpart to diamond hands, implying weakness and lack of conviction. In NFT contexts, paper hands means listing or selling during a floor price dip rather than holding through the downturn. The term carries social stigma in communities that value long-term holding.",
    category: "web3",
    related: ["diamond-hands", "jeet", "ngmi"]
  },
  {
    id: "ct",
    term: "CT (Crypto Twitter)",
    definition: "Short for Crypto Twitter \u2014 the community of traders, developers, influencers, and enthusiasts on Twitter/X discussing crypto markets, protocol developments, and culture. CT has its own norms, prominent voices, and meta-narratives that can drive market sentiment and token prices. Being 'active on CT' often means participating in these discussions to stay informed and build reputation.",
    category: "web3",
    aliases: ["Crypto Twitter"],
    related: ["alpha-info", "fomo"]
  },
  {
    id: "alpha-info",
    term: "Alpha",
    definition: "Exclusive, actionable, or early information that others do not yet have. 'Dropping alpha' means sharing a token tip, trade setup, or insider insight before it becomes widely known. The term borrows from traditional finance where alpha represents excess returns above a benchmark. In crypto, alpha can refer to early knowledge of token launches, protocol updates, or whale movements.",
    category: "web3",
    related: ["ct", "dyor"]
  },
  {
  ...
  },
  {
    id: "sol-maxi",
    term: "SOL Maxi",
    definition: "Someone exclusively bullish on Solana and dismissive of other blockchains, believing Solana will dominate the smart contract platform market. Maxi (short for maximalist) is a label applied across crypto tribalism \u2014 Bitcoin maxis, ETH maxis, SOL maxis. Used both as proud self-description and as mild criticism implying closed-mindedness to other ecosystems.",
    category: "web3",
    aliases: ["Solana Maxi"],
    related: ["opos", "solana-summer"]
  },
  {
    id: "solana-summer",
    term: "Solana Summer",
    definition: "Reference to mid-to-late 2021 when Solana experienced explosive growth in TVL, NFT launches, developer adoption, and SOL price appreciation. Invoked both nostalgically to recall that period of rapid ecosystem expansion and prospectively when anticipating the next major Solana bull cycle. The phrase echoes 'DeFi Summer' (Ethereum's 2020 growth period).",
    category: "web3",
    related: ["sol-maxi", "opos", "tvl"]
  },
  {
    id: "opos",
    term: "OPOS",
    definition: "Acronym for 'Only Possible on Solana.' A community phrase highlighting features or applications that cannot be replicated on other chains due to their technical limitations. Used to describe high-frequency DeFi applications, massive concurrent NFT drops, sub-second settlement, and other use cases that leverage Solana's unique throughput and low fees.",
    category: "web3",
    aliases: ["Only Possible on Solana", "OPOS"],
    related: ["sol-maxi", "solana-summer"]
  },
  {
    id: "lfg",
    term: "LFG",
    definition: "Acronym for 'Let's F***ing Go.' An expression of intense excitement about a launch, price movement, milestone, or announcement. Widely used across crypto communities to rally enthusiasm. The term is so embedded in Solana culture that the Colosseum hackathon platform previously used LFG as a name, reflecting the community's high-energy ethos.",
    category: "web3",
    aliases: ["LFG"],
    related: ["wagmi", "fomo"]
  },
  ...
  },
  {
    ...
    category: "web3",
    aliases: ["Snipe"],
    related: ["trading-bot", "mev", "front-running"]
  },
  {
    id: "trading-bot",
    term: "Trading Bot",
  ...
  },
  {
    ...
    category: "web3",
    related: ["nft-general", "dex-general"]
  },
  {
    id: "pump-and-dump",
    term: "Pump and Dump",
    definition: "Market manipulation scheme where promoters accumulate a low-cap token, artificially inflate its price through coordinated hype (social media shilling, fake partnerships), then sell their holdings at the peak, crashing the price. Especially prevalent with memecoins on Solana due to low launch costs on platforms like pump.fun.",
    category: "web3",
    related: ["rug-pull", "dyor"]
  },
  {
    id: "shilling",
    term: "Shilling",
    definition: "Aggressively promoting a cryptocurrency or NFT project, often with undisclosed financial interest. Shillers may be paid promoters, project insiders, or holders trying to inflate prices. Common on Crypto Twitter (CT) and Discord, often paired with pump-and-dump schemes. The term carries negative connotations in crypto culture.",
    category: "web3",
    related: ["ct", "dyor", "fud"]
  },
  {
    id: "memecoin",
    term: "Memecoin",
    definition: "A cryptocurrency created around internet memes, jokes, or cultural themes rather than technical utility. On Solana, memecoins like BONK and WIF have generated significant trading volume. Platforms like pump.fun enable anyone to launch memecoins in minutes, making Solana the leading chain for memecoin activity since late 2023.",
    category: "web3",
    related: ["degen", "pump-fun", "bonk"]
  },
  {
    id: "gas-wars",
    term: "Gas Wars",
  ...
  },
  {
    ...
    category: "web3",
    related: ["squads", "multisig", "dao"]
  },
  {
    id: "hardware-wallet",
    term: "Hardware Wallet",
    definition: "A physical device (Ledger, Trezor) that stores private keys offline in a secure element chip, signing transactions without exposing keys to internet-connected computers. Hardware wallets protect against malware, phishing, and remote hacks. Solana hardware wallet support is available through Phantom, Solflare, and other wallet adapters.",
    category: "web3",
    related: ["wallet-general", "cold-storage", "self-custody"]
  },
  {
    id: "copy-trading",
    term: "Copy Trading",
    definition: "Strategy where users automatically replicate the on-chain trades of other wallets in real time. On Solana, copy trading bots monitor successful trader wallets and mirror their swaps, often using Jupiter or Raydium. While popular, it carries risks including front-running by the copied wallet and delayed execution.",
    category: "web3",
    related: ["trading-bot", "dex-general", "jupiter"]
  },
  {
...
var programming_fundamentals_default = [
  ...
  },
  {
    ...
    category: "programming-fundamentals",
    related: ["solana-web3js", "javascript"]
  },
  {
    id: "javascript",
    term: "JavaScript",
    definition: "The ubiquitous scripting language for web development, running in browsers and Node.js. JavaScript is dynamically typed and event-driven. Most Solana dApp frontends and scripts use JavaScript/TypeScript with libraries like @solana/web3.js. Node.js enables server-side JS for backend services, indexers, and bot development.",
    category: "programming-fundamentals",
  ...
  },
  {
    ...
    category: "programming-fundamentals",
    aliases: ["CLI"],
    related: ["solana-cli", "anchor-init"]
  },
  {
    id: "base58",
    term: "Base58 / Base58Check",
  ...
  },
  {
    ...
    category: "programming-fundamentals",
    aliases: ["CI/CD"],
    related: ["testing", "solana-verify"]
  },
  {
    id: "open-source",
    term: "Open Source",
    definition: "Software with publicly available source code that anyone can inspect, modify, and distribute. Most Solana infrastructure is open-source: the Agave validator (Rust, Apache 2.0), Anchor framework, SPL programs, and major DeFi protocols. Open-source enables community auditing, composability, and trust verification. Licenses (MIT, Apache 2.0, GPL) define usage terms.",
    category: "programming-fundamentals",
    related: ["solana-verify", "audit"]
  },
  {
    id: "git",
    term: "Git",
  ...
  },
  {
    ...
    category: "programming-fundamentals",
    related: ["borsh", "serialization", "account-data"]
  },
  {
    id: "elf-binary",
    term: "ELF Binary",
    ...
    related: ["sbf", "cargo-build-sbf", "program-data"]
  },
  {
    id: "fixed-point-arithmetic",
    term: "Fixed-Point Arithmetic",
  ...
  },
  {
    ...
    category: "programming-fundamentals",
    related: ["anchor-toml", "ci-cd", "git"]
  },
  {
    id: "deterministic-address-derivation",
    term: "Deterministic Address Derivation",
    ...
    category: "programming-fundamentals",
    related: ["pda", "find-program-address", "seeds"]
  },
  {
    id: "pipelining",
    term: "Pipelining",
    definition: "Processing technique where multiple stages of a task execute in overlapping fashion, like an assembly line. Solana's validator pipeline processes transactions through fetch, SigVerify, banking, and broadcast stages simultaneously, achieving higher throughput than sequential processing. Essential to Solana's 400ms slot production.",
    category: "programming-fundamentals",
    related: ["tpu", "tvu", "parallel-execution"]
  },
  {
    id: "cap-theorem",
    term: "CAP Theorem",
    ...
    related: ["commitment-levels", "consensus", "finality"]
  },
  {
    id: "lamport-timestamp",
    term: "Lamport Timestamp",
    definition: "Logical clock mechanism proposed by Leslie Lamport (1978) for ordering events in distributed systems without synchronized physical clocks. Solana's Proof of History extends this concept by using a SHA-256 hash chain as a verifiable, cryptographic logical clock, creating a global ordering of events without requiring validators to communicate timestamps.",
    category: "programming-fundamentals",
    aliases: ["Logical Clock"],
    related: ["proof-of-history", "sha256-hash-chain", "consensus"]
  },
  {
    id: "state-machine-replication",
    term: "State Machine Replication",
    definition: "Distributed computing pattern where identical copies of a state machine are maintained across multiple nodes. Each node processes the same sequence of inputs deterministically, producing identical outputs. Solana validators are state machines: given the same ledger of transactions, every validator produces the same account state.",
    category: "programming-fundamentals",
    aliases: ["SMR"],
    related: ["validator", "deterministic-execution", "replay-stage"]
  },
  {
    id: "pbft",
    term: "PBFT",
    definition: "Practical Byzantine Fault Tolerance. Classical BFT consensus algorithm (Castro & Liskov, 1999) tolerating up to f faulty nodes in 3f+1 total, requiring O(n^2) message complexity per round. Solana's Tower BFT reduces this to O(n) by using Proof of History as a clock, replacing round-based message exchanges with time-based vote lockouts.",
    category: "programming-fundamentals",
    aliases: ["Practical Byzantine Fault Tolerance", "PBFT"],
    related: ["tower-bft", "byzantine-fault-tolerance", "consensus"]
  },
  {
    id: "amdahls-law",
    term: "Amdahl's Law",
    definition: "Theoretical limit on parallel speedup: if P% of a task is parallelizable, maximum speedup is 1/(1-P + P/N) for N processors. Limits Sealevel's gains \u2014 transactions touching the same account must serialize. Solana mitigates this through account-level locking (maximizing P) and local fee markets (pricing serial contention).",
    category: "programming-fundamentals",
    aliases: ["Amdahl's Law"],
    related: ["parallel-execution", "sealevel", "account-locking"]
  },
  {
    id: "memory-mapped-io",
    ...
    category: "programming-fundamentals",
    aliases: ["mmap"],
    related: ["accounts-db", "append-vec", "heap-memory"]
  },
  {
    id: "copy-on-write",
    term: "Copy-on-Write",
    ...
    category: "programming-fundamentals",
    aliases: ["CoW"],
    related: ["accounts-db", "append-vec"]
  },
  {
    id: "bloom-filter",
    term: "Bloom Filter",
  ...
  },
  {
    ...
    category: "programming-fundamentals",
    aliases: ["AES-GCM"],
    related: ["confidential-transfers", "encryption", "zk-proofs"]
  },
  {
    id: "ecdh",
  ...
  },
  {
    ...
    category: "programming-fundamentals",
    related: ["concurrency", "parallel-execution", "account-locking"]
  },
  {
    id: "eventual-consistency",
    term: "Eventual Consistency",
    definition: "Distributed systems model where replicas may temporarily diverge but converge to the same state given sufficient time without new updates. Contrasts with strong consistency where reads always return the latest write. Solana RPC nodes may exhibit eventual consistency \u2014 different nodes may return different slot data briefly before converging.",
    category: "programming-fundamentals",
    related: ["cap-theorem", "concurrency", "rpc"]
  },
  {
    id: "io-uring",
    term: "io_uring",
    definition: "Linux kernel interface for asynchronous I/O operations that eliminates system call overhead through shared memory ring buffers between user and kernel space. Used in high-performance Solana validator implementations (notably Firedancer) to achieve maximum disk and network throughput with minimal CPU overhead for I/O-bound operations.",
    category: "programming-fundamentals",
    related: ["kernel-bypass", "firedancer", "pipelining"]
  },
  {
    id: "avx-512-simd",
    term: "AVX-512 (SIMD Instructions)",
    definition: "Intel's 512-bit Advanced Vector Extensions enabling single-instruction-multiple-data parallel processing. Firedancer leverages AVX-512 for high-throughput signature verification, hashing, and data processing, executing operations on 16 32-bit values simultaneously. This hardware-level parallelism is key to achieving millions of signature verifications per second.",
    category: "programming-fundamentals",
    aliases: ["AVX-512"],
    related: ["firedancer", "pipelining", "ed25519"]
  },
  {
    id: "crash-fault-vs-byzantine",
    term: "Crash Fault vs Byzantine Fault",
    ...
    category: "programming-fundamentals",
    related: ["byzantine-fault-tolerance", "consensus", "tower-bft"]
  }
];

// data/terms/ai-ml.json
var ai_ml_default = [
  {
    id: "llm",
  ...
  },
  {
    ...
    category: "ai-ml",
    related: ["llm", "attention-mechanism"]
  },
  {
    id: "attention-mechanism",
    term: "Attention Mechanism",
    definition: "A neural network component that allows models to weigh the relevance of different parts of the input when producing output. Self-attention computes query-key-value dot products across all positions, enabling each token to 'attend' to every other token. Multi-head attention runs multiple attention functions in parallel. Attention is O(n\xB2) in sequence length, driving context window research.",
    category: "ai-ml",
  ...
  },
  {
    ...
    category: "ai-ml",
    aliases: ["PEFT", "LoRA"],
    related: ["foundation-model", "llm"]
  },
  {
    id: "rag",
    term: "RAG (Retrieval-Augmented Generation)",
    definition: "An AI architecture that combines LLMs with external knowledge retrieval. Instead of relying solely on training data, RAG systems retrieve relevant documents from a knowledge base (using embeddings and vector search), then provide them as context to the LLM. RAG reduces hallucinations and enables up-to-date responses. Useful for blockchain documentation bots and developer assistants.",
    category: "ai-ml",
    aliases: ["RAG"],
  ...
  },
  {
    ...
    definition: "AI models with publicly released weights that can be downloaded, modified, and self-hosted. Notable open models: Llama 3 (Meta), Mistral, Falcon, Gemma (Google), Phi (Microsoft). Open models enable privacy (data stays local), customization (fine-tuning), and cost control. Trade-off: generally less capable than frontier proprietary models but rapidly improving.",
    category: "ai-ml",
    aliases: ["Open-Weight Models"],
    related: ["foundation-model", "inference"]
  },
  {
    id: "multimodal",
    term: "Multimodal AI",
    definition: "AI models that can process and generate multiple data types: text, images, audio, video, and code. Modern multimodal models (GPT-4V, Claude, Gemini) can analyze screenshots of dApp UIs, read code from images, generate diagrams, and understand charts. In blockchain development, multimodal capabilities help analyze transaction visualizations, audit UI screenshots, and process documentation with images.",
    category: "ai-ml",
  ...
  },
  {
    ...
    category: "ai-ml",
    related: ["depin", "training", "inference"]
  },
  {
    id: "ai-blockchain-integration",
    term: "AI \xD7 Blockchain Integration",
  ...
  },
  {
    ...
    category: "ai-ml",
    aliases: ["MCP"],
    related: ["agent-ai", "tool-use", "langchain"]
  },
  {
    id: "langchain",
    term: "LangChain / LangGraph",
  ...
  },
  {
    ...
    related: ["rlhf", "fine-tuning", "training"]
  },
  {
    id: "synthetic-data",
    term: "Synthetic Data (AI Training)",
  ...
  },
  {
    ...
    related: ["inference", "open-source-ai", "distillation"]
  },
  {
    id: "diffusion-model",
    term: "Diffusion Model",
    definition: "A generative AI architecture that creates images, video, or audio by learning to reverse a noise-adding process\u2014starting from pure noise and iteratively denoising to produce coherent output. Diffusion models power leading image generators (Stable Diffusion, DALL-E 3, Midjourney) and video generators (Sora). Key variants include latent diffusion (operating in compressed space) and diffusion transformers (DiT).",
    category: "ai-ml",
    aliases: ["Latent Diffusion", "DiT"],
    related: ["multimodal", "stable-diffusion"]
  },
  {
    id: "stable-diffusion",
    ...
    category: "ai-ml",
    aliases: ["SD", "SDXL"],
    related: ["diffusion-model", "open-source-ai"]
  },
  {
    id: "ai-alignment",
    term: "AI Alignment",
  ...
  },
  {
    ...
    related: ["transformer", "state-space-model", "llm"]
  },
  {
    id: "state-space-model",
    term: "State Space Model (Mamba)",
    definition: "An alternative to the Transformer architecture that processes sequences with linear O(n) complexity instead of quadratic O(n^2) attention, enabling efficient handling of very long sequences. Mamba introduced selective state spaces where the model dynamically filters information based on content. Hybrid architectures like Jamba combine SSM efficiency with attention's retrieval strength.",
    category: "ai-ml",
    aliases: ["SSM", "Mamba"],
    related: ["transformer", "mixture-of-experts", "attention-mechanism"]
  },
  {
    id: "bittensor",
    term: "Bittensor (TAO)",
    definition: "A blockchain protocol that decentralizes AI by creating an open marketplace for machine intelligence. Built on Substrate with a Yuma consensus mechanism, Bittensor rewards contributors with TAO tokens based on the quality of their AI outputs. The network hosts 100+ specialized subnets for different AI tasks. TAO supply is capped at 21M with Bitcoin-style halvings.",
    category: "ai-ml",
    aliases: ["TAO"],
    related: ["depin", "gpu-compute", "decentralized-inference"]
  },
  {
    id: "decentralized-inference",
    term: "Decentralized Inference",
    definition: "Running AI model inference across distributed networks of GPU providers rather than centralized cloud infrastructure, using blockchain for coordination, payment, and verification. Key verification approaches include ZKML (zero-knowledge proofs of correct inference) and trusted execution environments (TEEs). Projects include Bittensor, Render Network, and io.net on Solana.",
    category: "ai-ml",
    aliases: ["Proof of Inference", "ZKML"],
    related: ["bittensor", "gpu-compute", "on-chain-ai"]
  },
  {
    id: "deepseek",
    term: "DeepSeek",
    definition: "A Chinese AI lab that released DeepSeek-R1 in January 2025, a 671B-parameter open-weight reasoning model achieving performance comparable to OpenAI's o1 at significantly lower cost. DeepSeek-R1 generates visible chain-of-thought reasoning using GRPO training and demonstrated that pure RL with verifiable rewards can produce emergent reasoning. DeepSeek-V3 uses a MoE architecture with ~37B active parameters.",
    category: "ai-ml",
    aliases: ["DeepSeek-R1", "DeepSeek-V3"],
    related: ["reasoning-model", "mixture-of-experts", "open-source-ai"]
  },
  {
    id: "github-copilot",
    term: "GitHub Copilot",
...
var solana_ecosystem_default = [
  {
    ...
    related: ["wallet-general", "solflare", "wallet-adapter"]
  },
  {
    id: "solflare",
    term: "Solflare",
    definition: "A Solana-native wallet offering browser extension, mobile, and web interfaces with deep staking integration, hardware wallet support (Ledger), and a built-in dApp browser. Solflare was one of the first Solana wallets and provides advanced features like stake account management, token metadata display, and priority fee controls. It supports the Solana Wallet Standard for seamless dApp connectivity.",
    category: "solana-ecosystem",
    related: ["wallet-general", "phantom", "wallet-adapter"]
  },
  {
    id: "backpack-wallet",
    term: "Backpack Wallet",
    definition: "A multi-chain wallet developed by Coral (the team behind xNFT protocol) that combines MPC and self-custody models. Backpack pioneered executable NFTs (xNFTs) that embed full applications inside NFTs, enabling a decentralized app store experience within the wallet. It supports Solana, Ethereum, and other chains with a focus on developer extensibility and programmable wallet experiences.",
    category: "solana-ecosystem",
    related: ["wallet-general", "phantom", "mad-lads"]
  },
  {
    id: "backpack-exchange",
    term: "Backpack Exchange",
    definition: "A regulated cryptocurrency exchange built by the same team behind Backpack Wallet (Coral). Licensed in multiple jurisdictions, it offers spot and perpetual futures trading with a focus on the Solana ecosystem. Backpack Exchange integrates tightly with the Backpack Wallet for seamless fund transfers and aims to bridge the gap between centralized exchange liquidity and self-custodial wallet experiences.",
    category: "solana-ecosystem",
    related: ["backpack-wallet", "cex", "dex"]
  },
  {
    id: "helium",
    term: "Helium",
    definition: "A decentralized wireless infrastructure network that migrated from its own L1 blockchain to Solana in April 2023 for improved scalability and composability. Helium incentivizes individuals to deploy hotspots providing LoRaWAN (IoT) and 5G cellular coverage, earning HNT, MOBILE, and IOT tokens. With over 900,000 hotspots deployed globally, Helium is one of the largest DePIN projects on Solana.",
    category: "solana-ecosystem",
    related: ["depin", "sol", "compressed-nft"]
  },
  {
    id: "render-network",
    term: "Render Network",
    definition: "A decentralized GPU rendering network that migrated to Solana in November 2023 from Ethereum for lower fees and higher throughput. Render connects artists and studios needing GPU power for 3D rendering, motion graphics, and AI workloads with node operators who supply idle GPUs. Contributors earn RENDER tokens for completed jobs. The migration leveraged Solana's state compression for efficient job tracking.",
    category: "solana-ecosystem",
    related: ["depin", "gpu-compute", "sol"]
  },
  {
    id: "hivemapper",
    term: "Hivemapper",
    definition: "A decentralized mapping network on Solana where contributors install dashcams to collect street-level imagery while driving, building an up-to-date global map. Contributors earn HONEY tokens for mapping coverage, with rewards weighted by map freshness and geographic demand. Hivemapper has mapped millions of kilometers of roads and sells map data to enterprises as an alternative to Google Maps.",
    category: "solana-ecosystem",
    related: ["depin", "sol", "helium"]
  },
  {
    id: "bonk",
    term: "BONK",
    definition: "Solana's first community-created dog-themed memecoin, launched in December 2022 during the post-FTX bear market to revitalize ecosystem morale. BONK was airdropped to Solana developers, NFT communities, and DeFi users. It grew into one of the largest memecoins by market cap and became a cultural symbol of Solana's community resilience, with integrations across major Solana DEXes and lending protocols.",
    category: "solana-ecosystem",
    related: ["airdrop-crypto", "sol", "dex"]
  },
  {
    id: "moonshot",
    term: "Moonshot",
    definition: "A mobile-first token launch platform on Solana that simplifies purchasing newly launched tokens using credit cards, Apple Pay, or other fiat payment methods. Moonshot lowers the barrier to entry for retail users who may not be familiar with DEX interfaces or wallet management, enabling direct participation in Solana token launches from a smartphone app.",
    category: "solana-ecosystem",
    related: ["pump-fun", "dex", "sol"]
  },
  {
    id: "colosseum",
    term: "Colosseum",
    definition: "The organization behind Solana's major hackathons and accelerator programs. Colosseum runs online hackathons attracting thousands of developers building on Solana, with prize pools and follow-on funding for winning projects. Its accelerator provides mentorship, funding, and ecosystem connections to promising teams. Many successful Solana projects originated from Colosseum hackathons.",
    category: "solana-ecosystem",
    related: ["solana-foundation", "superteam", "solana-breakpoint"]
  },
  {
    id: "superteam",
    term: "Superteam",
    definition: "A community-driven DAO supporting the Solana ecosystem through regional chapters across 15+ countries. Superteam members earn reputation and rewards by completing bounties, creating content, organizing events, and onboarding developers to Solana. It serves as a decentralized talent and community network that connects builders, creators, and business developers with ecosystem opportunities.",
    category: "solana-ecosystem",
    related: ["solana-foundation", "dao", "colosseum"]
  },
  {
    id: "solana-foundation",
    term: "Solana Foundation",
    definition: "A non-profit organization based in Zug, Switzerland, dedicated to the growth, security, and decentralization of the Solana network. The Foundation provides grants to developers, funds validator operations, organizes Breakpoint conferences, supports ecosystem tooling, and manages delegation programs that distribute stake to performant validators. It operates independently from Solana Labs and Anza.",
    category: "solana-ecosystem",
    related: ["sol", "validator", "solana-breakpoint"]
  },
  {
    id: "neon-evm",
    term: "Neon EVM",
    ...
    category: "solana-ecosystem",
    related: ["evm", "solidity", "svm"]
  },
  {
    id: "debridge",
    term: "deBridge",
    definition: "A cross-chain interoperability protocol supporting high-performance bridging between Solana, Ethereum, and other major chains. deBridge uses a network of independent validators to verify cross-chain messages and enables asset transfers, cross-chain swaps, and arbitrary message passing. Its DLN (Decentralized Liquidity Network) facilitates zero-slippage cross-chain trades through a maker-taker model.",
    category: "solana-ecosystem",
    related: ["bridge", "interoperability", "wormhole"]
  },
  {
    id: "allbridge",
    term: "Allbridge",
    definition: "A multi-chain bridge protocol supporting asset transfers between Solana, Ethereum, BNB Chain, and other networks. Allbridge Core uses a liquidity pool model where users swap assets through chain-specific pools connected by a messaging layer, reducing the need for wrapped tokens. It supports both native stablecoin bridging and general token transfers across supported chains.",
    category: "solana-ecosystem",
    related: ["bridge", "interoperability", "wormhole"]
  },
  {
    ...
    category: "solana-ecosystem",
    related: ["dao", "governance-token", "spl"]
  },
  {
    id: "solana-breakpoint",
    term: "Solana Breakpoint",
    definition: "Solana's flagship annual conference organized by the Solana Foundation, bringing together developers, founders, validators, and community members for keynotes, workshops, and networking. Breakpoint has been held in Lisbon (2022), Amsterdam (2023), and Singapore (2024), featuring major ecosystem announcements, product launches, and technical deep dives. It serves as the primary gathering for the global Solana community.",
    category: "solana-ecosystem",
    related: ["solana-foundation", "colosseum", "hacker-house"]
  },
  {
    id: "hacker-house",
    term: "Hacker House",
    definition: "Developer-focused co-working and event spaces organized by the Solana Foundation, ecosystem teams, and community groups where builders collaborate on Solana projects in an intensive, in-person setting. Hacker Houses are typically held during crypto conferences or as standalone multi-day events, providing workspace, mentorship, and networking opportunities for Solana developers worldwide.",
    category: "solana-ecosystem",
    related: ["solana-breakpoint", "colosseum", "solana-foundation"]
  },
  {
    id: "star-atlas",
    term: "Star Atlas",
    ...
    category: "solana-ecosystem",
    related: ["nft", "sol", "dapp"]
  },
  {
    id: "degods",
    term: "DeGods",
    definition: "A blue-chip NFT collection that launched on Solana in October 2021 and became one of the ecosystem's most culturally significant projects. Created by Frank (Rohun Vora), DeGods features 10,000 generative art pieces with a deflationary mechanism. The collection briefly migrated to Ethereum and Bitcoin before returning to Solana, reflecting broader multi-chain NFT trends.",
    category: "solana-ecosystem",
    related: ["nft", "magic-eden", "sol"]
  },
  {
    id: "mad-lads",
    term: "Mad Lads",
    definition: "An NFT collection of 10,000 generative characters created by the Backpack/Coral team, launched in April 2023 as the first major xNFT collection. Mad Lads are executable NFTs that can embed applications directly within the token, pioneering a new paradigm for NFT utility beyond static media. The mint was one of the most anticipated Solana NFT launches and sold out rapidly.",
    category: "solana-ecosystem",
    related: ["nft", "backpack-wallet", "magic-eden"]
  },
  {
    id: "claynosaurz",
    term: "Claynosaurz",
    definition: "A popular Solana NFT collection featuring animated claymation-style dinosaur characters. Known for high-quality 3D art and strong community engagement, Claynosaurz has expanded into gaming, merchandise, and brand collaborations. The collection demonstrated that Solana NFT projects could build lasting brands and IP beyond profile pictures, with a focus on entertainment and content creation.",
    category: "solana-ecosystem",
    related: ["nft", "magic-eden", "sol"]
  },
  {
    ...
    category: "solana-ecosystem",
    related: ["compressed-nft", "zk-compression", "helius"]
  },
  {
    id: "step-finance",
    term: "Step Finance",
    definition: "A portfolio dashboard and DeFi aggregator on Solana that provides users with a unified view of their token balances, LP positions, staking rewards, and DeFi activity across Solana protocols. Step Finance aggregates data from multiple DEXes, lending platforms, and yield farms into a single interface, enabling portfolio tracking and one-click position management.",
    category: "solana-ecosystem",
    related: ["defi-general", "sol", "jupiter"]
  },
  {
    id: "dialect",
    term: "Dialect",
    definition: "A messaging and notifications protocol built on Solana that enables wallet-to-wallet communication and smart messaging within dApps. Dialect powers on-chain notifications for DeFi events (liquidation warnings, governance votes, trade fills), token-gated group chats, and protocol announcements. It integrates with Solana wallets to deliver context-aware messages tied to on-chain activity.",
    category: "solana-ecosystem",
    related: ["dapp", "wallet-general", "sol"]
  },
  {
    id: "magic-eden",
    term: "Magic Eden",
    definition: "The leading NFT marketplace that launched on Solana in September 2021 and expanded to support Ethereum, Bitcoin Ordinals, Polygon, and other chains. Magic Eden pioneered features like collection launchpads, rarity tools, and cross-chain NFT trading. It processes the majority of Solana NFT volume and played a central role in the growth of the Solana NFT ecosystem.",
    category: "solana-ecosystem",
    related: ["nft", "tensor", "sol"]
  },
  {
    id: "marginfi",
    term: "marginfi",
    definition: "A decentralized lending and borrowing protocol on Solana that enables users to supply assets to earn yield or borrow against collateral. marginfi features a risk engine that evaluates asset correlations and concentration risk, isolated lending pools for higher-risk assets, and integration with Solana DeFi composability. It became one of the largest lending protocols on Solana by TVL during 2023-2024.",
    category: "solana-ecosystem",
    related: ["lending", "borrowing", "tvl"]
  },
  {
    id: "solend-save",
    term: "Solend / Save",
    definition: "One of the earliest lending and borrowing protocols on Solana, originally launched as Solend in 2021 and later rebranded to Save. The protocol enables users to deposit assets to earn interest and borrow against collateral with algorithmic interest rates. It supports isolated lending pools, permissionless pool creation, and has processed billions in cumulative lending volume on Solana.",
    category: "solana-ecosystem",
    aliases: ["Solend", "Save"],
    related: ["lending", "borrowing", "marginfi"]
  },
  {
    id: "lifinity",
    term: "Lifinity",
    definition: "A proactive market-making DEX on Solana that uses oracle-driven pricing (via Pyth) instead of relying solely on arbitrageurs to maintain accurate prices. By referencing external oracle prices, Lifinity reduces impermanent loss for liquidity providers and captures value that would otherwise go to arbitrage traders. It operates concentrated liquidity positions that automatically rebalance around the oracle price.",
    category: "solana-ecosystem",
    related: ["dex", "oracle", "pyth"]
  },
  {
    id: "solana-accelerate",
    term: "Solana Accelerate",
    definition: "Solana Foundation's product-focused conference series. The inaugural event was held in May 2025 in New York City with over 3,000 attendees, later expanding to a global format in 2026 with editions in APAC and Miami. Positioned around product showcases and founder talks, Solana Accelerate complements Breakpoint's developer focus by highlighting real-world applications and go-to-market strategies.",
    category: "solana-ecosystem",
    related: ["solana-breakpoint", "solana-foundation"]
  },
  {
    id: "catstanbul",
    term: "Catstanbul",
    definition: "Jupiter Exchange's annual community conference, first held in January 2025 in Istanbul. Catstanbul features live transparency audits, JUP token burning ceremonies, ecosystem strategy discussions, and community showcases. Named after Jupiter's cat mascot branding, the event serves as Jupiter's primary gathering for announcing roadmap updates and celebrating community milestones.",
    category: "solana-ecosystem",
    related: ["jupiter", "governance-token"]
  },
  {
    id: "met-dhabi",
    term: "Met-Dhabi",
    definition: "Meteora's inaugural community event held in December 2025 in Abu Dhabi, one day before Solana Breakpoint 2025. Met-Dhabi featured main stage presentations, interactive booths, and a community gathering tied to Meteora's MET token generation event. The conference highlighted Meteora's DLMM technology and growing role in Solana's liquidity infrastructure.",
    category: "solana-ecosystem",
    related: ["meteora", "solana-breakpoint"]
  },
  {
    id: "solana-crossroads",
    term: "Solana Crossroads",
    definition: "A community-organized annual Solana conference in Istanbul, led by the Step Finance team rather than the Solana Foundation. Crossroads attracted over 2,500 attendees in 2024, making it one of the largest independently run Solana events. It is one of the few major Solana conferences entirely organized by ecosystem contributors, featuring talks, panels, and side events.",
    category: "solana-ecosystem",
    related: ["step-finance", "solana-breakpoint", "solana-foundation"]
  },
  {
    id: "mtndao",
    term: "mtnDAO",
    definition: "A month-long Solana developer retreat held annually in Salt Lake City, Utah, described as Solana's most notorious hacker house. Developers rent shared coworking space and build together for a full month in an intensive, community-run, sponsor-backed environment. mtnDAO fosters deep collaboration and has produced notable projects and lasting professional relationships within the Solana ecosystem.",
    category: "solana-ecosystem",
    related: ["hacker-house", "colosseum"]
  },
  {
    id: "colosseum-hackathon",
    term: "Colosseum Hackathon",
    definition: "Colosseum's global online hackathon series, formerly run by the Solana Foundation. Two major hackathons are held per year, plus Colosseum Eternal, a perpetual rolling format. Notable past hackathons include Grizzlython, Hyperdrive, Renaissance, Radar, Breakout, and Cypherpunk, which drew over 9,000 participants and 1,576 project submissions. Many successful Solana protocols originated from these events.",
    category: "solana-ecosystem",
    related: ["colosseum", "solana-foundation", "hacker-house"]
  },
  {
    id: "solana-allstars",
    term: "Solana Allstars",
    definition: "A global grassroots ambassador program launched by Step Finance in 2023. Solana Allstars is a network of local country managers who host recurring small-scale IRL meetups of 50 to 100 people to onboard new Solana community members worldwide. The program focuses on education, community building, and expanding Solana's reach into underrepresented regions.",
    category: "solana-ecosystem",
    related: ["step-finance", "superteam", "solana-foundation"]
  },
  {
    id: "solana-summit",
    term: "Solana Summit",
    definition: "A free, community-funded annual conference focused on APAC and emerging markets. The 2025 editions were held in Da Nang, Vietnam and Abuja, Nigeria. Organized by Superteam chapters and ecosystem partners, Solana Summit brings together local developers, founders, and community members in regions outside major crypto hubs to foster growth and adoption.",
    category: "solana-ecosystem",
    related: ["superteam", "solana-foundation", "solana-breakpoint"]
  },
  {
    id: "ondo-finance",
    term: "Ondo Finance",
    ...
    category: "solana-ecosystem",
    related: ["defi-general", "staking-general"]
  },
  {
    id: "maple-finance",
    term: "Maple Finance",
    definition: "An institutional lending protocol connecting capital providers with vetted institutional borrowers through on-chain credit pools. Pool delegates underwrite loans and manage risk on behalf of depositors. Maple offers Solana pools for US Treasury yield and direct institutional lending, bridging traditional credit markets with DeFi infrastructure for higher-quality borrowers.",
    category: "solana-ecosystem",
    related: ["defi-general", "marginfi"]
  },
  {
    id: "credix",
    term: "Credix",
    definition: "A private credit DeFi protocol on Solana providing financing to fintech lenders in Latin America and Africa. Credix structures on-chain credit facilities backed by real-world receivables, connecting global DeFi liquidity to underserved emerging markets. The protocol enables institutional and accredited investors to earn yield from real economic activity in regions with limited access to traditional capital markets.",
    category: "solana-ecosystem",
    related: ["defi-general", "ondo-finance"]
  },
  {
    id: "parcl",
    term: "Parcl",
    definition: "A Solana-native DeFi protocol providing exposure to real estate price indices through perpetual-style markets. Parcl tracks price-per-square-foot in major US cities, allowing users to take long or short positions on residential real estate price movements without owning physical property. The protocol brings real estate market speculation on-chain with Solana's low fees and fast settlement.",
    category: "solana-ecosystem",
    related: ["defi-general", "drift"]
  },
  {
    id: "zeta-markets",
    term: "Zeta Markets",
    definition: "An options and perpetual futures DEX on Solana using a fully on-chain order book and risk engine. Zeta Markets enables undercollateralized trading with cross-margining, allowing traders to take leveraged positions on SOL and other assets. The protocol combines the capital efficiency of centralized exchanges with the transparency and self-custody of decentralized trading.",
    category: "solana-ecosystem",
    related: ["dex", "perpetuals", "drift"]
  },
  {
    id: "flash-trade",
    term: "Flash Trade",
    definition: "A perpetual trading protocol on Solana offering synthetic asset trading with a pool-based liquidity model (FLP). Flash Trade uses oracle-based pricing from Pyth to enable zero-slippage trades against a shared liquidity pool, where liquidity providers earn fees from trader activity. The protocol supports leveraged perpetual positions on crypto assets with fast execution on Solana.",
    category: "solana-ecosystem",
    related: ["perpetuals", "pyth", "drift"]
  },
  {
    id: "goosefx",
    term: "GooseFX",
    definition: "A DeFi suite on Solana offering a perpetual futures DEX, single-sided liquidity staking, and an NFT marketplace in one integrated platform. GooseFX's perpetual DEX uses an order book model while its single-sided staking allows users to provide liquidity with a single asset rather than token pairs, reducing impermanent loss exposure for liquidity providers.",
    category: "solana-ecosystem",
    aliases: ["GOFX"],
    related: ["dex", "perpetuals", "nft"]
  },
  {
    id: "mango-markets",
    term: "Mango Markets",
    ...
    category: "solana-ecosystem",
    aliases: ["Mango"],
    related: ["dex", "oracle", "defi-general"]
  },
  {
    id: "hubble-protocol",
    term: "Hubble Protocol",
    ...
    category: "solana-ecosystem",
    aliases: ["Hubble", "USDH"],
    related: ["stablecoin", "defi-general", "liquid-staking"]
  },
  {
    id: "tulip-protocol",
    term: "Tulip Protocol",
    definition: "The first yield aggregator on Solana, automating auto-compounding strategies for Raydium, Orca, and Saber LP positions. Launched in 2021, Tulip simplified yield farming by automatically reinvesting earned fees and rewards back into liquidity positions. The protocol operated during Solana's DeFi Summer period and wound down operations around 2023 as the yield farming landscape evolved.",
    category: "solana-ecosystem",
    aliases: ["Tulip"],
    related: ["yield-farming", "raydium", "orca"]
  },
  {
    id: "port-finance",
    term: "Port Finance",
  ...
  },
  {
    ...
    related: ["defi-general", "yield-farming", "sol"]
  },
  {
    id: "hxro-network",
    term: "Hxro Network",
    definition: "A derivatives infrastructure layer on Solana providing a shared on-chain risk engine and order book for parimutuel and perpetual futures markets. Hxro enables other protocols to build derivatives products on top of its infrastructure rather than creating their own matching and risk systems. The network supports prediction markets, perpetuals, and structured products through composable smart contracts.",
    category: "solana-ecosystem",
    aliases: ["Hxro", "HXRO"],
    related: ["perpetuals", "dex", "defi-general"]
  },
  {
    id: "blazestake",
    term: "BlazeStake",
    definition: "A liquid staking protocol on Solana distributing stake across 200+ validators to promote network decentralization. BlazeStake issues bSOL as its liquid staking token, which accrues staking rewards while remaining composable across Solana DeFi. The protocol uses an algorithmic stake distribution strategy that prioritizes smaller validators to reduce stake concentration among top operators.",
    category: "solana-ecosystem",
    aliases: ["bSOL"],
    related: ["liquid-staking", "validator", "sanctum"]
  },
  {
    id: "solblaze",
    term: "SolBlaze",
    definition: "The team behind BlazeStake liquid staking, also building custom liquid staking solutions and the SolBlaze Stake Pool SDK for Solana validators and protocols. SolBlaze provides infrastructure for projects to create their own branded liquid staking tokens and manage stake pool operations, contributing to the diversification of Solana's liquid staking ecosystem beyond single-provider solutions.",
    category: "solana-ecosystem",
    related: ["blazestake", "liquid-staking", "validator"]
  },
  {
    id: "jpool",
    term: "JPool",
    definition: "A liquid staking protocol on Solana focusing on decentralized stake distribution across validators. JPool issues JSOL as its liquid staking token, which represents staked SOL plus accumulated rewards. The protocol emphasizes validator diversity and uses performance-based algorithms to distribute stake among qualifying validators, contributing to Solana's network decentralization goals.",
    category: "solana-ecosystem",
    aliases: ["JSOL"],
    related: ["liquid-staking", "validator", "sanctum"]
  },
  {
    ...
    category: "solana-ecosystem",
    related: ["lending", "marginfi", "kamino"]
  },
  {
    id: "saber",
    term: "Saber",
    definition: "The first major stableswap AMM on Solana, using a Curve-style invariant for efficient swaps between same-peg assets such as stablecoins and wrapped token variants. Saber reached peak TVL of over $4B in 2021, becoming the largest protocol on Solana by TVL at that time. The protocol pioneered the stableswap model on Solana but declined significantly by 2023 as liquidity migrated to newer platforms.",
    category: "solana-ecosystem",
    aliases: ["SBR"],
    related: ["amm", "stablecoin", "liquidity-pool"]
  },
  {
    id: "jupiter-lfg",
    term: "Jupiter LFG Launchpad",
    definition: "Jupiter's decentralized token launch platform using dynamic bonding curves and community voting to curate new token launches on Solana. LFG replaces traditional IDO and ICO models by allowing JUP holders to vote on which projects get featured, combined with a bonding curve mechanism for fair price discovery. Launched projects receive deep liquidity integration across Jupiter's swap infrastructure.",
    category: "solana-ecosystem",
    aliases: ["LFG Launchpad"],
    related: ["jupiter", "bonding-curve", "dex"]
  },
  {
    id: "hawksight",
    term: "Hawksight",
    definition: "An automated DeFi strategy vault on Solana that manages concentrated liquidity positions across Orca, Raydium, and Meteora pools with auto-rebalancing. Hawksight optimizes LP positions by dynamically adjusting price ranges based on market conditions, compounding fees, and rebalancing when prices move outside the active range, reducing the active management burden for liquidity providers.",
    category: "solana-ecosystem",
    related: ["liquidity-pool", "orca", "raydium"]
  },
  {
    id: "armada",
    term: "Armada",
    definition: "Token launch infrastructure on Solana providing customizable launch pools, vesting schedules, and staking programs for new projects. Armada offers a suite of composable smart contracts that handle token distribution mechanics including cliff vesting, linear unlock schedules, and staking rewards, enabling projects to manage their token economics without building custom launch infrastructure.",
    category: "solana-ecosystem",
    related: ["defi-general", "sol", "dapp"]
  },
  {
    id: "access-protocol",
    term: "Access Protocol",
    definition: "A content monetization protocol on Solana where creators stake ACS tokens and subscribers lock tokens to access premium content, replacing traditional paywalls with a staking-based model. Subscribers maintain access as long as their tokens remain locked, earning staking yields while supporting creators. The protocol enables a non-extractive relationship between content producers and their audience.",
    category: "solana-ecosystem",
    aliases: ["ACS"],
    related: ["dapp", "sol", "web3"]
  },
  {
    id: "mean-finance",
    term: "Mean Finance",
    definition: "A DeFi protocol on Solana offering dollar-cost averaging (DCA) and money streaming services. Mean Finance's MeanFi app enables users to set up automated recurring token purchases and stream payments over time, combining CeFi-style scheduled transactions with DeFi self-custody. The protocol supports customizable DCA intervals and real-time payment streaming for payroll and subscriptions.",
    category: "solana-ecosystem",
    aliases: ["MeanFi"],
    related: ["defi-general", "jupiter", "sol"]
  },
  {
    id: "symmetry",
    term: "Symmetry",
    definition: "An on-chain asset management protocol on Solana enabling creation of token baskets, indices, and actively managed funds using automated rebalancing. Symmetry allows anyone to create and manage diversified crypto portfolios as on-chain funds that others can invest in, with transparent rebalancing rules and fee structures enforced by smart contracts rather than centralized fund managers.",
    category: "solana-ecosystem",
    related: ["defi-general", "dex", "sol"]
  },
  {
    id: "rain-fi",
    term: "Rain.fi",
    definition: "An NFT lending protocol on Solana offering instant loans against NFT collateral with floor-price-based loan-to-value ratios. Rain.fi enables NFT holders to access liquidity without selling their assets by borrowing against collection floor prices. The protocol provides automated liquidation mechanisms and supports major Solana NFT collections as collateral for peer-to-pool lending.",
    category: "solana-ecosystem",
    aliases: ["Rain"],
    related: ["nft", "lending", "defi-general"]
  },
  {
    id: "banx",
    term: "Banx",
    definition: "A peer-to-peer NFT lending platform on Solana enabling holders to borrow against NFTs using customizable loan terms and perpetual loan structures. Unlike pool-based lending, Banx connects individual lenders and borrowers directly, allowing negotiation of interest rates, durations, and LTV ratios. The platform supports perpetual loans that auto-renew until either party exits the position.",
    category: "solana-ecosystem",
    related: ["nft", "lending", "defi-general"]
  },
  {
  ...
  },
  {
    id: "cogent-staking",
    term: "Cogent Crypto",
    definition: "A high-performance Solana validator operator known for low commission rates, reliable infrastructure, and active community engagement. Cogent Crypto operates validator nodes and RPC infrastructure, contributing to Solana network security and decentralization. The team provides staking services for delegators seeking consistently performing validators with transparent operations and strong uptime records.",
    category: "solana-ecosystem",
    related: ["validator", "stake", "sol"]
  },
  {
    id: "laine-validator",
    term: "Laine",
    definition: "A prominent Solana validator operator and staking service provider known for building the StakeWiz analytics platform, participating in community governance, and advocating for validator ecosystem improvements. Laine operates high-performance validator infrastructure and actively contributes to Solana governance discussions around stake distribution, commission policies, and network decentralization.",
    category: "solana-ecosystem",
    related: ["validator", "stakewiz", "stake"]
  },
  {
    id: "stakewiz",
    term: "StakeWiz",
    definition: "A Solana validator analytics platform built by Laine providing performance metrics, commission tracking, APY comparisons, and stake distribution data. StakeWiz helps delegators evaluate and choose validators based on historical performance, skip rates, voting records, and commission changes, serving as one of the primary tools for informed staking decisions on Solana.",
    category: "solana-ecosystem",
    related: ["validator", "laine-validator", "stake"]
  },
  {
    id: "aurory",
    term: "Aurory",
    definition: "A play-to-earn RPG game on Solana featuring Aurorians NFT characters in tactical turn-based combat. Aurory combines on-chain asset ownership with gameplay through an NFT marketplace and token economy. Players can earn AURY tokens through gameplay, trade NFT characters and items, and participate in a player-driven economy built on Solana's low-fee infrastructure.",
    category: "solana-ecosystem",
    aliases: ["AURY"],
    related: ["gaming", "nft", "sol"]
  },
  {
    id: "genopets",
    term: "Genopets",
    definition: "A move-to-earn RPG on Solana combining physical activity tracking with game progression. Players evolve their Genopet NFTs by walking in the real world, with step data from mobile devices translated into in-game experience points and evolution stages. The game merges fitness incentives with blockchain gaming through habitat NFTs, crafting systems, and a dual-token economy on Solana.",
    category: "solana-ecosystem",
    related: ["gaming", "nft", "sol"]
  },
  {
    id: "stepn",
    term: "STEPN",
    definition: "A move-to-earn fitness app originally launched on Solana where users purchase NFT sneakers and earn GST and GMT tokens by walking or running. STEPN reached peak activity in mid-2022 with millions of users, becoming one of the most visible Solana consumer applications. The project later expanded to multi-chain support but remains closely associated with the Solana ecosystem's mainstream adoption push.",
    category: "solana-ecosystem",
    aliases: ["GMT", "GST"],
    related: ["gaming", "nft", "sol"]
  },
  {
    id: "nyan-heroes",
    term: "Nyan Heroes",
    definition: "A free-to-play hero shooter game built on Solana featuring cat characters piloting mechs in competitive multiplayer combat. Nyan Heroes integrates NFT-based cosmetics, in-game items, and a token economy while maintaining a free-to-play model that does not require crypto ownership to play. The game targets mainstream gamers with blockchain elements as optional enhancements rather than requirements.",
    category: "solana-ecosystem",
    related: ["gaming", "nft", "sol"]
  },
  {
    id: "photo-finish",
    term: "Photo Finish LIVE",
    definition: "A horse racing game on Solana where players breed, train, and race NFT horses with real prize pools funded by entry fees. Photo Finish features genetic breeding mechanics that produce offspring with inherited traits, a functioning race economy, and player-owned stables. The game launched with a working game loop and demonstrates sustainable blockchain gaming economics on Solana.",
    category: "solana-ecosystem",
    related: ["gaming", "nft", "sol"]
  },
  {
    id: "drip-haus",
    term: "DRiP",
    definition: "A free digital art distribution platform on Solana delivering weekly compressed NFT drops from artists to collectors at no cost. DRiP uses Solana's state compression technology to mint and distribute millions of NFTs for fractions of a cent each, making it economically viable to give away art at scale. The platform has become a major onboarding tool, introducing hundreds of thousands of users to Solana wallets and NFTs.",
    category: "solana-ecosystem",
    aliases: ["DRiP Haus"],
    related: ["compressed-nft", "nft", "sol"]
  },
  {
    id: "okay-bears",
    term: "Okay Bears",
    definition: "A blue-chip Solana NFT collection of 10,000 bear PFPs that briefly became the top NFT collection across all blockchains by trading volume in April 2022. Known for its clean, minimalist art style, Okay Bears demonstrated that Solana NFT projects could compete with Ethereum collections at the highest level. The collection helped establish Solana as a viable chain for premium NFT launches.",
    category: "solana-ecosystem",
    related: ["nft", "magic-eden", "degods"]
  },
  {
    id: "solana-monkey-business",
    term: "Solana Monkey Business",
    definition: "One of the earliest Solana NFT collections, launched in June 2021 featuring 5,000 pixel art monkeys. Solana Monkey Business (SMB) pioneered the Solana NFT ecosystem and gave rise to MonkeDAO, the first NFT-holder DAO on Solana. The collection holds historical significance as a foundational project that proved the viability of NFTs on Solana before the ecosystem's rapid growth.",
    category: "solana-ecosystem",
    aliases: ["SMB"],
    related: ["nft", "monkedao", "magic-eden"]
  },
  {
    id: "teleport",
    term: "Teleport",
    ...
    category: "solana-ecosystem",
    related: ["depin", "sol", "dapp"]
  },
  {
    id: "roam",
    term: "Roam",
    definition: "A decentralized global WiFi roaming network on Solana where users share WiFi access points and earn tokens for providing connectivity. Roam creates a community-owned wireless layer by incentivizing individuals and businesses to open their WiFi networks to roaming users, building a decentralized alternative to traditional roaming agreements between telecom providers.",
    category: "solana-ecosystem",
    related: ["depin", "helium", "sol"]
  },
  {
    id: "genesysgo",
    term: "GenesysGo",
    definition: "The infrastructure company behind Shadow Drive (decentralized storage) and Shadow RPC services on Solana. GenesysGo pivoted from validator operations to building decentralized infrastructure, with Shadow Drive providing an on-chain storage solution where files are stored across a distributed network of operators. The platform offers S3-compatible storage APIs with on-chain proof of storage verification.",
    category: "solana-ecosystem",
    aliases: ["Shadow Drive", "Shadow RPC"],
    related: ["rpc", "depin", "sol"]
  },
  {
    id: "bonfida",
    term: "Bonfida",
  ...
  },
  {
    ...
    term: "Solana Name Service",
    definition: "A domain name system on Solana enabling .sol domains that map human-readable names to wallet addresses, built and maintained by Bonfida. SNS supports subdomains, custom records for social profiles, websites, and other metadata. Users can register, transfer, and trade .sol domains on secondary markets, providing a decentralized identity layer for the Solana ecosystem.",
    category: "solana-ecosystem",
    aliases: ["SNS", ".sol domains"],
    related: ["bonfida", "dapp", "web3"]
  },
  {
    id: "tiplink",
    term: "TipLink",
    definition: "A Solana payment link service that lets anyone send crypto via URL without requiring the recipient to have an existing wallet. Senders create a link containing tokens, and recipients claim them through a simple web interface that generates a wallet automatically. TipLink dramatically lowers the onboarding barrier for new crypto users by removing the need to install wallet software before receiving funds.",
    category: "solana-ecosystem",
    related: ["solana-pay", "sol", "wallet-general"]
  },
  {
    id: "sphere-pay",
    term: "Sphere",
    definition: "Payment infrastructure on Solana enabling merchants to accept crypto payments with fiat settlement, recurring billing, and customizable payment links. Sphere targets e-commerce and SaaS businesses with Stripe-like APIs for Solana payments, handling the complexity of on-chain transactions while providing merchants with familiar payment processing workflows and automatic currency conversion.",
    category: "solana-ecosystem",
    related: ["solana-pay", "sol", "dapp"]
  },
  {
    id: "helio-pay",
    term: "Helio",
    definition: "A Solana payment gateway used by e-commerce platforms, creators, and businesses for accepting crypto payments. Helio offers a Shopify plugin, pay links, subscription billing, and NFT-gated payment flows. The platform simplifies merchant onboarding to Solana payments with embeddable checkout widgets and supports multiple tokens including SOL, USDC, and other SPL tokens.",
    category: "solana-ecosystem",
    related: ["solana-pay", "sol", "spl-token"]
  },
  {
    id: "code-app",
    term: "Code",
    definition: "A mobile payments app built on Solana using a privacy-preserving protocol with time-locked accounts for instant, fee-free payments. Code abstracts blockchain complexity behind a simple cash-like UX where users send and receive payments by scanning codes, without needing to understand wallets, gas fees, or transaction signing. The app targets mainstream consumers who want simple peer-to-peer payments.",
    category: "solana-ecosystem",
    related: ["solana-pay", "sol", "wallet-general"]
  },
  {
    id: "grape-protocol",
    term: "Grape Protocol",
    definition: "A community management and identity verification tool on Solana enabling token-gated access, reputation scoring, and community analytics for DAOs and projects. Grape allows communities to set membership requirements based on token holdings, NFT ownership, or on-chain activity, creating verified community spaces with tiered access levels and contributor reputation tracking.",
    category: "solana-ecosystem",
    related: ["dao", "web3", "dapp"]
  },
  {
    id: "monkedao",
    term: "MonkeDAO",
    definition: "The first NFT-holder DAO on Solana, created by Solana Monkey Business (SMB) NFT holders. MonkeDAO operates validator nodes generating staking revenue, organizes community events, and makes venture investments on behalf of its members. The DAO demonstrated that NFT communities could evolve beyond collectibles into functioning organizations with real economic activity and governance on Solana.",
    category: "solana-ecosystem",
    related: ["dao", "solana-monkey-business", "validator"]
  },
  {
    id: "jupiter-dao",
    term: "Jupiter DAO",
    definition: "The decentralized governance body for Jupiter Exchange using JUP tokens for voting on protocol decisions. Jupiter DAO conducts votes on protocol fees, ecosystem grants, token buybacks and burns, and strategic initiatives through an active governance process. The DAO represents one of the largest and most active governance communities on Solana with regular proposal cycles and high voter participation.",
    category: "solana-ecosystem",
    aliases: ["JUP DAO"],
    related: ["jupiter", "dao", "governance-token"]
  },
  {
    id: "metaplex-foundation",
    term: "Metaplex Foundation",
    ...
    category: "solana-ecosystem",
    aliases: ["Metaplex"],
    related: ["nft", "compressed-nft", "metaplex-aura"]
  },
  {
    id: "birdeye",
    term: "Birdeye",
    definition: "A leading Solana token analytics platform providing real-time price charts, trading volume, holder distribution data, and wallet activity tracking across all Solana DEXes. Birdeye aggregates on-chain data to offer comprehensive market intelligence including new token discovery, trending pairs, and portfolio tracking. It serves as a primary research tool for Solana traders and investors.",
    category: "solana-ecosystem",
    related: ["dex", "sol", "jupiter"]
  },
  {
    id: "solscan",
    term: "Solscan",
    definition: "A popular Solana block explorer offering detailed transaction views, token analytics, account inspection, DeFi activity tracking, and developer API access. Solscan provides human-readable transaction parsing, labeled accounts for known protocols, and comprehensive token holder data. It is one of the most widely used tools for exploring Solana on-chain activity alongside the official Solana Explorer.",
    category: "solana-ecosystem",
    related: ["rpc", "sol", "spl-token"]
  },
  {
    id: "solanafm",
    term: "SolanaFM",
    definition: "A Solana block explorer and indexer providing human-readable transaction parsing, account labeling, and developer APIs for on-chain data access. SolanaFM differentiates itself with an IDL-aware transaction decoder that can parse program-specific instructions into readable formats, making complex DeFi transactions understandable for users and developers investigating on-chain activity.",
    category: "solana-ecosystem",
    related: ["rpc", "sol", "indexer"]
  },
  {
    id: "xray-explorer",
    term: "XRAY",
    definition: "An open-source Solana transaction explorer built by Helius providing clean visualization of complex transactions with human-readable descriptions and labeled accounts. XRAY breaks down multi-instruction transactions into understandable steps, showing token transfers, program interactions, and account changes in an intuitive interface. Its open-source codebase allows community contributions and self-hosting.",
    category: "solana-ecosystem",
    related: ["helius", "rpc", "sol"]
  },
  {
    id: "dexscreener",
    term: "DEX Screener",
    definition: "A multi-chain DEX analytics platform widely used on Solana for tracking new token launches, price charts, liquidity data, and trading activity in real time. DEX Screener provides instant visibility into new trading pairs as they appear on Solana DEXes, making it an essential tool for traders monitoring token launches on platforms like Raydium, Orca, and Pump.fun.",
    category: "solana-ecosystem",
    related: ["dex", "raydium", "pump-fun"]
  },
  {
    id: "rugcheck",
    term: "Rugcheck",
    definition: "A Solana token safety analysis tool that scans token contracts for red flags including concentrated holdings, unlocked liquidity, retained mint authority, frozen accounts, and other rug pull indicators. Rugcheck provides a risk score for any SPL token, helping traders evaluate token safety before investing. It became an essential due diligence tool during the 2024 memecoin trading surge on Solana.",
    category: "solana-ecosystem",
    related: ["spl-token", "dex", "pump-fun"]
  },
  {
    id: "defined-fi",
    term: "Defined.fi",
    definition: "A real-time DEX data and analytics API provider covering Solana and other chains. Defined.fi offers token price feeds, wallet analytics, trading pair data, and historical market data through developer-friendly APIs. The platform serves as infrastructure for trading bots, portfolio trackers, and analytics dashboards that need reliable, low-latency on-chain market data across multiple blockchains.",
    category: "solana-ecosystem",
    related: ["dex", "rpc", "indexer"]
  },
  {
    id: "bubblemaps",
    term: "Bubblemaps",
    definition: "A visual token holder analysis tool that displays wallet clusters and concentration patterns as interactive bubble diagrams. On Solana, Bubblemaps is used to identify insider wallets, connected accounts, and supply distribution by mapping relationships between holders. Traders use it to detect potential manipulation by visualizing whether large portions of supply are controlled by related wallet clusters.",
    category: "solana-ecosystem",
    related: ["spl-token", "dex", "sol"]
  },
  {
    id: "mayan-finance",
    term: "Mayan Finance",
    definition: "A cross-chain swap protocol built on Wormhole enabling token transfers between Solana, Ethereum, and other chains with competitive rates through auction-based pricing. Mayan uses a relay system where solvers compete to fulfill cross-chain swaps at the best rates, combining Wormhole's messaging security with market-driven pricing to offer efficient cross-chain asset transfers.",
    category: "solana-ecosystem",
  ...
  },
  {
    id: "portal-bridge",
    term: "Portal Bridge",
    definition: "Wormhole's user-facing bridge interface for transferring tokens and NFTs between Solana and 20+ blockchains. Portal Bridge uses Wormhole's guardian network of 19 validators to verify cross-chain messages and secure asset transfers. It was one of the earliest production cross-chain bridges for Solana and remains a primary interface for moving assets between Solana and other ecosystems.",
    category: "solana-ecosystem",
    aliases: ["Portal"],
    related: ["bridge", "wormhole", "interoperability"]
  },
  {
    id: "hashflow",
    term: "Hashflow",
    definition: "A cross-chain DEX aggregator offering zero-slippage RFQ (Request for Quote) based trading across Solana and EVM chains. Hashflow uses professional market makers to provide guaranteed quotes rather than AMM-based pricing, eliminating slippage and MEV exposure for traders. The protocol enables cross-chain swaps without requiring traditional bridge transfers through its market maker network.",
    category: "solana-ecosystem",
    aliases: ["HFT"],
    related: ["dex", "bridge", "interoperability"]
  },
  {
    id: "solana-labs",
    term: "Solana Labs",
    definition: "The original development company that created the Solana blockchain, co-founded by Anatoly Yakovenko and Raj Gokal in 2018. Solana Labs developed the initial validator client, designed Proof of History, and launched the network in March 2020. In 2024, the company spun off validator client development to Anza, shifting focus to ecosystem growth, incubation, and strategic initiatives.",
    category: "solana-ecosystem",
    related: ["sol", "agave", "anza"]
  },
  {
    id: "jump-crypto",
    term: "Jump Crypto",
    definition: "The cryptocurrency division of Jump Trading, a major quantitative trading firm. Jump Crypto built the Firedancer validator client for Solana, representing a massive infrastructure investment in the network. The team also contributed to Wormhole's development and has been one of the most significant engineering and capital contributors to the Solana ecosystem since its early stages.",
    category: "solana-ecosystem",
    related: ["firedancer", "wormhole", "validator"]
  },
  {
    id: "multicoin-capital",
    term: "Multicoin Capital",
  ...
  {
    id: "solana-ventures",
    term: "Solana Ventures",
    definition: "An investment arm focused on funding Solana ecosystem projects across DeFi, NFTs, gaming, and infrastructure. Solana Ventures co-launched the $100M Solana Gaming Fund and participated in numerous ecosystem investments to accelerate adoption. The fund operated as a strategic capital deployment vehicle to bootstrap critical ecosystem verticals during Solana's early growth period.",
    category: "solana-ecosystem",
    related: ["solana-labs", "solana-foundation", "gaming"]
  },
  {
    id: "glow-wallet",
    term: "Glow Wallet",
    definition: "A Solana wallet emphasizing speed and developer experience with a clean interface, built-in token swap functionality, and experimental features for power users. Glow Wallet focuses on fast transaction signing, minimal UI friction, and support for advanced Solana features, positioning itself as an alternative to Phantom for users who prioritize performance and developer-oriented tooling.",
    category: "solana-ecosystem",
    related: ["wallet-general", "phantom", "solflare"]
  },
  {
    id: "stripe-solana",
    term: "Stripe Solana Integration",
    definition: "Payment processor Stripe's support for USDC on Solana, enabling businesses to accept and settle crypto payments through Stripe's existing merchant infrastructure. The integration allows millions of Stripe merchants to receive USDC payments on Solana with automatic fiat conversion, representing a major milestone for Solana's adoption in mainstream commerce and payment processing.",
    category: "solana-ecosystem",
    related: ["solana-pay", "stablecoin", "sol"]
  },
  {
    id: "shopify-solana",
    term: "Shopify Solana",
    ...
    category: "solana-ecosystem",
    related: ["solana-pay", "nft", "sol"]
  },
  {
    id: "dogwifhat",
    term: "dogwifhat",
    definition: "A major Solana memecoin (ticker: WIF) featuring a Shiba Inu dog wearing a pink knitted hat. Dogwifhat rose to become a top-5 memecoin by market cap in early 2024, symbolizing the resurgence of memecoin culture on Solana. The token became a cultural icon of Solana's vibrant trading community and helped establish Solana as the primary chain for memecoin activity.",
    category: "solana-ecosystem",
    aliases: ["WIF", "$WIF"],
    related: ["bonk", "pump-fun", "sol"]
  },
  {
    id: "samo",
    term: "SAMO",
    definition: "Solana's self-styled ambassador dog memecoin, launched in April 2021 as one of the earliest memecoins on the network. The SAMO community positions the project as an ecosystem education and adoption vehicle, hosting events, producing content, and onboarding new users to Solana. SAMO maintains one of the longest-running memecoin communities on Solana with a focus on positive ecosystem contribution.",
    category: "solana-ecosystem",
    aliases: ["Samoyed Coin"],
    related: ["bonk", "sol", "web3"]
  },
  {
    id: "wormhole-queries",
    term: "Wormhole Queries",
    ...
    category: "solana-ecosystem",
    related: ["wormhole", "interoperability", "bridge"]
  },
  {
    id: "jupiter-perpetuals",
    term: "Jupiter Perpetuals",
    definition: "Jupiter's perpetual futures trading platform using a pool-based model (JLP) where liquidity providers deposit assets to earn trading fees while traders access up to 100x leverage on SOL, ETH, and BTC. JLP holders earn real yield from trading fees and liquidations, making it one of the highest-yield opportunities on Solana. The platform integrates directly with Jupiter's swap infrastructure.",
    category: "solana-ecosystem",
    aliases: ["JLP"],
    related: ["jupiter", "perpetuals", "defi-general"]
  },
  {
    id: "sanctum-infinity",
    term: "Sanctum Infinity",
  ...
  },
  {
    ...
    term: "Famous Fox Federation",
    definition: "A Solana NFT collection and ecosystem tools builder featuring fox-themed digital collectibles. The team created utility tools including token staking platforms, holder verification systems, and the Transdimensional Fox Federation collection. Famous Fox Federation contributed tooling infrastructure used by other Solana NFT projects and built one of the more engaged NFT communities on the network.",
    category: "solana-ecosystem",
    aliases: ["FFF"],
    related: ["nft", "magic-eden", "sol"]
  },
  {
    id: "streamflow",
    term: "Streamflow",
    definition: "A token vesting and payroll streaming protocol on Solana enabling time-locked token distribution, cliff vesting, and real-time payment streams for teams and DAOs. Streamflow handles token unlock schedules for project launches, employee compensation with linear or custom vesting curves, and continuous payment streaming where recipients can withdraw accumulated tokens at any time.",
    category: "solana-ecosystem",
    related: ["dao", "sol", "spl-token"]
  },
  {
    id: "dual-finance",
    term: "Dual Finance",
    definition: "A DeFi options protocol on Solana enabling projects to distribute tokens through option-based incentive programs called Staking Options rather than direct airdrops. Dual Finance allows protocols to offer staking rewards as options contracts, giving recipients the right to purchase tokens at a discount. This model aligns incentives by rewarding committed stakeholders over passive airdrop recipients.",
    category: "solana-ecosystem",
    related: ["defi-general", "yield-farming", "airdrop-crypto"]
  },
  {
    id: "jito-foundation",
    term: "Jito Foundation",
    definition: "The organization governing the Jito protocol ecosystem including JitoSOL liquid staking, MEV rewards distribution, and the JTO governance token. The Jito Foundation oversees protocol upgrades, manages the treasury, and conducts governance votes on fee structures and ecosystem grants. JitoSOL distinguishes itself by passing MEV rewards to stakers in addition to standard staking yield.",
    category: "solana-ecosystem",
    aliases: ["Jito", "JTO"],
    related: ["jito-sol", "liquid-staking", "validator"]
  },
  {
    id: "chorusone",
    term: "Chorus One",
    definition: "A major institutional staking provider running Solana validators alongside 50+ other proof-of-stake networks. Chorus One is known for MEV research and development on Solana, operating the OPUS staking platform for institutional clients. The team contributes to Solana validator ecosystem improvements and provides enterprise-grade staking infrastructure for funds, exchanges, and custodians.",
    category: "solana-ecosystem",
    related: ["validator", "stake", "sol"]
  },
  {
    id: "everstake",
    term: "Everstake",
    definition: "One of the largest staking providers by number of networks supported, operating Solana validators with enterprise-grade infrastructure serving both institutional and retail delegators. Everstake manages billions in staked assets across multiple proof-of-stake networks and provides Solana users with reliable validator operations, staking analytics, and portfolio management tools for their delegated SOL.",
    category: "solana-ecosystem",
    related: ["validator", "stake", "sol"]
  },
  {
    id: "pyth-data-association",
    term: "Pyth Data Association",
    definition: "A Swiss-based entity governing the Pyth oracle network, managing the PYTH governance token, publisher onboarding, and protocol governance decisions. The association coordinates between data publishers (financial institutions, exchanges, trading firms) and consuming protocols to ensure oracle data quality, uptime, and accuracy across the Pyth network's multi-chain oracle infrastructure originating from Solana.",
    category: "solana-ecosystem",
    related: ["pyth", "oracle", "governance-token"]
  },
  {
    id: "squads-labs",
    term: "Squads Labs",
    ...
    category: "solana-ecosystem",
    related: ["squads", "multisig", "wallet-general"]
  },
  {
    id: "helius-labs",
    term: "Helius Labs",
    definition: "A Solana infrastructure company providing RPC nodes, webhooks, DAS API (Digital Asset Standard), and developer tools. Helius is one of the most widely used Solana RPC and indexing providers, offering enhanced APIs for compressed NFTs, transaction parsing, and real-time event streaming. The team also built the XRAY explorer and contributes to Solana developer tooling and documentation.",
    category: "solana-ecosystem",
    related: ["helius", "rpc", "indexer"]
  },
  {
    id: "crossmint",
    term: "Crossmint",
    definition: "An NFT infrastructure-as-a-service platform on Solana and other chains providing APIs for minting, custodial wallets, and credit card NFT purchases. Crossmint enables enterprises and developers to integrate NFT functionality into applications without requiring users to hold crypto or manage wallets. The platform abstracts blockchain complexity for brands, games, and loyalty programs adopting digital collectibles.",
    category: "solana-ecosystem",
    related: ["nft", "compressed-nft", "wallet-general"]
  },
  {
    id: "underdog-protocol",
    term: "Underdog Protocol",
    ...
    category: "solana-ecosystem",
    related: ["compressed-nft", "nft", "dapp"]
  },
  {
    id: "metadao",
    term: "MetaDAO",
    definition: "A governance experimentation project on Solana implementing futarchy, a decision-making model where conditional token markets determine policy outcomes instead of traditional voting. In MetaDAO's system, proposals are evaluated by market participants trading conditional tokens that represent outcomes if the proposal passes or fails. The market price signals which outcome the market expects to create more value.",
    category: "solana-ecosystem",
    related: ["dao", "governance-token", "defi-general"]
  },
  {
    id: "cubik",
    term: "Cubik",
    definition: "A quadratic funding platform on Solana connecting open-source projects with community funding through matching pools, inspired by Gitcoin's model but built natively on Solana. Cubik uses quadratic funding mathematics where many small contributions receive larger matching amounts than fewer large donations, democratically amplifying community preferences for which public goods receive the most funding.",
    category: "solana-ecosystem",
    related: ["dao", "solana-foundation", "sol"]
  },
  {
    id: "ore",
    term: "Ore",
    definition: "A proof-of-work mining token on Solana where miners compete to find valid hashes using a smart contract that implements Bitcoin-style mining mechanics on a proof-of-stake chain. Ore mining involves submitting hash solutions to an on-chain program, with difficulty adjusting based on participation. The project attracted significant attention and briefly caused Solana network congestion due to high mining transaction volume.",
    category: "solana-ecosystem",
    related: ["sol", "svm", "dapp"]
  },
  {
    id: "send-token",
    term: "Send",
    definition: "A Solana-native messaging and payments token enabling wallet-to-wallet transfers through a simplified mobile interface focused on consumer accessibility. Send aims to make crypto payments as intuitive as messaging apps by combining a social graph with payment functionality, targeting mainstream users who want to send and receive tokens without navigating complex DeFi interfaces.",
    category: "solana-ecosystem",
    related: ["solana-pay", "sol", "wallet-general"]
  },
  {
    id: "tensor-foundation",
    term: "Tensor Foundation",
    definition: "The organization governing the Tensor NFT marketplace and its TNSR governance token. Tensor Foundation oversees protocol development, community grants, and ecosystem initiatives for the leading Solana NFT trading platform. The foundation manages TNSR token distribution and governance processes that guide Tensor's product roadmap and fee structures.",
    category: "solana-ecosystem",
    related: ["tensor", "nft", "governance-token"]
  },
  {
    id: "backpack-exchange-eu",
    term: "Backpack MiFID License",
    definition: "Backpack Exchange's EU regulatory milestone, obtaining a MiFID II license through its acquisition of FTX EU in 2024. This made Backpack one of the few crypto-native exchanges with European regulated securities trading capabilities, enabling it to offer tokenized securities and regulated crypto products to EU customers alongside its Solana-focused spot and perpetual markets.",
    category: "solana-ecosystem",
    related: ["backpack-exchange", "backpack-wallet", "cex"]
  },
  {
    id: "solayer",
    term: "Solayer",
    definition: "A restaking protocol on Solana enabling users to restake their SOL or liquid staking tokens to secure additional services and earn extra yield. Solayer extends Solana's economic security model by allowing staked assets to simultaneously secure multiple protocols and infrastructure services, similar to EigenLayer on Ethereum but natively built for the Solana ecosystem.",
    category: "solana-ecosystem",
    related: ["restaking", "liquid-staking", "sol"]
  },
  {
    id: "jito-restaking",
    term: "Jito Restaking",
    ...
    category: "solana-ecosystem",
    related: ["jito-foundation", "jito-sol", "restaking"]
  },
  {
    id: "phoenix-wallet",
    term: "Phoenix Wallet",
    definition: "A Solana wallet focused on DeFi power users with built-in limit orders, DCA strategies, and advanced transaction simulation. Phoenix Wallet integrates directly with Jupiter's routing for optimal swap execution and provides real-time portfolio analytics alongside standard wallet functionality for daily Solana users.",
    category: "solana-ecosystem",
    related: ["wallet-general", "jupiter", "dex"]
  },
  {
    id: "switchboard-on-demand",
    term: "Switchboard On-Demand",
    ...
    category: "solana-ecosystem",
    related: ["switchboard", "oracle", "pyth"]
  },
  {
    id: "marinade-native-staking",
    term: "Marinade Native",
    ...
    category: "solana-ecosystem",
    related: ["marinade", "stake", "validator"]
  },
  {
    id: "solana-actions-blinks",
    term: "Solana Actions & Blinks",
    ...
    category: "solana-ecosystem",
    related: ["actions", "blinks", "solana-pay"]
  },
  {
    id: "dune-analytics",
    term: "Dune Analytics (Solana)",
    definition: "A blockchain analytics platform that added Solana data support, enabling analysts to write SQL queries against indexed Solana transaction, account, and program data. Dune dashboards provide community-created visualizations of Solana DeFi metrics, protocol TVL, user activity, and network statistics used by researchers, protocols, and investors for on-chain analysis.",
    category: "solana-ecosystem",
    related: ["indexer", "birdeye", "solscan"]
  },
  {
    id: "flipside",
    term: "Flipside Crypto",
    definition: "A blockchain analytics platform providing Solana on-chain data through SQL-queryable datasets and pre-built dashboards. Flipside incentivizes community analysts through bounty programs to create Solana ecosystem analytics covering DEX volumes, protocol metrics, wallet behavior, and network health indicators available to protocols and researchers.",
    category: "solana-ecosystem",
    related: ["indexer", "birdeye", "dune-analytics"]
  },
  {
    id: "phantom-mobile",
    term: "Phantom Mobile",
    definition: "Phantom's mobile wallet application for iOS and Android providing full Solana wallet functionality including token swaps via Jupiter, NFT management, staking, and a built-in dApp browser. Phantom Mobile uses biometric authentication and integrates with Mobile Wallet Adapter for seamless interaction with Solana mobile dApps.",
    category: "solana-ecosystem",
    related: ["phantom", "mobile-wallet-adapter", "wallet-general"]
  },
  {
    id: "helium-mobile",
    term: "Helium Mobile",
    definition: "A mobile carrier service built on the Helium network on Solana, offering cellular coverage through a combination of decentralized 5G hotspots and traditional carrier partnerships (T-Mobile). Subscribers earn MOBILE tokens for contributing to network coverage. Helium Mobile demonstrates a real consumer product powered by DePIN infrastructure on Solana.",
    category: "solana-ecosystem",
    related: ["helium", "depin", "sol"]
  }
];

// src/index.ts
var allTerms = [
  ...core_protocol_default,
  ...programming_model_default,
  ...token_ecosystem_default,
  ...defi_default,
  ...zk_compression_default,
  ...infrastructure_default,
  ...security_default,
  ...dev_tools_default,
  ...network_default,
  ...blockchain_general_default,
  ...web3_default,
  ...programming_fundamentals_default,
  ...ai_ml_default,
  ...solana_ecosystem_default
];
var termMap = new Map(allTerms.map((t) => [t.id, t]));
...
function searchTerms(query) {
  ...
  return allTerms.filter(
    (t) => t.term.toLowerCase().includes(q) || t.definition.toLowerCase().includes(q) || t.id.includes(q) || t.aliases?.some((a) => a.toLowerCase().includes(q))
  );
}
